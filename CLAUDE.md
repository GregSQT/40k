...
RAPPEL DE LEXIQUE — TOUJOURS : toute réponse commence par une ligne « Rappel. » qui redéfinit en une phrase chaque terme propre au projet ou introduit par l'agent qu'elle emploie : noms de runs et d'étapes (P0, P1, S9, B…), mécanismes (température, gate, pool, exploiteur, holdout…), sigles et options (A/B/C, T, B6…). Aucun de ces termes ne s'emploie dans la réponse sans avoir été défini ; « tu sais de quoi on parle » n'est jamais présumé, même au tour suivant. Une réponse d'une ligne (oui/ok) en est dispensée.
