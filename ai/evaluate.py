...
def setup_evaluation_environment():
    ...
    print("🏗️  Setting up evaluation environment with web-compatible replay logging...")
    ...
    print("✅ Evaluation environment enhanced with web-compatible replay logging")
...
def run_evaluation_episode_with_web_replay(model, env, episode_num: int):
    ...
    try:
        ...
        print(f"🎮 Running evaluation episode {episode_num} with web replay capture...")
        ...
        print(f"   ✅ Episode {episode_num}: {outcome}, {steps} steps, reward: {total_reward:.3f}")
        ...
        print(f"   ⚠️  Episode {episode_num} failed: {e}")
        return {
            "episode": episode_num,
            "reward": 0.0,
            "steps": 0,
...
def evaluate_model(model, env, num_episodes: int = 100):
    ...
    print("   ℹ️  Using deterministic policy for consistent evaluation")
    print("   🌐 Generating web-compatible replay files for each episode")
    ...
    for episode_num in range(1, num_episodes + 1):
        ...
        if episode_num % 10 == 0:
            ...
            print(f"   📊 Progress: {episode_num}/{num_episodes} - Win rate: {current_win_rate:.1%}")
...
def save_evaluation_results(results: Dict[str, Any], model_path: str):
    ...
    print(f"📋 RESULTS: Saving evaluation results...")
    ...
    if best_episode.get("replay_file") and os.path.exists(best_episode["replay_file"]):
        ...
        print(f"   🏆 Best episode web replay: {best_replay_dest}")
    else:
    ...
    if worst_episode.get("replay_file") and os.path.exists(worst_episode["replay_file"]):
        ...
        print(f"   📉 Worst episode web replay: {worst_replay_dest}")
    else:
    ...
    print(f"   🌐 All replay files are web-compatible!")
...
def cleanup_temp_episode_files(event_log_dir: str, episodes: List[Dict]):
    ...
    print("🧹 Cleaning up temporary episode replay files...")
    ...
    for episode in episodes:
        if episode["episode"] not in keep_episodes and episode.get("replay_file"):
            try:
                ...
                print(f"   ⚠️  Failed to remove {episode['replay_file']}: {e}")
    
    print(f"   ✅ Removed {removed_count} temporary replay files")
    print(f"   📁 Kept best (episode {best_episode['episode']}) and worst (episode {worst_episode['episode']}) replays")
...
def main():
    ...
    print("🎯 W40K AI Evaluation with Web-Compatible Replay Generation")
    print("=" * 65)
    print(f"📊 Evaluating over {args.episodes} episodes")
    print(f"💾 Model path: {args.model_path}")
    print("🌐 Generating ONLY web-compatible replay files")
    print("❌ NO simplified event logs will be created")
    print("=" * 65)
    
    # Check if model exists
    if not os.path.exists(args.model_path):
        print(f"❌ Model file not found: {args.model_path}")
        print("   🔧 Train a model first: python ai/train.py")
        return False
    ...
    print(f"\n🎯 Results:")
    print(f"   🏆 Win Rate: {results['wins']}/{args.episodes} ({results['wins']/args.episodes:.1%})")
    print(f"   📊 Avg Reward: {sum(results['rewards'])/len(results['rewards']):.3f}")
    print(f"   📁 Replay files: ai/event_log/eval_best_web_replay.json")
    print(f"                    ai/event_log/eval_worst_web_replay.json")
    print(f"\n✅ All replay files are web-compatible - no conversion needed!")
    print(f"\n🌐 Load replays in web app:")
