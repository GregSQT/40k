...
def model_near_objective_or_center(state: Any, mid: str, target_id: str) -> Optional[bool]:
    ...
    if tuple(pos) in state.objective_cells:
        return True
    ...
    if dist <= 6 * ish:
        return True
    # À x1 le socle EST son ancre : hors de l'aire et loin du centre, la clause ne joue pas.
    # À x5 le socle déborde : l'ancre hors de l'aire ne dit rien du socle → indéterminable.
...
def expected_fnp_thresholds(
    ...
    if mid is not None:
        ...
        if th_obj is not None:
            near = model_near_objective_or_center(state, mid, target_id)
