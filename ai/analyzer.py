...
def parse_step_log(filepath: str) -> Dict:
    ...
    reactive_activation_counts: Dict[Tuple[int, int, int], Dict[str, int]] = {}
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if deploy_match:
                ...
                unit_type = unit_types.get(unit_id)
                ...
                unit_player[unit_id] = player
                _position_cache_set(unit_positions, unit_id, dest_col, dest_row)
                positions_at_turn_start[unit_id] = (dest_col, dest_row)
                if unit_id not in unit_movement_history:
                    unit_movement_history[unit_id] = []
                unit_movement_history[unit_id].append({"position": (dest_col, dest_row)})
                continue
            ...
            if match:
                ...
                if hazardous_destroyed_match:
                    ...
                    if destroyed_unit_id in unit_hp and require_key(unit_hp, destroyed_unit_id) > 0:
                        destroyed_unit_type = require_key(unit_types, destroyed_unit_id)
                        ...
                        _position_cache_remove(unit_positions, destroyed_unit_id)
                        unit_deaths.append((turn, phase, destroyed_unit_id, line_number))
                        dead_units_current_episode.add(destroyed_unit_id)
                        _debug_log(
                            f"[DEATH REMOVED] E{current_episode_num} T{turn} {phase} "
                            f"target_id={destroyed_unit_id} target_type={destroyed_unit_type} reason=hazardous_destroyed_line"
                        )
                        del unit_hp[destroyed_unit_id]
                ...
                if reactive_decline_match:
                    ...
                    reactive_player = unit_player.get(reactive_unit_id) if reactive_unit_id in unit_player else player
                ...
                if reactive_move_match:
                    ...
                    reactive_unit_type = require_key(unit_types, reactive_unit_id)
                    key = ("reactive_move", reactive_unit_type)
                    stats['special_rule_usage'][key][reactive_player] += 1
                    reactive_key = (current_episode_num, turn, reactive_player)
                    if reactive_key in reactive_activation_counts:
                        reactive_counts = reactive_activation_counts[reactive_key]
                    else:
                        reactive_counts = {}
                        reactive_activation_counts[reactive_key] = reactive_counts
                    ...
                    if reactive_unit_id not in unit_movement_history:
                        unit_movement_history[reactive_unit_id] = []
                    timestamp_match = re.search(r'\[(\d+:\d+:\d+)\]', line)
                    timestamp = timestamp_match.group(1) if timestamp_match else None
                    unit_movement_history[reactive_unit_id].append({
                    ...
                    if reactive_unit_id not in unit_positions or unit_positions[reactive_unit_id] != (from_col, from_row):
                        _position_cache_set(unit_positions, reactive_unit_id, from_col, from_row)

                    if reactive_unit_id not in unit_hp:
                        stats['parse_errors'].append({
                            'episode': current_episode_num,
                            'turn': turn,
                            'phase': phase,
                            'line': line.strip(),
                            'error': f"Reactive move for unknown unit_id (missing in unit_hp): {reactive_unit_id}"
                        })
                    else:
                        unit_hp_at_reactive = dict(unit_hp)
                        positions_at_reactive = dict(unit_positions)
                        if (from_col, from_row) != (to_col, to_row):
                            if roll_value is not None:
                                ...
                                enemy_adjacent_hexes = _build_enemy_adjacent_hexes(
                                    positions_at_reactive, unit_player, unit_hp_at_reactive, reactive_player
                                )
                            ...
                            if (to_col, to_row) in wall_hexes:
                            ...
                            if require_key(unit_hp, reactive_unit_id) > 0:
                                _position_cache_set(unit_positions, reactive_unit_id, to_col, to_row)
                ...
                if re.search(
                    ...
                    re.IGNORECASE
                        ...
                        if shooter_match and target_match:
                            ...
                            if str(shooter_id) in units_fled:
                                ...
                                shooter_unit_rules_for_flee = require_key(unit_rules_by_type, shooter_unit_type_for_flee)
                                ...
                                if heavy_applied_in_log:
                                    ...
                                    if not heavy_valid:
                                        ...
                                        if key not in invalid_first:
                                            invalid_first[key] = {'episode': current_episode_num, 'line': line.strip()}
                        ...
                        if advance_match:
                            ...
                            # Source of truth at validation time: live reconstructed cache.
                            # Movement history can lag for units that only appear as fight/shoot
                            # targets (no explicit MOVE/ADVANCE/CHARGE entry for that unit),
                            # so do not overwrite fresh cache positions with history values.
                    ...
                    or "FLED from" in action_desc
                        ...
                        if fight_match:
                            ...
                            if fighter_id in unit_hp and require_key(unit_hp, fighter_id) > 0:
                                _position_cache_set(unit_positions, fighter_id, fighter_col, fighter_row)
                            if target_id in unit_hp and require_key(unit_hp, target_id) > 0:
                                _position_cache_set(unit_positions, target_id, target_col, target_row)
...
'''
    ...
    heavy_rule_invalid_usage = sum(
        require_key(counts, 1) + require_key(counts, 2)
