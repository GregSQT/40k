...
def test_a_renamed_anchor_is_detected(tmp_path: pathlib.Path) -> None:
    """Le trou fermé ici : le fichier existe toujours, mais son titre ne porte plus l'ancre citée.

    Le fragment était coupé avant résolution — 45 des 48 liens du corpus en portaient un
    (2026-08-18), donc 94 % des liens n'étaient vérifiés qu'à moitié : renommer une ancre d'un
    fichier sujet laissait l'index intégralement vert.
    """
    write(tmp_path, "bot.md", "# Bot\n\n## Étape 8 {#etape8-renomme}\n")
    doc = write(tmp_path, "note.md", "voir [l'étape 8](bot.md#etape8)\n")
    checked, _skipped, fragments, broken = cdr.check_links(doc)
    assert (checked, fragments) == (1, 1)
    assert len(broken) == 1 and cdr.LOST_ANCHOR in broken[0] and "bot.md#etape8" in broken[0]


def test_a_declared_anchor_is_confirmed(tmp_path: pathlib.Path) -> None:
    """La forme employée par tout le corpus : l'ancre DÉCLARÉE en fin de titre."""
...
@pytest.mark.parametrize("fragment", ["une-section-accentuée", "une-section-accentuee"])
def test_a_title_without_a_declared_anchor_is_reachable(
    tmp_path: pathlib.Path, fragment: str
) -> None:
    """Un titre sans `{#slug}` reste atteignable, sous les DEUX formes que produisent les rendus.

    Aucune spécification unique ne définit le slug d'un titre ; exiger l'ancre déclarée rendrait
    rouge un lien qui fonctionne vraiment, et un contrôle qui crie à tort finit désactivé.
    """
    write(tmp_path, "cible.md", "# Doc\n\n## Une section accentuée\n")
    doc = write(tmp_path, "note.md", f"voir [là](cible.md#{fragment})\n")
    _checked, _skipped, fragments, broken = cdr.check_links(doc)
    assert (fragments, broken) == (1, [])


def test_a_line_fragment_is_left_to_the_anchor_pass(tmp_path: pathlib.Path) -> None:
    """`#L629` cite une LIGNE : la passe 4 la rapporte déjà, sous le bon message.

    Confrontée aux titres, elle sortirait « ancre introuvable » et enverrait chercher un titre là
    où le document cite une ligne — deux messages pour un seul défaut, dont un qui égare.
    """
    write(tmp_path, "cible.md", "# Doc\n")
...
def test_a_fragment_on_a_non_markdown_target_is_not_confronted(tmp_path: pathlib.Path) -> None:
    """Hors `.md`, un fragment ne nomme pas un titre : il n'y a rien à confronter."""
