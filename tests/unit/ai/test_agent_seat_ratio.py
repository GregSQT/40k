...
def test_a_lineage_profile_pins_its_own_seat_ratio() -> None:
    ...
    réglage posé dans le profil par l'utilisateur, le verrou suit la config et non l'inverse.
    """
    assert LINEAGE_PROFILES == {"x1_lineage"}, sorted(LINEAGE_PROFILES)
    assert RESOLVED["x1_lineage"]["agent_seat_p2_ratio"] == pytest.approx(0.75)
