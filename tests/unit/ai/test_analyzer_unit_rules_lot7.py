...
_ROSTERS = "scale=5 AGENT_PLAYER=1 AGENT=sm (ref) OPPONENT=sm (ref)"
...
def _shoot_log(middle: str, tmp_path) -> dict:
    ...
    log_text = entete_step_log(
        body, units=UNITS_TORRENT, objectives=OBJECTIVES, rosters=_ROSTERS,
    )
...
def test_torrent_tir_correct_pas_erreur(tmp_path):
    ...
    stats = _shoot_log("Hit None(None+) [TORRENT] - Wound 5(4+)", tmp_path)
...
def test_torrent_tir_numerique_detecte(tmp_path):
    ...
    stats = _shoot_log("Hit 5(3+) [TORRENT] - Wound 5(4+)", tmp_path)
...
def _fight_log(middle: str, tmp_path) -> dict:
    body = (
        DEPLOY_MELEE_CLOSE
    ...
    log_text = entete_step_log(
        body, units=UNITS_MELEE_CLOSE, objectives=OBJECTIVES, rosters=_ROSTERS,
    )
    return _parse(log_text, tmp_path)
...
def test_torrent_melee_correct_pas_erreur(tmp_path):
    ...
    stats = _fight_log("Hit None(None+) [TORRENT] - Wound 5(4+)", tmp_path)
...
def test_torrent_melee_numerique_detecte(tmp_path):
    ...
    stats = _fight_log("Hit 5(3+) [TORRENT] - Wound 5(4+)", tmp_path)
...
def test_lethal_hits_tir_correct_pas_erreur(tmp_path):
    ...
    stats = _shoot_log("Hit 6(3+) [LETHAL HITS] - Wound None(4+)", tmp_path)
...
def test_lethal_hits_tir_numerique_detecte(tmp_path):
    ...
    stats = _shoot_log("Hit 6(3+) [LETHAL HITS] - Wound 5(4+)", tmp_path)
...
def test_lethal_hits_melee_correct_pas_erreur(tmp_path):
    ...
    stats = _fight_log("Hit 6(3+) [LETHAL HITS] - Wound None(4+)", tmp_path)
...
def test_lethal_hits_melee_numerique_detecte(tmp_path):
    ...
    stats = _fight_log("Hit 6(3+) [LETHAL HITS] - Wound 5(4+)", tmp_path)
