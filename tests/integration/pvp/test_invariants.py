...
class TestUnitHitPoints:
    @pytest.mark.anomaly
    def test_unit_hitpoints_ignore_attached_characters(self, game):
        """t2_anomalie_pv_personnage_attache : les PV d'unité changent de définition en jeu.

        ANOMALIE (sentinelle). La convention du moteur est documentée : ``unit["HP_CUR"]``
        est le total d'escouade HORS personnage attaché (PDF 19, Attached Units). Elle est
        respectée au démarrage — ``nombre_de_figurines × PV_du_profil_de_base`` — donc 5
        unités sur 42 du scénario ``pvp_test`` diffèrent de la somme réelle de leurs
        figurines (ex. 111 WolfGuardTerminator : 21 annoncés pour 26 répartis sur 7
        figurines, dont un Librarian à 5 PV et un Captain à 6).

        Le défaut est la SUITE : à la première figurine tuée, le moteur recalcule le total
        sur les survivantes, personnages COMPRIS — les PV de l'unité AUGMENTENT
        (21 → 23 en tuant une figurine à 3 PV). La grandeur ne veut donc pas dire la même
        chose avant et après la première perte, et tout consommateur du total d'unité
        (score, observation de l'agent, tri de cibles) compare des grandeurs différentes.

        Correctif attendu : une seule définition, tenue partout. Dériver ``HP_CUR`` de la
        somme des figurines vivantes de ``models_cache`` à la construction comme après
        chaque perte — c'est déjà ce qui se produit après la première mort.

        Quand ce sera corrigé, ce test échouera : le supprimer et promouvoir l'égalité en
        invariant transversal dans ``invariants.py`` (elle sera alors vraie partout).
        """
        divergent = []
        for unit in game.state["units"]:
            unit_id = str(unit["id"])
            models = [
                m for m in game.state["squad_models"].get(unit_id, [])
                if m in game.state["models_cache"]
            ]
            total = sum(game.state["models_cache"][m]["HP_CUR"] for m in models)
            if total != unit["HP_CUR"]:
                divergent.append((unit_id, unit["HP_CUR"], total, len(models)))

        assert divergent, "plus aucune divergence : l'anomalie est corrigée, supprimer ce test"
        for unit_id, announced, total, count in divergent:
            assert announced < total, (
                f"unité {unit_id} : {announced} PV annoncés pour {total} répartis sur "
                f"{count} figurines — la divergence a changé de sens, ré-analyser"
            )
