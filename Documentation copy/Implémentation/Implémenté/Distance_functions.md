...
| IA | `ai/analyzer.py` — `get_adjacent_enemies` | ● | | | ⁵ **Ancre 1 hex** + voisinage grille — pas empreinte multi-hex ni **B** moteur / clearance Board ×10. Écarte les unités HORS TABLE (`analyzer_perfig.position_is_on_battlefield`) : la sentinelle `(-1,-1)` est voisine de `(0,0)`. |
...
| `ai.analyzer` adjacence ancre | Décision appliquée : `is_adjacent_to_enemy` / `is_hex_anchor_adjacent_to_enemy` SUPPRIMÉS (aucun appelant hors tests, jumeaux littéraux de `get_adjacent_enemies`) ; restent `get_adjacent_enemies` (**A** ancre) et `is_within_engine_engagement_zone` (**B** moteur) | Un seul filtre hors-table à maintenir au lieu de trois. |
| `handle_wait` (analyzer, phase de tir) | Décision appliquée le 2026-08-05 : ses **trois** mesures d'engagement passent de `is_adjacent` (**A**, ancre + `hex_distance == 1`) à `is_within_engine_engagement_zone` (**B** moteur), en miroir de `handle_shoot`. Le classement `wait`/`skip` suit 10.04 (unengaged), l'exclusion d'une cible suit `target_engaged`, et le gate par arme suit 10.06/17.03 avec les exemptions MONSTER/VEHICLE, absentes jusque-là | **Quatrième** occurrence de la famille « ancre vs par-figurine » (LoS, fight non-adjacent, close-quarters). `handle_shoot` avait été corrigé, `handle_wait` était resté derrière : à x5 un ennemi ENGAGÉ à 8 subhex laissait le WAIT compté comme délibéré. Deux boucles d'énumération écrites à la main disparaissent avec, dont leurs filtres hors-table. Verrou : `tests/unit/ai/test_analyzer_wait_engagement.py`. **Chemin de production vérifié par instrumentation** (pile d'appel, `step.log` réel) : 15 151 évaluations « unité engagée ? » dont **54 vraies**, `target_engaged` vrai 110 fois, `wait_unit_engaged_with_target` vrai 51 fois — les trois grandeurs sont vivantes, pas seulement testées. |
| `handle_wait` — pourquoi `skip` vaut 0 sur un run réel | Constat instruit le 2026-08-05, **pas** un défaut : la requalification `wait`→`skip` est `engagée ET sans arme [CLOSE-QUARTERS] ET pas M/V`. Sur `step.log`, 54 unités engagées entrent dans la branche et **0** en ressort en `skip` | Arithmétiquement, les 54 avaient donc toutes une arme close-quarters ou la nature M/V — 10.06 les autorisait à tirer, leur WAIT est un vrai choix. Cohérent avec les rosters : presque toutes les unités portent un pistolet (il a fallu chercher un `Terminator` pour en trouver une sans, cf. le verrou). Sans le volet M/V ajouté le même jour, une partie de ces 54 aurait été requalifiée à tort. |
| 🔴 `handle_wait` — le bloc n'était **jamais atteint** | Cause trouvée en instrumentant le correctif ci-dessus : son regex exigeait `Unit 1(5, 48)` (espace après la virgule) alors que les **quatre** producteurs de `w40k_core.py` écrivent `f"{unit_id}({col},{row})"`, sans espace. Aligné sur le `\s*` de la branche SHOOT ; le `else` remonte désormais une `parse_error` au lieu de compter `wait_no_targets` | **MESURÉ sur un `step.log` réel : 15 151 lignes WAIT en phase de tir, 0 matchée.** Toutes tombaient dans le `else` et ressortaient « aucune cible » — verdict rendu sans jamais regarder le plateau, donc `wait_with_targets` valait 0 depuis toujours. Après correctif, même journal : **15 151 lignes mesurées, 251 `wait_with_targets`, 0 erreur de lecture.** Motif « code corrigé mais jamais appelé » (T4) — et la correction d'engagement ci-dessus serait restée morte sans lui. |
