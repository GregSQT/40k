    ...
    def get_squad_action_mask_and_eligible_units(
        ...
        if pending_cr is not None:
            ...
            if alive:
                ...
                return mask, []
            # Escouade entièrement détruite après l'armement du pending (consolidation adverse ou
            # dernière perte fight). Aucune figurine à retirer : on purge la clé et on laisse le
            # masque poursuivre normalement — sans quoi step_with_mask reçoit un masque tout-faux
            # + eligible_units=[] et déclenche advance_phase en silence.
