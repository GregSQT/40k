            ...
            for d in range(budget, 0, -1):
                ...
                for d_col in range(-d, d + 1):
                    ...
                    for d_row in row_values:
                        ...
                        if (nc, nr) in occupied_after:
                            continue
                        # L'anneau parcouru est CARRE en coordonnees offset ; la distance de
                        # charge, elle, est hexagonale (11.04 MAXIMUM DISTANCE = le jet). Sans
                        # ce controle, un pas diagonal sortait du budget.
                        if calculate_hex_distance(orig_col, orig_row, nc, nr) > budget:
                            continue
                        if not _hex_legal_for_charge(nc, nr, game_state, squad_id, m, target_squad_ids):
                            continue
