...
def _charge_reverse_goal_bfs_for_eligibility(
    ...
    *,
    ...
    for _, _gz_enemy in indexed_enemy_engagement:
        ...
        _gz_fp = entry_footprint(_gz_enemy)
        _gz_fp_r = max(
            hex_distance(_gz_ec, _gz_er, int(_fc), int(_fr)) for _fc, _fr in _gz_fp
        )
...
def get_eligible_units(game_state: Dict[str, Any]) -> List[str]:
    ...
    for unit_id, cache_entry in units_cache.items():
        ...
        # au début de cette phase (`clear_ingress_move_lock`). Règle côté CHARGEUR, pas un filtre
        # d'énumération d'ennemis (ceux-là sont dans `enemy_entries_on_battlefield`).
...
def _compute_plan_context(
    ...
    for eid, entry in enemy_entries_on_battlefield(units_cache, player):
    ...
    if can_classify:
        ...
        if nontarget_entries:
            ...
            for ne in nontarget_entries:
                ntgt_union |= set(entry_footprint(ne))
    ...
    # Sans `game_state` : MEME resolution que la primitive `entries_in_engagement_zone`, qui n'a
    # pas d'etat sous la main. Deux chemins de lecture pour un seul seuil, c'est deux verdicts
    # possibles sur la meme paire — le defaut deja paye sur `metric` et sur le gate vertical.
    _vz = get_engagement_zone_vertical()
    if can_classify:
        ...
        def _eng(enemy_entry, e_shape, e_multi, mask, radius, cand_fp, cand_is_multi, vert_reachable):
            ...
            if radius > 1 and synth_shape == "round" and e_shape == "round" and not cand_is_multi and not e_multi:
                # `memoise=False` : `synth_base` N'EST PAS invariant — la boucle de classement
                # ci-dessous le réécrit à chaque cellule candidate (col/row/empreinte/cartes
                # par-figurine). C'est donc une clé neuve par (cellule × ennemi), du même flot que
                # les autres boucles de candidats. Ce site avait été classé « invariant » à tort
...
def _charge_pool_clip_under_floor(
    ...
    from engine.terrain_utils import floor_hexes_at_level, floor_polys_at_level
    from engine.hex_utils import (
    ...
    # Accesseur mémoïsé (`_FloorIndex`) et non reconstruction locale : le seul appelant garantit
    # ``view_level >= 1``, et les `_hex_center` des sommets sont alors calculés une fois par terrain.
    polys = floor_polys_at_level(terrain_areas, view_level)
    if not polys:
        return pool
    shape = require_key(model, "BASE_SHAPE")
    base = require_key(model, "BASE_SIZE")
...
def charge_build_valid_targets(game_state: Dict[str, Any], unit_id: str, max_distance: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    # Hors table (réserves 20.01) écarté par l'énumérateur : jamais chargeable.
    enemies = [
        enemy_id for enemy_id, _cache_entry in enemy_entries_on_battlefield(units_cache, unit_player)
    ]
...
def _has_valid_charge_target(game_state: Dict[str, Any], unit: Dict[str, Any],
    ...
    unit_player = int(unit["player"]) if unit["player"] is not None else None
    enemy_n = sum(
        1 for _eid, _enemy_entry in enemy_entries_on_battlefield(units_cache, unit_player)
    )
...
def charge_build_valid_destinations_pool(game_state: Dict[str, Any], unit_id: str, charge_roll: int,
    ...
    # mouvement : `took_to_the_skies` lit cette déclaration et rien d'autre, pour tous les sièges
    # (V11 §0.48 `L6` — le point de choix `fly_declaration` est posé avant ce pool, il n'y a plus
    # de politique moteur qui tranche à la place du joueur piloté par le modèle).
    ...
    if _charge_distance_metric(game_state) == "euclidean":
        ...
        for (_ene_id, _ce_near), (_pec, _per, _peth) in zip(indexed_enemy_engagement, _charge_enemy_prox):
            _ce_occ = entry_footprint(_ce_near)
...
def charge_click_handler(game_state: Dict[str, Any], unit_id: str, action: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    ...
    if click_target == "destination_hex":
        ...
        game_state["_charge_initial_rolls"].pop(unit_id, None)
        return True, {
            "action": "postpone",
            "unitId": unit_id,
...
def _charge_model_pos_is_closer(
    ...
    for eid, entry in enemy_entries_on_battlefield(units_cache, player):
        if str(eid) in declared:
            target_fps.append(set(entry_footprint(entry)))
        else:
            nontarget_entries.append(entry)
    if not target_fps:
        return False
    ...
    *,
    ...
    for eid, entry in enemy_entries_on_battlefield(units_cache, player):
        if str(eid) in declared:
            ...
            target_fp_by_id[str(eid)] = set(entry_footprint(entry))
        else:
            nontarget_entries.append(entry)
    # Blocage de traversée SOL par-figurine niveau 0 (ennemis) : une fig ennemie à l'étage ne bloque
    # pas le pas d'un chargeur au sol (03.04) — miroir move/fight (build_enemy_occupied_positions_set).
    ...
    nontarget_entries = [
        ...
        if set(entry_footprint(ne)) & _zone
    ]
