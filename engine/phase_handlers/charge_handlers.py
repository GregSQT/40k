...
def charge_autoplace_plan(
    ...
    *,
    ...
    # Le niveau annoncé est celui auquel l'ILP a CALCULÉ : cet auto-placement est intégralement
    # au sol (« 3a », tous ses synths portent `level=0`, ses obstacles sont ceux du niveau 0).
    # Y coller le niveau courant d'un chargeur parti d'un étage rendrait un plan dont le niveau
    # contredit ses propres positions — refusé par le preview, ou committé sur une case sans
    # plancher. Aucun plan muet ne sort du moteur pour autant : l'étage est explicite.
    plan = [
        [mid, int(provisional[mid][0]), int(provisional[mid][1]), SQUAD_RIGID_MOVE_DESTINATION_LEVEL]
        for mid in alive
    ]
    return {"plan": plan}
