...
def parse_log(filepath):
    ...
    current_episode = []
    current_episode_num = 1  # Start at 1 (first episode is #1, not #0)
    episode_turn = 0
    episode_actions = 0
            ...
            if match:
                ...
                if step_inc:
                    ...
                    if 'shoots' in action_desc.lower() or 'shot' in action_desc.lower():
                        ...
                        advance_match = re.search(r'Unit (\d+)\((\d+), (\d+)\) ADVANCED from \((\d+), (\d+)\) to \((\d+), (\d+)\)', action_desc)
                        if advance_match:
                            ...
                            if (start_col, start_row) != (dest_col, dest_row):
                                ...
                                if (dest_col, dest_row) in unit_positions.values():
                                    colliding_units = [uid for uid, pos in unit_positions.items() 
                                                      if pos == (dest_col, dest_row)
                            ...
                            # Fallback: parse without "from/to" format
                            advance_match = re.search(r'Unit (\d+)\((\d+), (\d+)\)', action_desc)
                            if advance_match:
                                advance_unit_id = advance_match.group(1)
                                dest_col = int(advance_match.group(2))
                                dest_row = int(advance_match.group(3))
                                
                                current_tracked_pos = unit_positions.get(advance_unit_id, None)
                                
                                if current_tracked_pos and current_tracked_pos != (dest_col, dest_row):
                                    if (dest_col, dest_row) in unit_positions.values():
                                        colliding_units = [uid for uid, pos in unit_positions.items() 
                                                          if pos == (dest_col, dest_row)
                                                          and uid != advance_unit_id
                        ...
                        charge_match = re.search(r'Unit (\d+)\((\d+), (\d+)\) CHARGED unit \d+ from \((\d+), (\d+)\)', action_desc)
                        if charge_match:
                            ...
                            if (start_col, start_row) != (dest_col, dest_row):
                                ...
                                # Use current tracking table (before update) to find collisions
                                if (dest_col, dest_row) in unit_positions.values():
                                    # Find all OTHER units at destination (alive only)
                                    colliding_units = [uid for uid, pos in unit_positions.items() 
                                                      if pos == (dest_col, dest_row)
                        ...
                        move_match = re.search(r'Unit (\d+)\((\d+), (\d+)\) MOVED from \((\d+), (\d+)\) to \((\d+), (\d+)\)', action_desc)
                        if move_match:
                            ...
                            if move_match:
                                ...
                                # Only process if unit is actually moving
                                if current_tracked_pos and current_tracked_pos != (dest_col, dest_row):
                                    # RULE CHECK 1: Position collision
                                    if (dest_col, dest_row) in unit_positions.values():
                                        colliding_units = [uid for uid, pos in unit_positions.items() 
