...
export class AIService {
  ...
  private async executeAiRequest(
    ...
    for (let attempt = 1; attempt <= this.config.retries; attempt++) {
      try {
        console.log(
          `[AI] Attempt ${attempt}/${this.config.retries} - Sending gameState:`,
          gameState
        );
        ...
        const timeoutId = setTimeout(() => {
          console.warn(
            `[AI] Request timeout after ${this.config.timeout}ms - backend likely unavailable`
          );
          controller.abort();
        }, this.config.timeout);
  ...
  private isValidAIAction(obj: unknown): obj is AIAction {
    ...
    return (
      ...
      ["move", "moveAwayToRngRng", "shoot", "charge", "attack", "skip"].includes(
        candidate.action
      ) &&
