...
export function isPositionOccupied(units: Unit[], position: Position, excludeId?: UnitId): boolean {
  return units.some(
    (unit) => unit.id !== excludeId && unit.col === position.col && unit.row === position.row
  );
}
...
export function getAdjacentPositions(
  ...
  const directions = [
    ...
    [-1, 0], // East, West
    [0, -1],
    [0, 1], // Northeast, Southeast (for even cols)
    [-1, -1],
    [1, -1], // Northwest, Southwest (for even cols)
  ];
...
export function areUnitsAdjacent(unit1: Unit, unit2: Unit): boolean {
  return (
    getHexDistance({ col: unit1.col, row: unit1.row }, { col: unit2.col, row: unit2.row }) === 1
  );
}
...
export function getUnitsInRange(units: Unit[], center: Unit, range: number): Unit[] {
  return units.filter((unit) => unit.id !== center.id && isUnitInRange(center, unit, range));
}

export function getEnemiesInRange(units: Unit[], attacker: Unit, range: number): Unit[] {
  return getUnitsInRange(units, attacker, range).filter((unit) => unit.player !== attacker.player);
}
...
export function isValidPosition(position: Position, boardCols: number, boardRows: number): boolean {
  return (
    position.col >= 0 && position.col < boardCols && position.row >= 0 && position.row < boardRows
  );
}
...
export function isValidMove(
  ...
  const distance = getHexDistance({ col: unit.col, row: unit.row }, targetPosition);
