...
interface WeaponDropdownProps {
  ...
  targetData?: Record<
    string,
    Array<{ code: string; weapon: WeaponOption["weapon"]; m: number; x: number }>
  >;
      ...
      {openTargets && targetData ? (
        (() => {
          ...
          const siblingBlocked = (code: string, combi?: string): boolean => {
            ...
            return weapons.some(
              (wo) =>
                wo.weapon.COMBI_WEAPON === combi &&
                (wo.weapon.code ?? "") !== code &&
                assignedCodes.has(wo.weapon.code ?? "")
            );
          };
          return (
            <table className="weapon-table">
              ...
              <tbody>
                ...
                {weapons
                  .filter((wo) => wo.canUse)
                  .filter(
                    (wo) => !restrictToCodes || restrictToCodes.includes(wo.weapon.code ?? "")
                  )
                  .map((wo) => {
                    const w = wo.weapon;
                    const code = w.code ?? "";
                    ...
                    const nAssigned = openTargets.reduce(
                      (acc, t) =>
                        acc + ((targetData[t] ?? []).find((e) => e.code === code)?.x ?? 0),
                      0
                    );
                        ...
                        <tr
                          ...
                          style={{
                            ...
                            ...(highlighted
                              ? {
                                  boxShadow:
                                    "0 0 0 2px #86efac, 0 0 10px 3px rgba(134,239,172,0.85)",
                                }
                              : {}),
                            ...
                            {w.WEAPON_RULES?.map((rule) => (
                              <span key={rule} className="rule-badge-wrapper">
                                ...
                                <span className="rule-tooltip">
                                  {weaponRules[rule as keyof typeof weaponRules]?.description ||
                                    rule}
                                </span>
                              </span>
                            ))}
