...
import type { GameMode, Unit } from "../types/game";
...
export const GameController: React.FC<GameControllerProps> = ({ initialUnits, className }) => {
    ...
    if (!initialUnits || initialUnits.length === 0) {
      ...
      const loadUnits = async () => {
        try {
          ...
          throw new Error(
            `Failed to load units from scenario.json: ${error instanceof Error ? error.message : String(error)}`
          );
        }
      };
  ...
  const { gameState, movePreview, attackPreview, shootingPhaseState, chargeRollPopup, actions } =
    useGameState(gameUnits);
  ...
  React.useEffect(() => {
    ...
    deadUnits.forEach((unit) => {
      ...
      const alreadyLogged = gameLog.events.some(
        (event) => event.type === "death" && event.unitId === unit.id
      );
        ...
        <div className="turn-phase-tracker-right">
          ...
          <div className="hex-toggle-container">
            ...
            <div className="hex-toggle-switch">
              ...
              />
              <div
                className={`hex-toggle-track ${debugMode ? "hex-toggle-track--on" : "hex-toggle-track--off"}`}
              >
                <div
                  className={`hex-toggle-thumb ${debugMode ? "hex-toggle-thumb--on" : "hex-toggle-thumb--off"}`}
                />
              </div>
            </div>
          </div>
  ...
  return (
    <SharedLayout className={className} rightColumnContent={rightColumnContent}>
      <GameBoard
        units={gameState.units}
        selectedUnitId={gameState.selectedUnitId ?? null}
