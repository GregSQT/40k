...
def build_probe_context(agent: str, stage_name: str, training_config_name: str, resolution: int,
    ...
    Aucune garde sur `advantage_source` ici : chaque sonde pose la sienne sur `ctx["model"]`
    (`refuse_q_head_model` pour celle-ci, `require_q_head_model` pour
    scripts/q_head_structure_probe.py) avant toute collecte.
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    refuse_q_head_model(ctx["model"])  # avant toute collecte : la sonde ne mesure que l'acteur GAE
    ctx["workdir"] = workdir
