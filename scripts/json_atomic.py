...
Deux contrôles à l'ouverture, parce qu'aucun ne suffit seul :
  - `path` ne doit pas être un DOSSIER — sinon seul le `os.replace` FINAL échouerait, tout à la
    fin, quand le travail est déjà fait et perdu ;
...
def _draft(path: StrPath) -> Iterator[TextIO]:
    ...
    try:
        ...
        # et surtout `path` n'a jamais été ouvert, donc il n'a pas bougé.
        handle.close()
