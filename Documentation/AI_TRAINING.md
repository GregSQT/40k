...
**Toute la ligne de durées est en secondes par épisode produit depuis le 2026-08-01**, le diviseur `n_envs` annoncé une fois en tête : `[s/ep (48 env): cur 0.300, moy 0.211, min/max: 0.240/0.338]`. `moy` vaut `(elapsed − temps d'éval bot) / episode_count` ([training_callbacks.py:506](../ai/training_callbacks.py#L506)) ; `cur`, `min` et `max` sont les durées par slot divisées par `n_envs`. Une seule unité par ligne : c'est le mélange des deux échelles qui avait fait conclure à un ralentissement le 2026-08-01. Quatre pièges évités délibérément, chacun verrouillé par un test de [tests/unit/ai/test_progress_wall_per_episode.py](../tests/unit/ai/test_progress_wall_per_episode.py) — (1) `moy` n'est PAS `moyenne par slot / n_envs`, division exacte seulement une fois que **chaque** slot a fini un épisode et qui vaut `n_envs/k` fois trop peu tant que seuls `k` slots sont arrivés ; (2) son dénominateur est le compteur **local au chunk**, pas celui de l'affichage, qui cumule les chunks précédents en rotation de scénarios alors que le chronomètre repart à chaque chunk ; (3) le temps d'évaluation bot est retranché des quatre valeurs, sinon le slot qui enjambe une éval de 120 s la porte dans sa propre durée (mesuré : `max` à 121,80 s contre 1,20 s de durée moteur réelle) et un run à `bot_eval_freq` différent n'est plus comparable ; (4) le démarrage du process (imports torch, fork des workers, chargement du modèle) n'est couvert par aucune des quatre : le chronomètre part du début de `learn()`.
