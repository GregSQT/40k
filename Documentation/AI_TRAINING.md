...
- **Environnement propre exigé** : `subprocess` hérite du shell, et 11 variables `W40K_*` (vérifications, instrumentation, traces) ralentissent chaque run **sans laisser de trace dans la sortie** — `W40K_MASK_VERIFY` a été mesurée à ×4,7 au niveau 1 et ×17 au niveau 2. Les bancs refusent de démarrer tant qu'une seule est armée ; `W40K_X=0` / `off` est accepté. Réflexe avant toute campagne : `env | grep W40K`.
