...
**Pourquoi pas le `cur` / `min/max` de la barre de progression** : ce sont les durées wall-clock d'un épisode **sur un slot d'env**, attente de synchronisation des autres envs comprise ([training_callbacks.py:415](../ai/training_callbacks.py#L415)). C'est une *latence*, pas un débit, et elle croît mécaniquement avec `n_envs` : mesuré 2,24 s/ep à `n_envs=6` contre 2,61 à `n_envs=8`, alors que le run à 8 était le plus rapide (37 s contre 42 s pour 100 épisodes). Elle a en outre un CV=36 % sur x10_debug → ≥50 épisodes pour ±10 % IC95 %. Pour comparer deux valeurs de `n_envs`, utiliser `scripts/ab_bench_nenvs.py` (débit wall, entrelacé) ; pour en classer plusieurs, `scripts/ab_sweep_nenvs.py` (tours à ordre inversé, échéance horaire).

**La colonne `mur`, ajoutée le 2026-08-01, se compare elle entre `n_envs`** : c'est `elapsed / episode_count` ([training_callbacks.py:515](../ai/training_callbacks.py#L515)), soit les secondes de wall-clock par épisode réellement produit. Deux pièges que ce calcul évite délibérément, chacun verrouillé par un test de [tests/unit/ai/test_progress_wall_per_episode.py](../tests/unit/ai/test_progress_wall_per_episode.py) — (1) `moyenne par slot / n_envs` n'est exacte qu'une fois que **chaque** slot a fini un épisode, et vaut `n_envs/k` fois trop peu tant que seuls `k` slots sont arrivés ; (2) le dénominateur est le compteur **local au chunk**, pas celui de l'affichage, qui cumule les chunks précédents en rotation de scénarios alors que le chronomètre, lui, repart à chaque chunk. Le temps d'évaluation bot est retranché des **quatre** valeurs affichées — `mur`, `cur`, `min` et `max` — sans quoi le slot qui enjambe une éval de 120 s la porte dans sa propre durée (mesuré : `max` à 121,80 s contre 1,20 s de durée moteur réelle), et un run à `bot_eval_freq` différent n'est plus comparable. `mur` ne couvre en revanche pas le démarrage du process (imports torch, fork des workers, chargement du modèle) : il part du début de `learn()`.
