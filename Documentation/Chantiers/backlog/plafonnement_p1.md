...
- `/code-review` (04:18) : 2 findings, tous deux mesurés. (1) HIGH — `adv_heads` enregistré sur
  ...
  NU (`def load_ai_model_for_pve` de `engine/pve_controller.py`, `def _eval_worker_load_model_if_needed` et `def _worker_checkpoint_opponent` de `ai/bot_evaluation.py`, snapshots du pool,
...
relancée à 11:35 sous la règle « plateau » ci-dessous** ; log archivé
`logs/training_x1_07-p01-cycle2_avorte_0918.log`, artefacts (3 checkpoints, `_pre_resume_095748`)
dans la corbeille.

**PLATEAU, INSTANTANÉS À SEUILS, RUNS DE CHECK — LIVRÉ (2026-09-17, 11:34, merge `f5ebdf678`,
...
|---|---|---|---|---|
| A1 | S25 tourne sous la table **B6** (marge de VP), pas sous la récompense de référence de P1 | l'ancienne récompense d'objectif a été supprimée du code le 09-14 (0 hit `objective_reward_factor` hors doc) ; le run de référence n'est plus rejouable sur main ; `value_warmup_updates 20` reste actif (zip P0 sans marqueur → régime joué) | CONFIRMÉ (`def _calculate_vp_margin_reward` de `engine/reward_calculator.py`, `def train_with_scenario_rotation` et `def arm_value_warmup` de `ai/train.py`) | S25 = meilleur cas **sous B6** ; la bissection contre la config P1 se fera sous B6 (le « run marge » P1 devient le bras de comparaison) |
| A2 | cellules « P0 échantillonné vs P0 argmax » et « P0 argmax vs P0 argmax » de S27 | le script impose `deterministic=True` (`def evaluate_against_checkpoints` de `ai/bot_evaluation.py`) ; la première se lit sur `03_selfplay/P0` de S25 **pendant le warmup** (politique figée au bit près, ~500–1 440 épisodes d'étape) ; la seconde devait venir de la parité d'ouverture | CONTESTÉ sur la seconde : **un exploiteur n'a pas de sonde de parité** (`def _run_main` de `ai/train.py` monte `ExploiterProbeCallback` seul et réserve `PoolEarlyStoppingCallback` aux non-exploiteurs) — donc aucun garde-fou d'appariement zip/pkl sur S25 | P0 vs P0 mesuré par le script sur les deux sièges forcés AVANT S25, lu comme verrou de parité [0,40, 0,60] |
| A3 | lecture de S25 : sondes exploiteur 100 parties / 2 000 épisodes (pas 10 000 comme écrit en §9.5), confirmation 500 à ≥ 0,95, pas de bloc early_stop, budget en épisodes d'étape | | CONFIRMÉ (`ExploiterProbeCallback._on_step` et `_EvalPoolOwnerMixin._stage_episode` de `ai/training_callbacks.py`, `def stage_origin` de `ai/curriculum.py`) ; **les sondes exploiteur ne sont PAS publiées dans TensorBoard**, seulement dans le log (`🔬 Sonde exploiteur`) et `curriculum.log` à la clôture | verdict sur la moyenne glissante de 3 sondes calculée depuis le log (300 parties, ±2,9 pts) + `03_selfplay/P0`, `win_rate_overall`, `seat_aware/*` (100 % des épisodes contre P0) |
| A4 | lr 0,0005, un seul bras | bras 0,001 seulement si S25 ≥ 0,68 | CONFIRMÉ (`def _apply_curriculum_model_params` de `ai/train.py`, `PatchedMaskablePPO.train` de `ai/patched_ppo.py`, rampe non montée sur scalaire dans `def setup_callbacks`) | |
| A5 | `opponent.deterministic` global atteint l'exploiteur | | CONFIRMÉ (`def _stage_opponent_mix` de `ai/train.py` → `_SELF_PLAY_ARGS` et `def make_training_env` de `ai/training_utils.py` → `BotControlledEnv.__init__` et `BotControlledEnv._get_self_play_opponent_action` de `ai/env_wrappers.py`) | |
| R1 | **bug attrapé** : `seat_matrix_probe.py` ne posait pas `W40K_BOARD_PATH` → plateau x5 en silence (grille à taille fixe, aucun refus) | | signalé par la contre-relecture (clé `board` de `config/config.json`, `def iter_window_hexes` de `engine/spatial_grid.py`) | corrigé avant toute mesure : `board_path_for_agent`, test ; la sonde de fumée à 8 parties faite avant ce correctif est **jetée** |
| R2 | le canonique de fin de S25 (`model_ArmageddonAgent_x1_expl.zip`) sera le meilleur instantané **holdout bots** (`save_best_robust`), pas les poids finaux de l'exploiteur ; `_close_exploiter_stage` ne promeut aucun `_E0.zip` | | signalé (`def final_save_publishes_live_weights` de `ai/train.py`) | toute mesure a posteriori sur l'exploiteur vise le dernier `ppo_checkpoint` (rotation 3) ; noté, pas modifié |
| R3 | S27 : seuls les profils forcés mesurent un siège, ils n'existent que dans l'agent expl | | signalé (`def evaluate_against_checkpoints` de `ai/bot_evaluation.py`) | toutes les cellules S27 passent `--agent ArmageddonAgent_x1_expl` (tables identiques), y compris P1 `robust_0.9078` |
...
tirée par tâche, correctif du 2026-09-19 soir) : une première série, dont la graine de siège était
commune à toutes les tâches, donnait 0,590 / 0,457 / 0,522 / 0,406 — mêmes conclusions, mais sa
ligne de contrôle ne contrôlait rien, le bot du siège agent n'y changeant jamais de côté à index
d'épisode donné.
