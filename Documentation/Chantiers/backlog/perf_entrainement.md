...
### Phase 3 — Architecture de la collecte — ✅ Option A actée (2026-08-26) · ✅ Implémentée (2026-08-28) · ✅ Fix obs aliasing validé (2026-08-30)
...
**Cibles** : ~~`geodesic_move_reach` (21 %)~~ **→ 8,0 % depuis la réécriture du 2026-09-08** (voir §6) + `erode_move_pool_by_squad_block` (18 %) + `hex_line_iter` dans `_los_line_segment_clear` (20 %). Un noyau C/Cython réduirait ces postes d'un facteur ×5–10. Gain estimé sur fps : **+25–35 %** (confiance : **moyenne** — le gain sur le poste est déterministe, la fraction du wall restante l'est moins).
...
- Goulot BFS natif (chantier lourd EN PAUSE, pré-existant) :
  `Documentation/Chantiers/backlog/perf_noyau_natif_et_gzip.md` §2 — les étapes 1.1-1.2 ci-dessus
...
|---|---|---|---|
| 2026-09-09 | **Zones d'engagement publiées en `frozenset`** — le fingerprint de `_move_spatial_cache` re-hachait le CONTENU des deux `enemy_adjacent_hexes_player_*` à chaque lecture. Sonde directe sur `bench_env_step.py`, 40 échantillons répartis sur 80 steps, i9-13900H WSL2, machine au repos | coût par appel du fingerprint · ventilation · appels et écritures par step | **AVANT : 184,2 µs/appel** — zones d'engagement 162,4 µs (88 %, dont 25,5 µs de `sorted(game_state.items())` sur 167 clés et ~137 µs de `hash(frozenset(...))` sur 7 942 hexes cumulés), positions des 60 figurines 13,4 µs (7,3 %), `battle_shocked` 0,4 µs. **APRÈS : 26,7 µs/appel, −85 %.** Cadence mesurée par instrumentation de pile : **15,5 appels et 0,96 écriture de ces clés par step** (77 écritures / 80 steps — 74 par `build_enemy_adjacent_hexes`, 2 + 1 par les deux propagations ; les deux chemins réactifs : 0 appel, d'où le test dédié). Soit ~2,4 ms/step retirés du calcul — **le wall du bench ne résout PAS ce gain** : 3 répétitions de 300 steps donnent 27,3–39,0 s avant et 26,6–37,4 s après, distributions superposées, la variance machine (±30 %) écrase l'effet ; ne pas en tirer de pourcentage de wall. Le hash d'un `frozenset` est mis en cache par CPython sur l'objet (0,03 µs contre 26 µs de reconstruction, mesuré sur 3 000 éléments) et reste dérivé du CONTENU : ce n'est pas le compteur de version interdit par §0.18, et c'est plus sûr que la relecture, un ensemble immuable ne pouvant pas changer de contenu sans devenir un autre objet. |
| 2026-09-08 | **`geodesic_move_reach` réécrit sur `hex_index_table`** (index entiers + expansion par couches) — `bench_env_step.py` 80 steps, temps cumulé dans la fonction rapporté au wall du MÊME run, **une variante par processus, 3 répétitions**, i9-13900H WSL2 16 cœurs, load ~1,5 | temps cumulé dans la fonction · part du wall · médiane ms/step | **AVANT : 2 393 / 2 425 / 2 382 ms · 20,4 / 19,9 / 20,4 % du wall**<br>**APRÈS : 809 / 816 / 806 ms · 8,0 / 8,0 / 8,0 % du wall** — **−66 % sur le poste**, wall du bench −14 % (11 702 → 10 060 ms au minimum des 3). 601 appels dans les deux cas (aucune mémoïsation ajoutée), champ moyen 5 105 cellules, plateau 220×300. Décomposition micro-banc sur 120 appels capturés : expansion par couches seule **−8,5 %**, table de voisins bornés seule **−39,6 %**, les deux **−43,0 %**, **index entiers −66 %** — c'est le hachage de PAIRES, pas le filtre de bornes, qui dominait. Coût de la table : 68 ms et 12,8 Mo par processus, une fois par plateau — construite hors du cache unitaire `_HEX_NEIGHBORS_CACHE`, qu'un balayage complet du plateau aurait gonflé de 66 000 entrées (35 Mo) pour des cases jamais interrogées une à une. Alternative écartée par mesure : BFS par masques de bits (entiers géants) — 2,13 ms/appel pour les seules opérations bitwise obligatoires, plus 3,78 ms d'extraction du champ, soit pire que l'implémentation d'origine (le plateau fait 66 000 cases pour un champ de 5 105). |
...
| 2026-08-28 | **Phase 3 — bench mono-env post-livraison**, `bench_env_step.py --steps 600 --seed 42`, machine au repos, 3 répétitions | wall · médiane · P99 | **70,5 / 63,6 / 73,4 s · 32,5 / 29,1 / 33,5 ms · 1300 / 991 / 1367 ms** — médiane ~32 ms, dans la plage Phase 2 (29-34 ms). Aucune régression côté env. Le gain Phase 3 (lockstep éliminé) n'est PAS visible ici (bench mono-env sans SubprocVecEnv) — mesurer `time/fps` sur un vrai run PPO (x1_debug, ≥10 updates, TensorBoard) et comparer à 226-233 fps (baseline Phase 2). |
| 2026-08-28 | **Phase 3 — collecte distribuée livrée** : `ai/vec_normalize_frozen.py` (snapshot/restore VecNorm), `ai/maskable_subproc_vec_env.py` (COLLECT_TRAJECTORY + `_run_worker_trajectory`), `ai/patched_ppo.py` (`_collect_rollouts_distributed` + dispatch). 4 bugs de review corrigés avant commit : double normalisation, raw_global_cont corrompu, ordre bootstrap (après normalisation, sémantique SB3 exacte), num_timesteps step-by-step. Verrou : 34 tests verts (Phase 2+3) + 5 tests harnais parité verts. | — | — |
| 2026-08-28 | **Phase 3 — mesure de clôture `time/fps`**, `x1_debug` (n_envs=24, n_steps=340), machine au repos, 3 répétitions (runs 230623 / 230940 / 231047) | `time/fps` TensorBoard | **574 / 487 / 430 fps — médiane 487 fps** vs baseline Phase 2 : 226-233 fps (médiane 229). **Gain Phase 3 : +113 % (×2,1).** Note : `cur`/`min`/`max` d'`EpisodeTerminationCallback` invalides en mode distribué (callbacks rejoués à vitesse Python post-collecte, pas à vitesse moteur) — seul `moy` reste fiable pour le débit. | — | — |
| 2026-08-27 | **4.2 — Clôture des 4 critères** (commit `ad426b55`) : (1) 4 `_FakeEnv` dans `test_checkpoint_eval_parallel.py` reçoivent `close()` — `env.close()` ajouté dans le `finally` de `_eval_worker_task` au commit `95e5f843` cassait ces tests ; (2) `test_worker_reloads_when_mtime_changes` : `mock_normalizer.assert_called_once()` — le normalizer VecNormalize est rechargé avec le modèle ; (3) `_probe` : `except Exception → self._eval_pool = None` — justifié à l'époque par « `BrokenProcessPool` sur le prochain `submit` », motif faux (une exception de sonde termine le run, il n'y a pas de prochain `submit`) ; **repris le 2026-09-04** en `_shutdown_eval_pool()` — `shutdown(wait=False, cancel_futures=True)` puis lâcher la référence, parce que `MaskablePPO.learn` appelle `on_training_end` hors `finally` et que ce chemin est le seul à fermer le pool. Tests `test_exploiter_probe_closes_its_pool_when_the_evaluation_raises` + jumeau `PoolEarlyStoppingCallback` ; (4) non-régression 4.1 : les 4 tests ep_offset/close verts. **134 tests verts. 4 mutations rouges constatées.** Phase 4 fermée. | `training_callbacks.py`, `test_checkpoint_eval_parallel.py` | — |
| 2026-08-29 | **Phase 3 — retrait du re-scaling des sorties du critique** (`ai/patched_ppo.py`, `ai/vec_normalize_frozen.py`). Introduit le 2026-08-28 (`e03f41e8`) pour faire baisser un `value_loss` jugé gonflé, il multipliait `rollout_buffer.values`, `last_values` et le bootstrap par `sqrt(old_ret_var)/sqrt(new_ret_var)`. Ce facteur suppose V proportionnel à 1/sqrt(ret_var), vrai seulement à convergence : au rollout 1 d'un run `--new`, `old_ret_var` vaut 1.0 parce que `RunningMeanStd` vient d'être initialisé, pas parce que le critique aurait appris à cette échelle. SB3 ne rescale jamais ; le chemin stepwise non plus. | `diag/old_values_mean` ÷ `diag/last_values_cpu_mean` par rollout · `train/value_loss` · `train/explained_variance` | Run `run_20260829-092312` : ratio **0,060** au rollout 1 puis **1,069 / 0,978 / 1,021 / 1,009** — le facteur ne mordait qu'au premier rollout, en y écrasant les prédictions du critique de **17×**. Conséquence sur les rollouts suivants : `value_loss` **1,06 → 15,1 → 1,26** (P1 stepwise : 5,55 → 3,92 → 1,96, décroissance monotone) et `explained_variance` **0,003 → 0,16 → −0,37** (P1 : −0,03 → 0,32 → 0,57). À budget de steps égal le run distribué restait 4-5× derrière : `ep_rew_mean` **+9,6 à 750 k** contre **+126 à 726 k** pour P1, `explained_variance` **0,69** contre **0,88** à ~1,2 M. Verrou : 28 tests verts, dont `TestCriticOutputsNotRescaled` qui exerce `_collect_rollouts_distributed` de bout en bout au cold-start (mutation constatée rouge : values 5,0 → 0,187). **Gain sur la vitesse d'apprentissage non mesuré — à relever au prochain run `x1_long --new`** en comparant `explained_variance` et `ep_rew_mean` à ~750 k steps aux chiffres ci-dessus. |
| 2026-08-30 | **Phase 3 — validation fix obs aliasing**, run `x1_long --etape P2` (`run_20260830-105040`) | `diag/ratio_mb0_mean` · `explained_variance` | `ratio_mb0_mean` = **1,000** (verrouillé dès les premiers rollouts) · `explained_variance` monte 0→**0,85** en ~6–7k épisodes et se stabilise. Critère d'acceptation du 2026-08-29 rempli. Fix Phase 3 validé — run P2 peut continuer jusqu'au bout. |
| 2026-08-30 | **Coût x5 — NON mesuré** : décision de bascule lignée production en x5 après E1 actée (option C) — agents nommés `ArmageddonAgent_x1` / `ArmageddonAgent_x5`, plus de risque d'écrasement de lignée. `x5_long` existe déjà (mêmes hyperparams, budgets 200k) ; seul `exploiter_config.training_config_required` reste à changer x1_long→x5_long. Forward/backward GPU inchangé (obs 16 735 + grille 32×32×9 identiques) ; BFS moteur ~25× cases → facteur réel inconnu. **Mesurer AVANT d'engager le budget x5** : `bench_env_step.py --steps 600 --resolution 5`, machine au repos, ≥ 3 répétitions, lecture médiane + P99. Variante ouverte : warm-start du seul P0-x5 depuis le champion x1 du moment de la bascule, avec resets de normalisation + A/B mesuré contre `--new`. | ms/step x5 | **NON MESURÉ** |
| 2026-08-29 | **Phase 3 — ROOT CAUSE du non-apprentissage : aliasing des buffers scratch d'observation** (`ai/maskable_subproc_vec_env.py` + `ai/vec_normalize_frozen.py`). Le moteur sert ses obs dans des buffers RÉUTILISÉS entre steps (`engine/observation_builder._empty_squad_observation`, contrat « ne jamais stocker le dict retourné au-delà du step courant ») ; `normalize_obs_with_snapshot` ne recréait que `global_cont` (shallow copy) et `_run_worker_trajectory` stockait les autres clés PAR RÉFÉRENCE pendant 340 steps → au `np.stack` final, toutes les obs de la trajectoire = l'état FINAL répliqué (grid, self_models, allies, enemies… — tout sauf `global_cont`). Le learner s'entraînait sur des obs découplées des actions/values/rewards. Le chemin stepwise était protégé par accident : chaque obs traversait le pipe subprocess (pickle = copie). Présent depuis `4b2fe20d` (commit initial Phase 3) — les 31 tests ne le voyaient pas car leurs StubEnv retournent des tableaux neufs à chaque `_obs()`. Fix : copie profonde dans `normalize_obs_with_snapshot` (AVANT `env.step()` — copier au site de stockage capture l'état post-step, prouvé rouge) + copie de `terminal_observation` (même motif). | Preuves lues sur `run_20260829-132022` (post-fixes TEMPS 1-4, 63 rollouts) : `diag/logprob_drift_mean_abs_mb0` · `diag/ratio_mb0_mean` · `diag/value_drift_mean_abs_mb0` · `diag/last_values_gpu/cpu_mean` · `train/explained_variance` | drift log_prob **0,15-0,21** (attendu 0) et ratio mb0 **0,92-0,95** (attendu 1) sur TOUS les rollouts · value_drift mb0 **0,35** alors que `last_values` GPU/CPU concordent à 1e-4 (mêmes poids, même forward → c'est le CONTENU du buffer qui diffère des obs vues par le worker) · critique effondré sur une constante (`old_values_mean` figé à 5,55, `explained_variance` épinglée à ~0,00 sur 47 updates) · verrou : `test_obs_stored_are_copies_not_scratch_refs` (rouge constaté : obs step 0 = état final 5,0, puis rouge off-by-one 1,0 avec copie au stockage, vert avec copie à la création) · **critère d'acceptation au prochain run `--new` : `ratio_mb0_mean` = 1,0 exactement, `logprob_drift` ≈ 0, `explained_variance` croissante dès les premiers rollouts** |
| 2026-09-04 | **4.2 — la persistance du pool des sondes n'existait pas** : `ExploiterProbeCallback` et `PoolEarlyStoppingCallback` créaient leur pool dans `_on_training_start` et le fermaient dans `_on_training_end`, que SB3 appaire autour de CHAQUE `learn()` (sb3-contrib lignes 448 et 467). La boucle budgétée en épisodes de `train_with_scenario_rotation` en enchaînant un par tranche de quatre updates, et une sonde tombant dans une tranche différente de la précédente, le pool était recréé à chaque sonde — la ligne du 2026-08-27 ci-dessus (« ouvre un pool en `_on_training_start` et le réutilise à chaque `_probe` ») décrivait une réutilisation qui n'avait jamais lieu. Remplacé par `_EvalPoolOwnerMixin` : création paresseuse à la première sonde, fermeture par `shutdown_probe_eval_pools` dans le `finally` de la boucle `learn()` — le seul point couvrant échec et interruption, et qui DEMANDE la libération des 4 workers avant l'évaluation finale et ses 16 (`shutdown(wait=False)` ne l'attend pas — cf. ligne du 2026-09-04 ci-dessous). La fermeture sur l'exception qui remonte de `_probe`, livrée le même jour par `126ecf0c`, est conservée : le nouveau `finally` la couvre aussi, mais plus tard, après les 30 s par VecEnv de `close_all_training_envs`. | durée de sonde, harnais identique, 3 sondes à travers des frontières de `learn()` · workers `spawn_main` après fermeture | **2,46 / 2,04 / 2,03 s avant → 2,57 / 0,02 / 0,00 s après** (spawn + `import sb3_contrib`). En sonde réelle s'y ajoute le chargement de l'archive adverse dans `_worker_ckpt_cache`, **mesuré 9,4 s pour un zip de 45 Mo**, économisé lui aussi ; le rechargement du modèle P1 ne l'est PAS (`mkstemp` neuf par sonde → jeton de version changé de toute façon). **0 orphelin** : retour à la valeur de départ après `shutdown_probe_eval_pools`. 3 mutations rouges, 66 tests verts. |
| 2026-09-04 | **4.2 — correctifs de review du pool des sondes** : (1) la lecture de configuration du pool était partie avec `_on_training_start`, donc repoussée à la PREMIÈRE sonde — un profil auquel il manque `vec_normalize_eval` ne levait plus qu'après `probe_every_episodes` épisodes (2 000 sur le curriculum courant), après des heures de run et une fois le modèle muté. `prepare_probe_eval_pools`, appelée avant la boucle `learn()`, résout et mémorise ces paramètres sans démarrer de worker ; la sonde ne relit plus rien. Le défaut du compte de workers (`4` codé en dur, indépendant du `max_in_flight`) a été corrigé en parallèle par `probe-pool-workers-require-key` le même jour, et cette livraison s'y superpose : la clé est exigée, et le compte RÉSOLU sert au pool comme à `n_workers_override`. Ce qui restait — et que la clé exigée au démarrage ne couvre pas — ce sont `vec_normalize` et `vec_normalize_eval`, toujours lues à la première sonde ; la résolution anticipée les couvre aussi. (3) `shutdown_probe_eval_pools` n'isolait pas ses porteurs : une fermeture qui lève abandonnait les pools suivants et, appelée depuis un `finally`, remplaçait l'exception en cours de propagation. (4) `_eval_pool` passe en attribut de CLASSE du mixin — un futur porteur qui oublierait de l'initialiser faisait lever la fermeture par `AttributeError`, depuis ce même `finally`. (5) Commentaire corrigé : `shutdown(wait=False)` ne garantit PAS l'absence de recouvrement avec l'évaluation finale, il demande la fermeture sans l'attendre ; `wait=True` la garantirait au prix d'un Ctrl-C bloqué jusqu'à la fin de la sonde en vol (`KeyboardInterrupt` échappe à l'`except Exception` de `_probe`), compromis assumé et désormais écrit. | mutations rouges · tests | **4 défauts réintroduits, rouges prouvés un par un (appel `prepare_*` retiré, lecture de config non anticipée, mémoïsation retirée, `try/except` de fermeture retiré), 58 tests verts** (`test_probe_eval_pool_lifetime.py`, `test_checkpoint_eval_parallel.py`, `test_intermediate_eval_workers.py`). |
| 2026-09-16 | **Fuite d'activations de l'échauffement critic (B6)** — `PatchedMaskablePPO.train`, `ai/patched_ppo.py`. Symptôme : run `run_20260916-092441` (`x1_lineage --etape P1`), `train/time_update` par mini-lot 0,20–0,25 s de 09:44 à 11:08 puis 0,9–3,2 s à partir de 11:36 (345 s d'update à 13:51), collecte inchangée à ~32 s par rollout, `moy` 0,55 → 0,87 s/ep. Cause : en échauffement `loss = vf_coef × value_loss`, aucun backward ne traverse la branche politique ; `policy_loss`/`entropy_loss` empilés AVEC leur graphe dans les listes de logging (plus `_terms` du diagnostic mb0) retenaient les activations de CHAQUE mini-lot jusqu'à la fin de `train()`. Réplique de `train()` sur le zip réel (340 × 24, lot 1020, 4 epochs, T = 2) : **update d'échauffement 9,69 Gio alloués / 12,11 réservés, update normale 1,79 / 2,08** — et la réservation reste à 12,11 après l'échauffement, WSL2 ne mettant jamais l'allocateur en OOM (débordement en RAM hôte). Compteurs Windows (`GPU Process Memory`, `powershell.exe`) : la VM WSL tient 7,4 Go dédiés + 5,1 Go partagés = 12,5 Go, VRAM à 83 Mio libres ; la première update du learner refait monter nvidia-smi de 0,98 à 7,9 Go après qu'un processus test a alloué/libéré 10 Gio. Le run de nuit `expl_05` (même échauffement) est resté à 3,2 s/update : le jeu chaud de 2 Gio restait résident faute de perturbation ; le 16, matrice à froid (10:38–11:18), sonde + éval à 10 000 et mes répliques ont remanié la résidence dans les deux sens. Correctif : `log_prob`/`entropy` détachés en échauffement hors mini-lot du diagnostic, listes de logging détachées, `del _terms` après le diagnostic. **Réplique après correctif : échauffement 1,79 / 2,29** (= régime normal). Métriques ajoutées : `train/cuda_peak_allocated_gib` (pic de l'update) et `train/cuda_reserved_gib` (réservation) — la courbe qui manquait. Verrou `tests/unit/ai/test_value_warmup_memory.py` (5 tests, rouge constaté sur le code d'origine : 75 activations par mini-lot survivantes) ; instrument `saved_tensors_hooks` avec vue détachée (rendre le tenseur lui-même crée un cycle nœud→tenseur→nœud et fabrique une fuite de 11 tenseurs par mini-lot, mesuré hors `train()`). Le run en cours garde ses 12 Gio : le fix ne vaut qu'au prochain lancement ; une reprise depuis checkpoint ne rejoue pas l'échauffement (`arm_value_warmup`) et repart à ~2 Gio même sans le fix. | pic alloué / réservé, échauffement · normal | **9,69 / 12,11 → 1,79 / 2,29** · 1,79 / 2,08 |
| 2026-09-16 | **Détachement de tête de l'échauffement retiré** — `ai/patched_ppo.py`. Le correctif du matin cumulait deux mécanismes pour le même invariant : les listes de logging détachées (`pg_losses_t.append(policy_loss.detach())` etc.) + `_terms` consommée, ET un détachement de `log_prob`/`entropy` en tête de boucle hors mini-lot diagnostic. Mesuré sur CPU par mutation en mémoire (modèle jouet du verrou, B = 4, A = 1389, run `x1_lineage` intact) : (1) tête détachée → `if False` : les 3 tests CPU de `test_value_warmup_memory.py` restent VERTS — le site est redondant pour la fuite ; (2) vivantes du mini-lot k−1 à l'entrée de k : 68 tenseurs / 1 789 440 o avec le détachement, 75 / 1 856 176 o sans — les 68 communs sont tenus par `policy.action_dist.distribution` (logits avec `grad_fn`, `distribution = None` après chaque forward → 0 vivante) un mini-lot quoi qu'il arrive ; les 7 libérés par le détachement sont exactement 3 × (B, A) + 4 × (B,), borne ≈ 3·B·A·4 octets ≈ 0,013-0,023 Gio à lot 1020 (extrapolation, pas mesure GPU — run en cours) ; (3) verrou : `pg_losses_t.append(policy_loss)` sans detach → rouge `{2: 72, 3: 0}` avec le détachement de tête (le rouge ne tenait plus qu'au mini-lot diagnostic), `{2: 72, 3: 72}` sans. Bloc 520-538 supprimé (`_diag_minibatch` inclus), historique déplacé au site des listes, docstring du verrou réécrite (celle du matin attribuait le rouge au détachement de tête ; celle de 15:56 affirmait « chaque site seul suffit », faux : (1) seule est rouge via mb0). Chemin prod couvert : `evaluate_actions` (advantage_source gae par défaut) ; `q_head` (S14, réfuté) non mesuré. | tenseurs vivants mb k−1, avec · sans détachement de tête | **68 · 75** (Δ = 3 × (B, A) + 4 × (B,)) |
