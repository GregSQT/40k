...
| 21ter | `alloc_character_over_bodyguard['shooting']` (2026-09-18, chantier « chaîne d'attaque 100 % » ; même compteur par phase : `['move']` §1.1, `['charge']` §1.3, `['fight']` §1.4 — corpus `PROJ.1.x.alloc_character`) | `analyzer_core._judge_character_allocation`, verdict `analyzer_rules.character_allocation_fault` | 05.03 / 06.02 / 24.28 : la figurine allouée (`[ALLOC_MODEL:]`) est un CHARACTER alors qu'un non-CHARACTER de l'unité est encore vivant dans l'état `ai/analyzer_state.py` (`unit_model_hp`) (état d'AVANT la blessure de la ligne ; les lignes d'un lot sont dans l'ordre de résolution, les bodyguards tombés plus tôt dans le lot ont déjà quitté l'état). Légal si la ligne porte `[PRECISION]` (override 24.28 appliqué par le moteur) ET que la blessure est NORMALE ; pour une blessure MORTELLE (`Save [DEVASTATING WOUNDS]`, `SUFFERS`) la clé `alloc.precision_mw_to_character` de l'entête tranche (`False` = cascade 06.02 exigée, `True` = le CHARACTER imposé encaisse, absente = indécidable, non compté). Occasion jugée = ligne allouée sur une unité MIXTE (au moins un CHARACTER et un non-CHARACTER vivants). Non vérifié : priorité « figurine déjà blessée » (PV reconstruits, pas journalisés), ordre des groupes W/Sv/InSv. Test : `tests/unit/ai/test_analyzer_alloc_character.py` |
