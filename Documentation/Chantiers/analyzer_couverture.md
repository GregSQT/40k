...
[hh:mm:ss] Run rules: alloc.precision_mw_to_character=… engagement_zone_subhex=… engagement_zone_vertical_inches=…
...
| `DID NOT ATTACK <cible> with [<arme>] [TARGET DESTROYED] - <N> attack(s) not made` (type `attacks_not_made`, 2026-09-18) | lot jamais joué : sa cible a été détruite par un lot précédent de la même activation (04.03, dés jetés lot par lot) — le moteur ne jette plus ces dés | analyzer : `shoot_cross_weapon_attacks_lost` (métrique, `_ATTACKS_NOT_MADE_RE`) ; la ligne compte comme « même activation » pour le Deadly Demise différé (25 DESTROYED) |
| `[ENGAGED TARGET]` (2026-09-18) | 17.03 : −1 au jet de touche contre une unité MONSTER/VEHICLE ennemie engagée (levé pour [CLOSE-QUARTERS] depuis l'unité engagée avec elle) ; champ log `engagedTargetMalus`, `hit_rule_modifier = "ENGAGED TARGET"` | analyzer : verdict de touche sur le seuil EFFECTIF imprimé (`base+->eff+`, `analyzer_hit`), comme [COVER]/[HEAVY] ; `shoot_at_engaged_enemy` (#16) porte déjà l'exemption 17.03 ; replay : `NON_ABILITY_ROLL_TOKENS` (avec [POINT-BLANK] et [PLUNGING FIRE], ajoutés le même jour) ; GameLog : bulle 17.03 |
| `[DEADLY DEMISE]` — POSITION (2026-09-18) | émise APRÈS les lignes d'attaque de l'activation qui a tué le porteur (25 DESTROYED : l'explosion se résout une fois toutes les attaques de l'unité attaquante résolues), plus immédiatement après le `DEAD` | analyzer : `dead_models_since_explosion` (remplace `last_dead`) garde les socles tués jusqu'à leur ligne DEADLY DEMISE ; `_SAME_ACTIVATION_LINE_RE` borne l'activation |
...
| `shoot` | `Unit N(c,r) SHOT [ASSAULT] [CLOSE-QUARTERS] [RAPID FIRE:X] [MELTA:X] [BLAST:X] [EXTRA ATTACKS] [PRECISION] Unit M(c,r) with [<arme>] - Hit R(T+ ou base+->eff+) [HEAVY\|COVER\|PLUNGING FIRE] [TORRENT] [IGNORES COVER] [PSYCHIC] [REROLLED:n] [SUSTAINED HITS] [<CAPACITÉ>] - Wound R(T+) [LETHAL HITS] [ANTI-<KW>:Y+] [<CAPACITÉ>] [REROLLED:n] - Save R(T+) [REROLLED:n] [<CAPACITÉ>] - Dmg:NHP [HAZARDOUS] Roll:N` ; ou `Save [DEVASTATING WOUNDS]` | jets, seuils, tokens de règle |
...
| `deadly_demise` | `Unit N DEADLY DEMISE Roll:D → Unit M(c,r) SUFFERS X MW [DEADLY DEMISE]` (d6≥6) ou `… → no effect [DEADLY DEMISE]` (d6<6) — une entrée par unité dans le rayon 6" | compteur `deadly_demise_triggers` + `note_rule_usage("24.08")` + relevé §1.7 sur la source, jugé sur le SEUL socle qui explose (`judged_mids` — ability CORE, non conférée par 19.04, cf. couverture_regles.md § Attached Units) — **COUVERT (2026-08-25, §1.7 le 2026-09-13)** |
...
| `secure_objective` | `Unit N(c,r) SECURES <zone> [GET DA GOOD BITZ\|OBJECTIVE SECURED]` | 14.03, grammaire 15 (2026-09-18) : l'escouade sécurise l'objectif qu'elle contrôle en fin de SA phase de commandement ; l'état par objectif est `Sec=` de l'instantané `OBJECTIVE CONTROL` ; contrôle `objective_secured_invalid` (`ai/analyzer_objectives.py`, PROJ.2.3.objective_secured) — **COUVERT** |
| `ability_call` | `Unit N(c,r) ABILITY CALL <Nom> [USED\|DECLINED]` | appel de capacité (`engine/ability_calls.py`, 2026-09-18) : « you can … » rendu au joueur ; DECLINED distingue « refusé » de « jamais proposé » ; compté par `ai/analyzer.py` (`ability_call_counts`, §1.6) et relevé dans `state.ability_calls` (acteur = préfixe `Unit N(` de la ligne). Grammaire 14 : `[FINEST HOUR]` sur une ligne `combat` n'élève le plafond que si un `ABILITY CALL Finest Hour [USED]` de la même escouade précède ce tour en FIGHT ; sinon une erreur de lecture et plafond non levé — plafond, relevé de l'erreur et verdict dans `ai/analyzer_phases/fight_handler.py` (`_cc_cap_for_line`, `parse_errors`, `fight_over_cc_nb`) |
...
| 21ter | `alloc_character_over_bodyguard['shooting']` (2026-09-18, chantier « chaîne d'attaque 100 % » ; même compteur par phase : `['move']` §1.1, `['charge']` §1.3, `['fight']` §1.4 — corpus `PROJ.1.x.alloc_character`) | `analyzer_core._note_character_allocation_in_lot` / `_flush_character_allocation` (lignes `SHOT`/`FOUGHT`), `_judge_character_allocation` (lignes `SUFFERS`, une ligne = toute l'allocation), verdict `analyzer_rules.character_allocation_fault` | 05.03 / 06.02 / 24.28 : la figurine allouée (`[ALLOC_MODEL:]`) est un CHARACTER alors qu'un non-CHARACTER de l'unité est encore vivant dans `unit_model_hp` **à la FIN DU LOT** (verdict PAR LOT, 2026-09-18 bis, même clé que #21bis + phase, l'activation à la place du tour : épisode, activation, phase, attaquant, cible, arme, signature `shoot_group_signature` — activation = `analyzer_core._activation_id` : le tour hors combat, `fight_phase_seq_id` en FIGHT, car un round porte deux phases de combat et le `P` de la ligne est celui de l'unité). **Jamais ligne à ligne** : dans un lot l'ordre des lignes est celui des JETS (`g["shots"]` étendu intent par intent dans `_roll_batch`), l'allocation suit le pool trié par sauvegarde croissante (05.04) — la ligne de l'Ancient peut précéder celles des bodyguards que le même lot a tués avant lui (piège de 41ter, à nouveau). Les morts d'un lot sont monotones : bodyguard vivant à la fin ⟺ vivant à chaque allocation du lot. L'état de fin de lot est relevé après les dégâts de chaque ligne (`AllocCharacterGroup.non_character_alive`) ; les lots vivent dans `AnalyzerState.alloc_character_groups` (même régime que #21bis) et le verdict est rendu une seule fois, journal lu — aucune fermeture déduite de l'ordre d'émission des unités (2026-09-18 ter, `/simplify` option B : la frontière « `[MODELS:]` étranger » du slot unique est retirée). Légal si la ligne porte `[PRECISION]` (override 24.28 appliqué par le moteur) ET que la blessure est NORMALE ; pour une blessure MORTELLE (`Save [DEVASTATING WOUNDS]`, `SUFFERS`) la clé `alloc.precision_mw_to_character` de l'entête tranche (`False` = cascade 06.02 exigée, `True` = le CHARACTER imposé encaisse, absente = indécidable, non compté). Occasion jugée = ligne allouée sur une unité MIXTE (au moins un CHARACTER et un non-CHARACTER vivants). **Piège DEAD-before-SHOT/FOUGHT** (mesuré 2026-09-18, 22/22 faux positifs) : le moteur journal les lignes DEAD avant les lignes d'attaque du lot fatal ; le handler DEAD pose `unit_hp=0` sans vider `unit_model_hp`. La garde DEAD `_apply_damage_and_handle_death:ai/analyzer.py` retire désormais `alloc_model_id` de `unit_model_hp` avant de retourner, de sorte que le relevé `non_character_alive` de la ligne lit un état cohérent. Non vérifié : priorité « figurine déjà blessée » (PV reconstruits, pas journalisés), ordre des groupes W/Sv/InSv. Test : `tests/unit/ai/test_analyzer_alloc_character.py` |
| 21bis | `shoot_cross_weapon_attacks_lost` / `shoot_cross_weapon_lost_groups` (**métrique, pas une erreur** — hors `error_totals['shooting']`, 2026-09-12) | `shoot_handler.py` (`note_shoot_allocation`, `flush_cross_weapon_lost`) | Tir fractionné : attaques d'une arme arrivées sur une cible qu'une AUTRE arme de la même activation venait d'anéantir. Verdict **par groupe** (épisode, tour, tireur, arme, cible, signature `shoot_group_signature` = tags de tir + seuils de touche et de blessure, constantes de groupe côté formateur — c'est ce qui sépare deux lots de même nom que le moteur sépare par `gkey`, ex. Bolt Pistol d'un personnage rattaché à BS 2+ et ceux de l'escouade à 3+), rendu une fois en fin de lecture du journal (la clé porte l'épisode ; dernier épisode sans `EPISODE END` compris) : un groupe dont TOUTES les lignes à segment `Save` sont `[NOT ALLOCATED]` = lot arrivé sur un cadavre (seul chemin non alloué du moteur, `_mark_manual_overkill_wasted` ; un lot n'existe que s'il a des blessures à allouer ; une cible se choisit vivante), donc tué par un autre groupe de la même activation ; chaque ligne compte. Un groupe **mixte** (allouées + non allouées) est le tueur avec son propre overkill → 0. **Jamais ligne à ligne** : dans un lot l'ordre des lignes est celui des tirs, pas de l'allocation (piège de 41ter, E79 T2) — une ligne `[NOT ALLOCATED]` du tueur peut précéder sa première `Dmg`. Ne lit pas l'état PV reconstruit (ce qui a tué 41ter). Ligne à `Save` sans ` with [arme]` → lève (T1). Perte OBSERVÉE a posteriori (jets compris), pas faute de décision. Règle « Target No Longer Eligible or Viable » (04 RESOLVE ATTACKS) absente du corpus PDF — seul le renvoi existe ; classement en métrique agent posé sur le comportement moteur (attaques perdues, 05.04), à reclasser si le texte prévoyait une réaffectation. Test : `tests/unit/ai/test_analyzer_cross_weapon_lost.py` |
...
| 58-bis | `save_threshold_mismatch` (05.04 : `Save R(<base>+ AP<n> → <eff>+)` vs min(Sv − AP, InSv effective) — InSv de datasheet, `invul_save_override` des sources présentes 19.04, `waaagh_invul` lu dans `T{tour} EFFECTS:` ; camp fautif = défenseur) | `ai/analyzer_save.py` (`check_save_threshold`), sites `shoot_handler.py` + `fight_handler.py` ; PROJ.2.3.save_threshold (2026-09-18) |
| 58-quater | `objective_control_mismatch` (14.02 : à chaque instantané, OC resommé par zone depuis les socles vivants — empreinte réelle ∩ aire, OC de datasheet + `oc_bonus` par figurine si un porteur vit, 0 si battle-shocked ; contrôleur = miroir de `_resolve_objective_controller`) | `ai/analyzer_objectives.py` (`handle_objective_control_snapshot`), site `analyzer_core.py` (branche `OBJECTIVE CONTROL`) ; PROJ.2.3.objective_control (2026-09-18) |
| 58-quinquies | `objective_secured_invalid` (14.03, grammaire 15 : `Sec=` n'apparaît qu'après une ligne SECURES du camp en COMMAND, escouade présente dans l'aire et porteuse ; perte seulement sur strictement plus ; maintien du contrôle sur égalité) | `ai/analyzer_objectives.py` (`handle_secures_line`, `_judge_secured`) ; PROJ.2.3.objective_secured (2026-09-18) |
| 58-sexies | `returned_models_invalid` (REVIVED / Grot Orderly : k ≤ D3, k ≤ mortes rendables, once per battle, phase COMMAND du propriétaire, types morts, aucun leader/support) | `ai/analyzer_objectives.py` (`check_returned_models`), site `analyzer_core.py` (branche `RETURNED`) ; PROJ.2.3.returned_models (2026-09-18) |
| 58-ter | `fnp_threshold_mismatch` (24.12 : `[FNP:s/t+ ×n]` — seuil = meilleur seuil des sources présentes (Dok's Toolz 5+, Psychic Hood 4+ si `[PSYCHIC]`/Da Jump, Unbreakable Resolve 4+ si l'Ancient alloué est dans une aire d'objectif ou à 6" du centre) ; FNP absent sur `Dmg>0` avec source → faute ; `Dmg ≠ n − s` → faute ; sur SUFFERS, `[FNP_ROLLS:]` (grammaire 17) jugé PAR FIGURINE : jet manquant sous une source, jet sans source, seuil faux, sauvés > blessures, attribué ≠ déclaré — en dessous, seule la destruction de l'escouade (aucun socle dans `[MODELS:]`) l'autorise ; source suspendue sous le sursis 19.04) | `ai/analyzer_save.py` (`check_fnp`, `check_fnp_mortal`), sites `shoot_handler.py`, `fight_handler.py`, branches SUFFERS d'`analyzer_core.py` ; PROJ.2.3.fnp (2026-09-18) |
...
| ~~L32~~ | ~~**Da Jump : jet et issue**~~ — **LIVRÉ le 2026-09-18** : `DA JUMP (D6=n) [REPOSITIONED\|MISCAST]` (`movement_handlers._da_jump_log`) + tag `[DA JUMP]` sur SUFFERS, lus par `ai/analyzer_da_jump.py`, LOG_GRAMMAR_VERSION=13 ; verrous `test_da_jump.py`, `test_analyzer_da_jump.py` | `da_jump`, 20.02, 24.09 |
| ~~L31~~ | ~~**Suppression : qui, par qui**~~ — **LIVRÉ le 2026-09-18** : `SUPPRESSES Unit M [SUPPRESSED→M]` (`shooting_handlers.suppress_squad`), lu par `ai/analyzer_suppression.py` (suppression ⇔ touche ⇔ malus, tir et mêlée), LOG_GRAMMAR_VERSION=12 ; verrous `test_suppress_target_decision.py`, `test_analyzer_suppression.py` | `suppress_target_on_shooting`, malus `hit_roll_malus_suppressed` |
