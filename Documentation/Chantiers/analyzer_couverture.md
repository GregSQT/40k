...
| `ability_call` | `Unit N(c,r) ABILITY CALL <Nom> [USED\|DECLINED]` | appel de capacité (`engine/ability_calls.py` (`apply_ability_call`), ligne écrite par `engine/w40k_core.py` (`_record_ability_call_action_log`), 2026-09-18) : « you can … » rendu au joueur ; DECLINED distingue « refusé » de « jamais proposé » ; compté par `ai/analyzer.py` (`ability_call_counts`, §1.6) et relevé dans `state.ability_calls` |
