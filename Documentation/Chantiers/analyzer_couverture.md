...
=======
| `ability_call` | `Unit N(c,r) ABILITY CALL <Nom> [USED\|DECLINED]` | appel de capacité (`engine/ability_calls.py`, 2026-09-18) : « you can … » rendu au joueur ; DECLINED distingue « refusé » de « jamais proposé » ; compté par `ai/analyzer.py` (`ability_call_counts`, §1.6) et relevé dans `state.ability_calls` (acteur = préfixe `Unit N(` de la ligne). Grammaire 14 : `[FINEST HOUR]` sur une ligne `combat` n'élève le plafond que si un `ABILITY CALL Finest Hour [USED]` de la même escouade précède ce tour en FIGHT ; sinon erreur de format et plafond non levé (`ai/analyzer_phases/fight_handler.py` : `_cc_cap_for_line`, `parse_errors`, `fight_over_cc_nb`) |
>>>>>>> main
...
| 58-ter | `fnp_threshold_mismatch` (24.12 : `[FNP:s/t+ ×n]` — seuil = meilleur seuil des sources présentes (Dok's Toolz 5+, Psychic Hood 4+ si `[PSYCHIC]`/Da Jump, Unbreakable Resolve 4+ si l'Ancient alloué est dans une aire d'objectif ou à 6" du centre) ; FNP absent sur `Dmg>0` avec source → faute en grammaire ≥ 16 (`FNP_MARKER_GRAMMAR`), abstention en deçà ; `Dmg ≠ n − s` → faute ; miroir `[FNP:n]` sur SUFFERS : présence sans source) | `ai/analyzer_save.py` (`check_fnp`, `check_fnp_mortal`), sites `shoot_handler.py`, `fight_handler.py`, branches SUFFERS d'`analyzer_core.py` ; PROJ.2.3.fnp (2026-09-18) |
