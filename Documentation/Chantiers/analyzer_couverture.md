...
| 58-ter | `fnp_threshold_mismatch` (24.12 : `[FNP:s/t+ ×n]` — seuil = meilleur seuil des sources présentes (Dok's Toolz 5+, Psychic Hood 4+ si `[PSYCHIC]`/Da Jump, Unbreakable Resolve 4+ si l'Ancient alloué est dans une aire d'objectif ou à 6" du centre) ; FNP absent sur `Dmg>0` avec source → faute ; `Dmg ≠ n − s` → faute ; miroir `[FNP:n]` sur SUFFERS : présence sans source) | `ai/analyzer_save.py` (`check_fnp`, `check_fnp_mortal`), sites `shoot_handler.py`, `fight_handler.py`, branches SUFFERS d'`analyzer_core.py` ; PROJ.2.3.fnp (2026-09-18) |
