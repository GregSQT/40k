...
**Mise à jour du 2026-09-18 — chantier « chaîne d'attaque 100 % »** (doc :
[`chaine_attaque_100.md`](../../Archives/chantiers/chaine_attaque_100.md)). Plusieurs lignes du tableau
ci-dessus décrivent un état DÉPASSÉ ; ce qui a changé, par règle :

| Règle | Ce qui a changé | Test |
|---|---|---|
| **04.03 — ordre des lots** | Plus de pré-jet de tous les lots à la validation : l'attaquant humain choisit **lot par lot** l'unité puis le profil (contrainte « toutes les armes d'une unité avant la suivante »), les dés sont jetés à l'ouverture du lot ; la question n'est posée que s'il y a un vrai choix (`select_attack_lot`, actions `squad_shoot_select_lot` / `squad_fight_select_lot`). Attaquant programmatique (gym/bot) : ordre de déclaration, sans question. Un lot dont la cible est déjà détruite n'est jamais joué (ligne `DID NOT ATTACK … N attack(s) not made`). | `test_lot_selection_04_03.py` (10) |
| **Relances (04.03 + 24.10/24.23/24.36)** | Politique **par lot** : « échecs seulement » (défaut) ou « tous les non-critiques » (chercher les 6), séparément pour la touche et la blessure ; posée à l'attaquant humain seulement si une relance existe ET qu'un critique déclenche une règle ([DEVASTATING WOUNDS] côté blessure, [SUSTAINED HITS]/[LETHAL HITS] côté touche). `RerollProfile.hit_non_crit` / `wound_non_crit`, `reroll_policy_choices`. | `test_reroll_policy_par_lot.py` (12) |
| **05.04 — choix de la figurine** | Le défenseur humain n'est interrogé que devant un **vrai choix** (plusieurs figurines intactes, ou plusieurs entamées — 05.04 « must be a model that has lost one or more wounds if possible » laisse le choix entre les entamées) ; un candidat unique est alloué d'office. `wounded[0]` retiré. | `test_allocation_choix_reel_05_04_06_02.py` (8) |
| **06.02 + PRECISION (24.28)** | Toute blessure MORTELLE suit la cascade 06.02 sur toute l'unité (Deadly Demise, hazard, Hold Still, [DEVASTATING WOUNDS]), plus le groupe courant. Les mortelles d'une arme [PRECISION] dont l'override a joué : clé `game_rules.precision_mortal_wounds_to_character` (défaut `false` = cascade, lecture littérale des PDF ; `true` = restent au CHARACTER imposé), **en attente de confirmation GW**. Moteur (`_precision_mortal_wounds_to_character`), analyzer (`Run rules: alloc.precision_mw_to_character`, contrôle `PROJ.1.x.alloc_character`) et tests lisent la même clé. Le tableau ci-dessus (ligne PRECISION) reste vrai pour les blessures normales. | idem + `tests/unit/ai/test_analyzer_alloc_character.py` (15) |
| **24.02 — Feel No Pain** | Un seul jet par figurine au **meilleur** seuil (abilities dupliquées non cumulatives) ; l'ancien enchaînement de jets est retiré. | `test_feel_no_pain_conditional.py` |
| **24.08 / 25 DESTROYED — Deadly Demise** | Différée **après** que l'unité attaquante a résolu toutes ses attaques (file `MORTAL_WOUND_QUEUE_KEY`, `drain_mortal_wound_queue`), 6" mesurés à la figurine la plus proche de chaque unité, défenseur humain : allocation manuelle 06.02. | `test_deadly_demise_apres_les_attaques.py` (3) |
| **24.15 — HAZARDOUS, reprise PvP** | Après l'allocation humaine des mortelles d'un hazard, l'activation se **termine** (`_resume_after_hazard` → `post_attack_effects` puis fin d'activation tir / état combat) — bug préexistant : l'unité restait active. | `tests/integration/pvp/test_shoot.py::test_hazardous_human_allocation_then_the_activation_ends` |
| **17.03 — MONSTER/VEHICLE engagée** | Ciblable au tir ; −1 au jet de touche (token `[ENGAGED TARGET]`), levé pour [CLOSE-QUARTERS] depuis l'unité engagée avec elle ; [BLAST] refusé (FAQ p. 88). | `test_engaged_monster_vehicle_17_03.py` (7) |
| **24.07 — PISTOL** | Choix **par figurine** (hors MONSTER/VEHICLE), imposé sur le chemin manuel (`_declare_qty_candidates`, `squad_shoot_menu_weapons`). | `test_squad_shoot_declaration.py::TestSidearms2407` |
| **04.01 / 04.02 / 24.11 — mêlée** | Une seule arme ordinaire par figurine, plus toutes ses [EXTRA ATTACKS] (imposé aux deux `validate`, `squad_fight_complete_extra_attacks`) ; répartition des attaques d'une arme entre plusieurs unités engagées (`squad_fight_split_weapon_attacks`, action `squad_fight_split_weapon_attacks`). La ligne EXTRA ATTACKS ci-dessus décrivait le seul chemin gym. | `test_squad_fight_declaration.py` (04.01/04.02/24.11) + `tests/integration/pvp/test_fight.py` |
