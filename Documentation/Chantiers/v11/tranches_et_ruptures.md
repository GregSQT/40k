...
`j_entropy_loss`, `m_immediate_reward_ratio_mean`, **56 points chacune** (lettres d'avant le
2026-09-06 : `g` à `j` sont depuis décalées en `h` à `k`, et `m_immediate_reward_ratio_mean` est
devenue `n_immediate_reward_ratio_mean` à l'insertion de `l_approx_kl_max` / `m_explained_var`,
cf. `metriques.md`) ; `training_critical/` expose ses
