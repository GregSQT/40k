...
| Quoi | Où |
|---|---|
| Rosters agent (training) | `config/agents/ArmageddonAgent_x1/rosters/500pts/training/agent_training_roster_{space_marines,orks}.json` |
...
| Scénario d'entraînement | `config/agents/ArmageddonAgent_x1/scenarios/training/scenario_training_armageddonN.json` (`1` = terrain mc1, `2` = terrain mc2 ; identiques par ailleurs) — `agent_roster_ref: "training_random"` (tirage 50/50, **pas de `agent_roster_seed`** : il figerait le tirage agent), `opponent_roster_ref` = liste explicite des 2 fichiers (sinon P2 tire dans tout `_p2_rosters`) |
