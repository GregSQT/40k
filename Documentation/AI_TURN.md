...
| 19.04 | Abilities in attached units (règle d'unité → toutes les figurines de l'unité attachée) | ❌ **Non implémenté** | **Trou vif, reproduit** : le fold pose les `UNIT_RULES` du character sur `models[i]` ([game_state.py:1019](../engine/game_state.py#L1019)) et **jamais** sur l'escouade ([game_state.py:918](../engine/game_state.py#L918)) ; tous les consommateurs lisent l'escouade (`_unit_has_rule_effect`, [shared_utils.py:1904](../engine/phase_handlers/shared_utils.py#L1904)). Vérifié : Intercessor + `CaptainPowerWeaponBolter` attaché → `UNIT_RULES` escouade `[]`, figurine `['reroll_charge','leader']`, `unit_can_reroll_charge` = **False**. Le leader ne perd pas seulement l'effet unit-wide : il perd sa règle **tout court**. Trois manques : (a) **montant** — règle du leader/support non appliquée à l'unité ; (b) **descendant** — règle du bodyguard toujours active alors que le PDF l'éteint quand la dernière figurine bodyguard meurt ; (c) fenêtre « until the attacking unit has resolved all of its attacks » après la mort de la source. Impacte gym ET PvP, et l'observation IA (`ObservationBuilder._encode_rule_features`, [observation_builder.py](../engine/observation_builder.py) — ancre par NOM de fonction : les numéros de ligne ont bougé avec le passage aux tenseurs d'entités du 2026-07-26) |
