...
| `test_agent_interface_contract.py` | 26 | V11 §8.2 — verrou d'interface agent : chaque entier d'action APPELÉ sur `convert_squad_action` rend l'intention attendue (cellule de move, wait, slots tir/charge/mêlée, combat sans cible, zone intent, CHOICE, slot de déploiement) + les gardes qui doivent lever. Vérifié par mutation, cas par cas |
