...
| `tests/unit/engine/test_forced_wait_not_penalised.py` | 8 | **Attente FORCÉE** (le masque n'ouvre que `wait`) : elle ne coûte plus la pénalité de passivité, et le moteur la joue lui-même au lieu de rendre la main. Situation CONSTRUITE (ennemi hors de portée d'arme), témoin à portée pour la non-régression, `info` qui reste celui de l'action de l'agent, et borne au joueur qui vient d'agir |
