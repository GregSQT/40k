...
| `tests/unit/engine/test_roster_fixed_positions.py` | 3 | Positions top/bottom du roster sur le VRAI moteur, mode `fixed` (P1 bande haute / P2 bande basse, cohérence 03.03) et `active` (sentinelle). Plus **le cas construit « réserves 20.01 en mode `fixed` »** : les zones de déploiement ne vivaient que dans `deployment_state`, écrit uniquement quand un joueur déploie en `active` — une unité en réserves étant hors table dès le reset, `squad_grid_anchor` et la clause 20.04 levaient, et le reset entier avec, sur 20 à 70 % des épisodes des profils réels. Le cas est ÉPINGLÉ sur la fixture à réserves des deux côtés, pas espéré d'un tirage. Verrou prouvé rouge |
