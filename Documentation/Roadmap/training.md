...
Option 2 CODÉE sans run (clé `logits_temperature_regulation`, profil expl à `null` = témoin) ; protocole de la paire inchangé (par graine, témoin T = 2 puis régulée, même P0′, règle ±5 pts = 2σ). Suite (prompt 6 du cycle 3, à toi après ta vérification large) : attendre la fin des correctifs tir/fight de la session parallèle (chaîne d'attaque 100 %, suite `worktree-chaine-attaque-100`), poser le hash de gel, puis P0′ à froid, paire option 2 sur l'exploiteur copiant P0′, P1′ en run de check.
