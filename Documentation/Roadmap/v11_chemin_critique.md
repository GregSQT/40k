...
🟢 **Move-after-shooting (2026-09-09, worktree `move-after-shooting-agent-decision`)** : type `move_after_shooting` dans `AGENT_DECISION_TYPE_IDS` (slot réservé — `obs_size` et `TOTAL_ACTION_SIZE` inchangés, mesuré). L'heuristique est supprimée ; gym et bot PvE reçoivent 3 intentions scorées (Pression = historique en `CHOICE_0`, Retrait, Objectif) plus `declines`, et la réponse passe par le handler du PvP. 16 tests rouge→vert. Correctif du 2026-09-09 (worktree `bot-move-after-shooting-baseline`) : l'armement se lit sur le drapeau GLOBAL `gym_training_mode`, donc la décision est posée aussi pour les escouades du BOT adverse — que le tirage générique de `random_action_for_pending_choice` faisait alors se replier au hasard. `bot_action_for_pending_choice` lui rend sa baseline (`CHOICE_0`, la destination historique) ; J2 concerne le siège de l'AGENT, pas celui de l'adversaire de référence, qui doit rester stable pour que les win-rates soient comparables d'un run à l'autre. ⚠️ Le contrat d'entraînement (`ai/training_contract.py`, dont l'artefact de run porte le même nom en `.json`) compare `AGENT_DECISION_TYPE_IDS` : une reprise `--append` sur un modèle antérieur sera refusée, l'ajout étant en fin de vocabulaire et inoffensif pour les ids existants.

🔴 **reactive_move** : `reactive_decision_mode` figé à `"auto"` (`w40k_core.py`, 2 sites), jamais `"state"` hors tests ; le mode auto ne décline jamais et choisit la case la plus proche de l'ennemi qui vient de bouger. Écarté sur MESURE : son seul porteur (`Termagant`) est absent du régime d'entraînement actif (500 pts, `ArmageddonAgent_x1`) — le brancher coûterait un type et ses tests pour zéro gradient. À rouvrir si un roster d'entraînement le porte. *(Rectifié le 2026-09-09, suite 40 : `FenrisianWolf` n'est pas un porteur, la règle lui a été retirée. La CONDITION D'ENGAGEMENT de la datasheet, elle, est livrée — seul le refus reste ouvert.)*
...
🟢 **Split-fire livré le 2026-08-24** (P3-8 gym) : 10 `SHOOT_WEAPON_SEL_SLOTS` (1379–1388), `TOTAL_ACTION_SIZE` 1379→1389. Flux 2-step : slot j → `squad_shoot_weapon_sel` (init activation + pending_shoot_weapon_split) → SHOOT_SLOT i → `squad_shoot_split_target` (déclare le lot IMMÉDIATEMENT sur l'état réel, puis referme les slots d'arme que plus aucune figurine ne peut déclarer — arme consommée, famille 24.07 pistolet / autre — ; résolution quand aucun slot ne reste ; corrigé le 2026-09-18, la déclaration différée commettait des comptes pris sur l'état initial). Tête dense `shoot_weapon_sel_net` (K_WEAPONS_RANGED=10 logits) dans `pointer_policy` — **remplacée le 2026-09-09 par la tête pointeur `shoot_weapon_sel_query_net`**, qui score l'ARME de l'emplacement et non son rang. Masque via `_model_can_shoot_target_with_weapon` (sans activation). Ré-entraînement `--new` nécessaire.
