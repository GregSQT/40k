...
`--test-only` utilise `eval_episodes` (pas `bot_eval_final`, qui ne s'applique qu'à la fin d'un run d'entraînement). Les **7** profils de la config sont tous à 48 envs ; seul `x1_long` atteint la précision cible :
