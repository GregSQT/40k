...
- **`--test-only --step` est inutilisable en l'état** : le modèle `ArmageddonAgent_x1` est incompatible avec l'espace d'observation courant (`global_bin` 83 contre 93, `global_cont` 13 contre 23, `grid` 9 canaux contre 11) et le run s'arrête avant d'écrire. Le verrou pilote donc l'env par politique aléatoire MASQUÉE et ensemencée : il porte sur le moteur, pas sur la policy, et ne dépend d'aucun artefact qui dérive.
