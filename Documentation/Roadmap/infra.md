...
- **Champ de montée en métrique HEX — sol = BFS hex du gym, livré le 2026-09-16 : −19 % de ms/step sur le banc x1** (`scripts/bench_env_step.py --steps 200`, x1_long bots, graine 42, 3 répétitions : 155,6 / 175,0 / 173,4 ms/step avant → 136,0 / 135,4 / 136,0 après ; `wall total` 32,9–37,2 s → 29,1–29,3 s ; machine partagée, l'avant est mesuré par mutation de l'injection sur le même arbre, chemin identique à l'ancien code). Le lot B du 2026-09-12 (ci-dessus) laissait le chemin hex intact : à x1 la passe de niveau 0 du champ multi-niveaux restait le Dijkstra ANY-ANGLE avec clairance de socle — 126 passes de ~4 900 cases à 70 ms = 8,8 s sur 39,8 s de wall pour 200 pas (champ de montée 10,4 s, 26 % du step ; passes d'étage 0,9 s). `ascent_field_for_model` (`shared_utils.py`) injecte désormais en `precomputed_start_field` le BFS hex de plain-pied (`geodesic_move_reach` sur le bitmap de niveau 0 déjà en main, × `ENGAGEMENT_NORM_HEX_WIDTH`, 191 BFS = 0,23 s) ; après : champ de montée 2,4 s, 0 passe any-angle au sol, 171 passes d'étage 1,3 s. **CHANGEMENT DE MÉTRIQUE ASSUMÉ, pas une optimisation à résultat identique** : le sol d'une montée est mesuré comme le move à plat du gym (pas hex par cellule, sans clairance) au lieu d'any-angle avec clairance — jusqu'à 15 % de portée en plus (√3/1,5 vers le sud) et couloirs d'une case admis ; la passe d'étage reste any-angle avec clairance (13.06 « no part of its base overhangs »). Masque (`erode_move_pool_by_squad_block`) et validation (`model_reach_predicate`) lisent le même champ ; le pool PvP ne construit d'étage qu'en euclidien, non concerné. **Pré-check de portée rendu conscient de la métrique** (`multilevel_target_within_straight_bound`, paramètre `ground_is_hex`, transmis par `_model_multilevel_reachable_field` via `precomputed_start_is_hex`) : la ligne droite euclidienne ne minore plus un sol hex (1,5 × pas ≤ euclid), un plancher 10 lignes plein sud est à 15 pas là où la ligne droite en compte 17,3 — sans l'aiguillage le champ rendait `{}` en silence (prouvé : budget 15 → 1 case, 16 → 4 cases, pré-check ligne droite `False`). Borne hex = 1,5 × `hex_distance` + |Δhauteur|, tolérance `_SEG_TOL` conservée (la passe d'étage admet à `budget + tol`) ; sur les mêmes 191 appels, 54 sorties précoces contre 65 en ligne droite (11 passes d'étage de ~7 ms de plus). L'appel charge garde la ligne droite (son sol reste any-angle). Verrous `tests/unit/engine/test_ascent_field_hex_ground.py` (10 tests : passe de départ = BFS hex, champ ⊇ any-angle à budget égal — vérifié en amont sur 32 245 cases de 60 cartes aléatoires, 0 violation — et borné par « distance hex + montée », couloir d'une case ouvert au hex et fermé à l'any-angle, pré-check hex sur la scène plein sud avec et sans tolérance, jamais plus serré que la ligne droite, chemin de production, parité érosion/prédicat) ; mutations : injection retirée → 3 rouges, borne ligne droite rétablie → 4 rouges. Rupture de comparabilité des sondes consignée dans `training.md` [#rupture-sol-hex-2026-09-16](training.md#rupture-sol-hex-2026-09-16).
