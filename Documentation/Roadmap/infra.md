...
- **La trace est déterministe.** Deux runs identiques de 150 pas rendent 402 lignes chacun, dont UNE seule diffère : `Duration=<n>s`, du temps de mur. C'est, avec l'horodatage de début de ligne, la seule chose à normaliser.
- **`step.log` porte bien le par-figurine sur la charge.** Le segment `[MODELS: <mid>@(c,r,z) …]` est capturé immédiatement après `commit_move` (`w40k_core.py:6248-6250`) et accolé génériquement par `log_action` : 302 lignes sur 402 le portent, lignes de charge comprises. Un verrou d'empreinte voit donc un placement par figurine modifié — il n'est PAS limité à l'ancre d'escouade.
- **Le verrou ne passe pas par `--test-only --step`, et c'est un choix, pas un contournement.** Au 2026-09-09 le modèle `ArmageddonAgent_x1` précède la dernière croissance de l'observation (`global_bin` 83 contre 93, `global_cont` 13 contre 23, `grid` 9 canaux contre 11), donc le chargement refuse le checkpoint et le run s'arrête — comportement VOULU (T1 : erreur explicite plutôt que reprise silencieuse sur poids aléatoires), qui se résout par un ré-entraînement `--new` et n'a rien d'un défaut. Surtout, un verrou d'empreinte adossé à un modèle serait à refaire à chaque ré-entraînement : celui-ci pilote l'env par politique aléatoire MASQUÉE et ensemencée, donc il porte sur le MOTEUR et survit aux changements de policy et d'observation.
