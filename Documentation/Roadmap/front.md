...
| Chaîne d'attaque 100 % 2026-09-18 | Navigateur PvP : (1) déclarer deux armes sur deux cibles → réticule rouge sur TOUTES les figurines des deux unités, voile jaune sur l'active ; (2) Shoot → `AttackLotPanel` s'ouvre de lui-même (choix de lot si ≥ 2 unités, sinon lot imposé), réticule CLIGNOTANT sur l'unité du lot en cours, clic sur une unité candidate du plateau = choix du lot ; (3) arme [DEVASTATING WOUNDS] avec relance → popup « échecs seulement / tous les non-critiques » ; (4) défenseur : panneau d'allocation (arme, blessures restantes, candidates cliquables) seulement s'il y a ≥ 2 candidates ; (5) mêlée : mêmes panneaux via `squad_fight_select_lot`. Vitest : `AttackLotPanel.test.tsx` (8), `reticle.test.ts` (6), `useEngineAPI.test.ts` (+5). `tsc` projet : laissé à la vérification large. |
