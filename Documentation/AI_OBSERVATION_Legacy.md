# AI_OBSERVATION_Legacy.md — pipeline MONO-FIGURINE (archive)
...
>   un `Dict` de tenseurs, pas un vecteur plat.
> - Les **« 12 unit-rule flags »** de `obs[314:346]` faisaient croire que le pipeline squad
>   exposait les règles d'unité ; il ne le faisait pas avant le 2026-07-27, et il les porte
>   désormais ailleurs (13 bits `rule_<effet>` par entité).
> - Les features **calculées** (`ranged_favorite_target`, `melee_favorite_target`,
>   `combat_mix_score`, `danger_to_me`, `best_kill_probability`…) ont été **supprimées** au profit
>   de données brutes (`V11_audit_observation.md` §9.1) : l'agent voit les caractéristiques
>   réelles et compose lui-même, au lieu de recevoir un score pré-mâché.
> - Les **tailles** citées (108, 165, 199, 323, 355, 357, 1011…) sont des jalons historiques.
>   La valeur courante vit dans `ObservationBuilder.SQUAD_OBS_SIZE_TARGET`.
>
> Scindé de `AI_OBSERVATION.md` le 2026-07-28 : ce contenu induisait en erreur une fois par
> lecture, malgré le bandeau d'avertissement qui le précédait dans le fichier unique.
