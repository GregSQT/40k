...
> **Principe** : mesurer avec `W40K_PERF_TIMING=1` (`perf_timing.log`) ; ne pas masquer les divergences de règles (`Documentation/Reference/moteur/AI_TURN.md`).
...
|-------|-------------|-----------------|
| **Noyau natif BFS / pool** | Porter la boucle chaude de `movement_build_valid_destinations_pool` (Rust / Cython / C) avec structures compactes (bitsets, etc.). | **Élevé** ; tests de **parité** obligatoires (liste de destinations, règles murs / alliés / FLY). Voir `Documentation/Chantiers/backlog/perf_noyau_natif_et_gzip.md` §2. **Ne pas livrer** sans jeux de tests bit-à-bit sur les pools. |
...
| Stratégie ×10 (gzip, natif) | `Documentation/Chantiers/backlog/perf_noyau_natif_et_gzip.md` |
...
1. Si **`bfs_s`** domine : suivre **`Documentation/Chantiers/backlog/perf_noyau_natif_et_gzip.md`** §2 (noyau natif ou Cython) **avec** tests de parité sur **`valid_move_destinations_pool`** (triés, mêmes entrées plateau).
