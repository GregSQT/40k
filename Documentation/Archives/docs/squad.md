...
6. **Format roster multi-fig (option B)** : 1 ligne par figurine declaree dans `unit["models"]` avec `col`, `row`, optionnellement `unit_type` pour profils mixtes. Cf. [squad_audit.md §8](../../Archives/chantiers/squad_audit.md). Pas encore utilise par un scenario en PR1 — premier usage en PR2 (tests multi-fig).
