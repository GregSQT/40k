> **⛔ ABSORBÉ le 2026-08-27 par `Documentation/Reference/moteur/architecture_moteur.md`** (consolidation Reference) — conservé comme source historique, ne plus maintenir ni citer.
...
- **Zones** : `game_state["deployment_pools"]`, à la **racine** et publiées **quel que soit le mode de mise en place** (`active`, `fixed`, `random`). C'est une donnée de **scénario**, pas d'une phase : la clause 20.04 « aucune figurine dans la zone adverse avant le 3e round » et l'ancre de grille d'une unité hors table en ont besoin hors déploiement. Les enfermer dans `deployment_state` faisait lever le reset en mode `fixed` dès qu'une unité était en réserves 20.01. Les murs en sont soustraits **inconditionnellement** : un hex de mur n'est une case de déploiement légale dans aucun mode, et l'ancre de grille (barycentre du pool) doit être la même quel que soit le tirage `fixed`↔`active`. Clé déclarée **statique** pour les snapshots PvP (`services/game_snapshots.py`) : elle est ré-attachée depuis l'engine vivant au restore, ce qui fait aussi reprendre les parties picklées avant son déplacement.
...
**Reactive move (unit rule)** : la règle `reactive_move` est une règle d’unité (déplacement réactif après mouvement ennemi). Spécification complète (objectif, game_state, éligibilité, résolution, caches, erreurs, flux, tests, plan d’implémentation) : **[Unit_rules.md](../jeu/Unit_rules.md)** section « 10) Specification : reactive_move ».
...
└─ CONVERT ACTION
   └─> action_decoder.convert_squad_action(action_int, game_state, eligible_units)
       ...
       │  • 1127-1138 → ACTIVATE, slot d'escouade ALLIÉE 0-11 : QUI activer
       │                (V11 §0.48 L2). Un slot par LIGNE du tenseur allié de
       │                l'observation — invariant D1 côté allié. Le masque est
       │                EXCLUSIF quand ce choix est posé : aucune action de
       │                phase n'est ouverte tant qu'il n'est pas joué
...
See `Documentation/Chantiers/backlog/perf_entrainement.md` §1 and §6, and the "CPU vs GPU"
section of `Documentation/Reference/training/AI_TRAINING.md`.
...
2. [AI_TRAINING.md](../training/AI_TRAINING.md) - PPO training integration
3. [AI_OBSERVATION.md](../training/AI_OBSERVATION.md) - Egocentric observation system
4. [Weapon_rules.md](../jeu/Weapon_rules.md) - Weapons system technical documentation
...
- **Configuration discipline**
  ...
  - Any new configuration keys are documented in `Documentation/Reference/outils/CONFIG_FILES.md` or
