...
> `Documentation/Reference/training/entrainement.md`, qui est désormais la source vivante. Conservé ici pour l'historique — ne plus
...
(`train.py`), et 1000 épisodes à `bot_eval_freq` 250 n'en donnent que 4 pour une fenêtre
...
`train.py`) : une clé absente ou nulle hérite de `_training_common.json`, qui définit
...
`save_freq` **appels du callback** (`callbacks.py`), soit un par pas du VecEnv — jamais des
...
- **Corollaire assumé** : un paramètre dont la valeur effective n'est pas relisible est **refusé**, pas mesuré à l'aveugle — `clip_range` et `policy_kwargs`/`net_arch` (sérialisés en cloudpickle par SB3). Les chemins sont donnés en toutes lettres (`model_params.batch_size`), jamais via les alias courts de `--param`, pour ne pas dupliquer la table `_PARAM_ALIASES` ([train.py](../../../ai/train.py)).
...
- **`n_steps` n'est pas une égalité** : la config donne le total par mise à jour, PPO reçoit `base // n_envs` ([train.py](../../../ai/train.py)). Le contrôle en tient compte.
...
- **`--scenario bot`, `self` ou `all` uniquement** : `--total-episodes` n'est lu que sur ce chemin ([train.py](../../../ai/train.py)) et en mode rule-checker ; le mode curriculum prend `max_episodes_in_phase` et `create_multi_agent_model` ignore l'option. Un banc lancé sur `phase1` jouerait 30000 épisodes au lieu de 96, sans aucun signal. Les bancs refusent à l'entrée, et vérifient en plus la ligne `Using total_episodes from CLI` puis lisent `Total episodes trained` — le débit est **normalisé sur les épisodes réellement joués**, jamais sur le budget demandé (les `n_envs` slots ne terminent pas ensemble, et ce dépassement dépend de la config mesurée).
- **La division `n_steps // n_envs` n'est pas un invariant** : les zips de `ai/models/ArmageddonAgent/` portent les deux régimes à `n_envs=8` identique. Le contrôle **lit la ligne émise par la division** au lieu de la supposer, sinon il avortait la campagne sur un run correct. ⚠️ Corrigé le 2026-08-27 : cette ligne situait la division dans `train_with_scenario_rotation`, ce qui est faux. Elle vit à un **point de passage unique**, `apply_rollout_n_steps` ([train.py](../../../ai/train.py)) — vérifié, c'est la seule occurrence de `// n_envs` dans le fichier, et `train_with_scenario_rotation` ([train.py](../../../ai/train.py), précédée de deux `@overload` de typage) ne la contient pas.
...
- Écart résiduel non corrigé : `effective_n_steps = base_n_steps // n_envs` est une division entière (`apply_rollout_n_steps` dans [train.py](../../../ai/train.py)) alors que le message annonce `base_n_steps`. Total réel par mise à jour **à `base_n_steps=8192`** : 8190 à `n_envs=6`, 8192 à 8 et 16, **8160** à 48 — écart max 0,4 %, sans effet sur le classement. À `base_n_steps=8160` (valeur courante) la division tombe juste à 24 comme à 48 : 8160 dans les deux cas.
...
`x1_long --etape P1 --resolution 1`). Source: `Documentation/Chantiers/backlog/perf_entrainement.md` §1 and §6.
...
(`_resolve_device_for_obs`, `train.py`); forcing `--mode CPU` moves the update onto the same
...
`--mode CPU` / `--mode GPU` still exist as an argument (`train.py`) and remain valid to
pin a device deliberately — e.g. reproducing a CPU-only measurement, or working around a driver
issue. The old MlpPolicy benchmark (`resolve_device_mode`, `train.py`) only applies to a
...
- Canonical layout reference: `Documentation/Reference/training/AI_OBSERVATION.md`
