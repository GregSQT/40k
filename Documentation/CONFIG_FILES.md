...
| `game_rules.max_base_size_hex` | integer | Plafond de DIAMÈTRE d'empreinte, en hexagones, servant à borner la prune conservatrice des ennemis (`movement_handlers._enemy_items_within_move_engagement_horizon` et `observation_builder._engagement_relevant_entries`, actives sous `engagement_zone > 1`). **Aucune valeur par défaut côté code** : `shared_utils.get_max_base_size_hex` lève si la clé manque — un défaut littéral masquerait un état de jeu malformé. ⚠️ N'est PAS scalé par `inches_to_subhex` (contrairement à `engagement_zone`) : c'est déjà un nombre d'hexagones, pas des pouces. |
