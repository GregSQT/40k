...
**Dépensée n'est pas éteinte.** Finest Hour consomme son usage à l'acceptation de son appel (sélection 12.04) mais accorde `[DEVASTATING WOUNDS]` jusqu'à la **fin de cette phase de combat** : le prédicat du moteur (`shared_utils`, `attack_sequence`) lit deux ensembles, `finest_hour_used` **moins** `finest_hour_active_this_phase`. L'observation lit les mêmes deux ensembles, sous garde de phase — `finest_hour_active_this_phase` n'est purgé qu'à l'entrée de la fight phase suivante, et le moteur ne s'en aperçoit pas parce qu'il ne le lit qu'en combat, alors que l'observation est construite à chaque step. Verrous : `def test_once_per_battle_capability_disappears_once_spent` et `def test_once_per_battle_capability_stays_visible_while_still_in_effect` (`tests/unit/engine/test_squad_obs_unit_rules.py`).
...
**Le contrat vaut dans les deux sens depuis le 2026-09-08** : le chargeur exigeait qu'une règle du vocabulaire ait un `obs_id`, rien n'exigeait qu'une règle portant un `obs_id` soit dans le vocabulaire — et 14 règles vives (dont `deadly_demise`, `oc_bonus`, `secure_objective_on_control`, les deux FNP conditionnels) ont vécu avec un id qu'aucun slot ne pouvait porter : le moteur les appliquait, l'agent ne les voyait pas. Une règle **inerte** reste hors vocabulaire, mais elle ne doit alors porter **aucun** `obs_id`. Verrou : `def test_every_registered_obs_id_is_in_the_vocabulary` — sans liste d'exceptions, une entrée de plus coûtant zéro scalaire. Les registres jumeaux (`config/weapon_rules.json`, `config/unit_statuses.json`) ont été vérifiés le même jour : aucun `obs_id` orphelin.
...
`AGENT_DECISION_TYPE_SLOTS` est passé de 16 à 24 dans la même refonte, qui a déclaré `suppress_target`, payé par le même `--new`. Le tuple porte aujourd'hui 15 types déclarés et 9 réservés.
...
| Finest Hour | Captain with Relic Shield | `once_per_battle_melee_buff` | mêlée +3 A et `[DEVASTATING WOUNDS]` jusqu'à la fin de la phase — **appel de capacité** à la sélection 12.04 (§Finest Hour) |
...
### Finest Hour : un CHOIX à la sélection, plus un déclenchement du roller — 2026-09-18

Datasheet : *« Finest Hour (Once per battle): In the Fight phase, when this unit is selected to fight, this model's melee weapons have the following until the end of the phase: +3 A, [DEVASTATING WOUNDS]. »* Jusqu'à cette date le roller (`_manual_roll_fight_intent`) **décidait à la place du joueur** : première activation de la partie → usage consommé, +3 A sur le seul premier intent, et le sélecteur d'armes scorait DEVASTATING WOUNDS sur « pas encore dépensée ». Désormais :

- **Un appel de capacité** (`engine/ability_calls.py`, §1) posé par `fight_arm_finest_hour_call` (`fight_handlers.py`) au moment où l'unité est sélectionnée, aux **deux sites** de sélection — gym/bot (`W40KEngine._process_squad_action`, branche `squad_fight`, juste après `_fight_v11_register_selection` et avant l'Exhortation) et flux manuel (`_fight_v11_manual_activate`, sous le verrou `_fight_v11_activation_locked`, qui compte désormais `fight_finest_hour_asked`). Conditions : porteur vivant (`fight_finest_hour_bearers`, règle lue sur la figurine — « this model's »), usage non dépensé, une question par phase et par escouade. Exhortation of Rage et Finest Hour sur la même escouade → `ValueError` (19.01 : un seul leader attaché).
- **Trois sièges** : gym `CHOICE_0/1` ; bot PvE et bot adversaire du gym par `_bot_finest_hour_policy` (accepter si une ennemie engagée compte ≥ 5 figurines vivantes ou porte CHARACTER) ; humain par le panneau d'appel et `select_rule_choice` — tant que le prompt est actif, tout autre verbe attend (garde `active_rule_choice_prompt`). Le combat est **suspendu** sous `FIGHT_SELECTION_FINEST_HOUR_KEY` (escouade, slot de cible, régime) et repris par `W40KEngine._resume_fight_after_finest_hour` — régime gym : `_continue_squad_fight_after_selection` ; manuel : l'unité reste active, le joueur déclare.
- **Résolution** : `apply_finest_hour_call` pose `finest_hour_used` (once per battle) ET `finest_hour_active_this_phase` (jusqu'à la fin de la phase) ; refus = rien. Les sites de résolution ne lisent **que** `finest_hour_active_this_phase` : le +3 A est entré dans `melee_attacks_characteristic_bonus` (connu à la déclaration, donc réparti par 04.02 comme le Waaagh! — et sur **chaque** arme de mêlée du porteur, « melee weapons »), DEVASTATING WOUNDS dans le roller et le sélecteur d'armes.
- **Journal** : `Unit N(c,r) ABILITY CALL Finest Hour [USED|DECLINED]` puis `[FINEST HOUR]` sur chaque attaque ; **grammaire 14** : l'analyzer (`_cc_cap_for_line`) ne lève le plafond qu'après un `USED` de la même escouade ce tour en FIGHT, sinon `parse_error` + `fight_over_cc_nb`.
- Verrous : `tests/unit/engine/test_finest_hour_call.py` (trois sièges, moteur réel), `test_primitive_b_granted_weapon_effects.py` (roller), `test_squad_fight_declaration.py` (04.02), `tests/unit/ai/test_analyzer_finest_hour_call.py`.
...
**Exhortation of Rage** : *« you can select one enemy unit it is engaged with »* — c'est un choix de joueur, donc une décision d'agent, pas une heuristique interne. **La cible précède le dé** (*« select one enemy unit … and roll one D6 »*) : la décision `mortal_wounds_target` est posée AVANT tout jet, et c'est `def _apply_exhortation_de_rage` (`engine/w40k_core.py`) qui lance le D6 une fois la cible connue. L'ordre inverse (jet, puis décision sur 4+ seulement) donnait à l'agent le nombre de blessures avant qu'il choisisse — information que la règle ne lui donne pas — et ne laissait aucune trace d'un jet raté. Corrigé le 2026-09-12 : **tout jet laisse sa ligne**, échec compris (`SUFFERS 0 Mortal Wounds … Trigger:<n> [NO ALLOC]`).

**Exhortation of Rage — deux régimes de sélection, un seul jet, le régime suit le SIÈGE.** Jusqu'au 2026-09-12, le seul site qui jouait le dé était `def _process_squad_action` (gym / politique) : un Chaplain HUMAIN (PvP, ou siège humain du PvE) enregistrait sa sélection 12.04 sans jamais rouler le D6. La machine V11 (`fight_handlers.py`) ne porte pas le moteur, donc elle **arme** la capacité à la sélection (`def _fight_v11_arm_selection_exhortation`, clé `FIGHT_SELECTION_EXHORTATION_KEY`) et `def _process_fight_phase` (`w40k_core.py`) la consomme dans la même requête — pendant exact du site gym. Le régime (`EXHORTATION_REGIME_*`) suit la décision et la reprise : **gym** (réponse par la politique, reprise par slots/armes), **manuel** — siège humain, PvP comme PvE (réponse par le panneau `mortal_wounds_target` de `BoardWithAPI.tsx`, reprise par `_fight_v11_manual_state`). L'ancien troisième régime **auto PvE** (siège humain PvE résolu sans choix de joueur, cible par `_ai_select_fight_target`) a disparu avec le chemin auto de la phase fight (2026-09-12, chantier « phase fight PvE par siège ») : le siège humain du PvE joue la machine manuelle, et son défenseur bot se voit attribuer les blessures mortelles headless (06.02, siège machine). En manuel, **l'activation EST la sélection** : le dé tombe à `activate_unit`, AVANT toute déclaration d'attaque (le joueur déclare contre les survivants), l'unité n'est enregistrée « selected to fight » qu'au moment où elle combat, et jusque-là le joueur ne peut plus lui préférer une autre unité (`def _fight_v11_activation_locked`, verrou partagé depuis le 2026-09-12 avec le pile-in additionnel de l'overrun 12.06) ; un seul dé par phase (`fight_exhortation_done`). Tant que le choix de cible est en attente, toute autre action est refusée, inerte (`mortal_wounds_target_pending`, `def _reject_action_while_exhortation_pending`) — `advance_phase` compris, qui aurait fermé la phase sur une décision posée. **Siège IA hors gym (Chaplain du bot en PvE, régime gym)** : aucune décision n'est posée — la cible est choisie sur-le-champ par le sélecteur `_ai_select_fight_target` (branche « siège sans canal de réponse » dans `_check_and_trigger_exhortation_de_rage`), comme les choix de faction du bot (`_resolve_faction_decisions_for_ai_seats`, heuristique non relevée) — la sélection 12.04 a déjà passé la main à l'humain, donc `execute_ai_turn` refuserait (`not_ai_player_turn`) et personne ne répondrait. Le régime est un `Literal` (`ExhortationRegime`) contrôlé par pyright aux sites qui passent une constante ; la seule valeur relue depuis `game_state` passe par `exhortation_regime_of` (T1 au point d'entrée).

**Hold Still and Say Aargh — le mot est « also »** (`Datasheets - Orks.pdf`, p4) : *« When this model scores a critical wound with its 'urty syringe against a non-VEHICLE unit, it ALSO inflicts D6 mortal wounds to that unit. »* Aucune clause « the attack sequence ends » n'existe dans le texte. La blessure critique n'est donc **pas** consommée : elle suit 05.03/05.04 comme n'importe quelle blessure, sauvegarde comprise — ce n'est pas [DEVASTATING WOUNDS] 24.10 — et les D6 s'y **ajoutent**. Corrigé le 2026-09-10 ; l'implémentation d'origine retirait le record de `pending_wounds`, perdait la blessure normale et faisait rendre `Save [NOT ALLOCATED]` au journal.

**Ordonnancement des blessures mortelles (06.02)** : *« if those attacks inflict a mixture of both mortal wounds and normal damage, resolve all of the normal damage first, then resolve all of the mortal wounds. »* Les blessures dues par une capacité sont donc portées par le lot (`pending_mortal_wounds`) et infligées à sa **fermeture**, par `def _apply_batch_mortal_wounds` (`engine/phase_handlers/shared_utils.py`) — jamais à l'instant du jet, sans quoi la cible peut mourir avant qu'une seule sauvegarde n'ait été lancée et les attaques restantes du lot sont perdues en « excess attacks ». **Qui choisit la figurine (06.02, « its controlling player … must select »)** : le défenseur humain, par un **lot mortel** manuel inséré après le lot courant (Hold Still) ou par `HAZARD_CTX` avec reprise du combat (Exhortation) ; le défenseur machine reste en AUTO (`allocate_mortal_wounds`). Cf. `Documentation/Reference/moteur/allocation_attaques.md`.

**Journal** : ces blessures empruntent la ligne générique `Unit N(c,r) SUFFERS X Mortal Wounds <tag> [Trigger:<D6>] [MW:<dés>] [FROM:<unité source>] [FNP_ROLLS: <mid>=<sauvés>/<seuil>+ ×<blessures> …]`, dont le dernier segment dit les jets Feel No Pain **par figurine** (grammaire 17 ; 24.12 attache le jet à la figurine, 06.02 en sélectionne une par blessure, donc une même ligne mélange les seuils), avec un tag propre par capacité (`HAZARD_CONTEXT_TAGS`, `shared/data_validation.py`). Le tag reste distinct de `[HAZARDOUS]` 24.15 : l'analyzer ne déclenche le contrôle d'armurerie que sur celui-là. `[FROM:]` est indispensable — contrairement à 24.15 et 09.07, ces blessures viennent d'un adversaire, et sans la source l'analyzer crédite la victime de ses propres morts. Trois segments de dés, trois sens : `Roll:` (24.15, jet de hasard comparé à 1-2), `Trigger:` (D6 de déclenchement d'Exhortation, comparé à 4+), `MW:` (dés qui se **somment** — un D6 par critique pour Hold Still, le D3 d'Exhortation sur 4-5 ; absent sur 6, où le compte vaut 3 sans dé). **Grammaire 9** (`LOG_GRAMMAR_VERSION`, `ai/step_logger.py`) : ces dés sont garantis, et l'analyzer contrôle le compte contre eux (`def mw_ability_dice_error`, `ai/analyzer_rules.py`, compteur `mw_ability_dice_mismatch` au camp de la source) au lieu de le croire.
