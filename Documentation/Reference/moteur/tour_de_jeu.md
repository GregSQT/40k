...
**L'étape Declare Battle Formations (20.01) PRÉCÈDE la mise en place**, et c'est une propriété de
la règle, pas un détail d'ordonnancement : « Before the battle, in the Declare Battle Formations
step, you can select one or more friendly units […] Instead of setting up these units on the
battlefield during deployment » (`20 Strategic reserves.pdf` ; `25 Rules appendix.pdf` situe
l'étape avant Pre-battle Abilities, elle-même avant Begin the Battle). Concrètement, la
déclaration se fait PAR CAMP (depuis le 2026-09-12 ; la file alternée de questions par escouade,
figée au reset, est retirée : 20.01 dit « select one or more friendly units », un ENSEMBLE sans
ordre) : le joueur 1 déclare pour toute son armée, puis le joueur 2 (`current_reserves_declarer`),
et aucun slot de pose ne s'ouvre tant qu'un camp déclare (`reserves_declaration_step_is_open`,
prédicat PARTAGÉ par le masque gym et par `deployment_commit_plan`, qui refuse par
`reserves_declaration_still_open`). L'état vit dans `deployment_state`, par joueur :
`reserves_declaration_declined` (escouades explicitement gardées pour la pose),
`reserves_declaration_validated` (déclaration FIGÉE), plus `reserves_declaration_closed` (étape
close, main rendue au premier déployeur — UNE fois) et `reserves_declaration_started` (verrou de
`change_roster`, jamais remis à faux). Sauvegarde TL09 (`services/game_saves.py`). Pendant
l'étape, `current_deployer` / `current_player` ne sont écrits que par
`move_seat_to_pending_reserves_declaration` : le siège suit le camp qui déclare, et ne bouge que
deux fois (joueur 1 → joueur 2 à la première validation, puis vers le premier déployeur à la
clôture, `close_reserves_declaration_step_if_done`).

Les deux sièges écrivent le même état final ; seule l'ergonomie d'entrée diffère :
- **Siège humain** : compose librement son ensemble — `deployment_place_in_strategic_reserves`
  réserve, `deployment_cancel_strategic_reserves` retire, dans n'importe quel ordre — puis le fige
  d'un clic (`deployment_validate_reserves_declaration` → `finalize_reserves_declaration`).
  Valider sans rien réserver est légal (« can select ») ; une déclaration figée ne se reprend pas.
  Le champ `declare` de l'ancienne question binaire n'existe plus : envoyé, il lève. La
  déclaration d'un camp appartient à SON siège (`_human_reserves_declarer`) — en PvE l'humain ne
  répond jamais pour le bot.
- **Siège piloté par le modèle** : PPO ne sait pas exprimer « je compose puis je valide », il reste
  donc interrogé escouade par escouade par le mécanisme générique « décision agent »
  (`arm_reserves_declaration_decision`, type `reserves_declaration`, `CHOICE_0` = mettre en
  réserves, `CHOICE_1` = garder pour le déploiement ; `next_reserves_declaration_question` = la
  première escouade déclarable du camp, dans l'ordre du pool de pose) : aucune dimension d'action
  n'est ajoutée et `obs_size` ne bouge pas. `apply_reserves_declaration_decision` inscrit un refus
  dans `reserves_declaration_declined` — sans quoi la question se reposerait indéfiniment — et fige
  le camp d'elle-même quand plus aucune escouade n'est interrogeable. Une unité que la règle refuse
  (FORTIFICATION, ou plafond de 50 % déjà atteint) n'est pas interrogée — un candidat unique n'est
  pas une décision. Les bots et le déploiement `auto` déclinent toujours
  (`reserves_declaration_decline_slot`) : 20.01 est une décision de LISTE, jamais de doctrine.

⚠️ Ce qui a été corrigé : `SQUAD_ACTION_WAIT` portait la mise en réserves DANS le tour de
déploiement de chaque unité. Mesuré sur `scenario_training_armageddon1.json` en déploiement
actif, le joueur 2 gardait le slot ouvert avec quatre unités adverses déjà posées — il déclarait
donc en voyant le déploiement adverse, et la politique apprenait une décision injouable en partie
légale. `SQUAD_ACTION_WAIT` n'a plus aucun sens en phase de déploiement et le masque ne l'ouvre
plus. Verrou : `tests/unit/engine/test_reserves_declaration_step_2001.py`.

En mode `deployment_type: "active"`, TOUTES les unités commencent hors table et sont posées une à
une : voir « Unités hors table » dans la section Mouvement pour l'invariant géométrique que cela
impose.
...
Hors décision en attente, la phase n'accepte que `skip`
(`W40KEngine.COMMAND_PHASE_ACTIONS`), le masque n'y ouvre que `wait`, et la reprise enchaîne
directement sur le mouvement pour les DEUX camps. Elle acceptait aussi `zone_intent` et gardait
la main sur le siège de l'agent pour ses cinq « free steps » d'intention par tour : cette famille
d'actions a été retirée le 2026-09-09, faute du moindre effet sur la partie.
...
- **Zone d'engagement ennemie** : **traversable, jamais en destination**. 03.01 n'interdit que
  la traversée des figurines ennemies et 09.05/09.06 n'exigent « unengaged » qu'APRÈS le
  mouvement ; la bande d'EZ n'est un obstacle de transit que si `config.move.
  can_move_through_enemy_engagement_zone` vaut `false` (`_get_move_traversal_rules`, lu par le
  pool d'ancre, le pool par-figurine et `build_move_transit_blocked`). En production la clé vaut
  `true` : une figurine sans **Fly** franchit la bande pour atteindre les cases derrière, sans
  pouvoir s'y arrêter. Verrou : `test_move.py::TestNormalMove0905::
  test_traverse_la_zone_d_engagement_sans_y_finir`.
...
| 09.07 | Fall-back (engaged ; Ordered Retreat / Desperate Escape ; hazard ; battle-shock ; après : pas tir/charge/action) | ✅ (mode sélectionnable 2026-09-17) | **Le mode est un CHOIX du joueur actif** (encart SELECTING MODES : « ordered retreat is not mandatory, so you could select desperate escape instead »), pour les DEUX sièges : humain par le bouton `Desp. Escape` (même popup et même action `hazard_confirm` que le mode imposé), modèle par le point de choix `fall_back_mode` (`arm_fall_back_mode_decision`, `CHOICE_0` = Desperate Escape / `CHOICE_1` = Ordered Retreat, posé AVANT le pool comme la déclaration de vol ; les bots gardent Ordered Retreat, `bot_action_for_pending_choice`). Battle-shocked + engagée → mode IMPOSÉ, sans question (`desperate_escape_pre_move`) ; tant que `hazard_confirm` ne l'a pas résolu (`fall_back_hazard_pending`, un prédicat pour l'activation, le preview et le commit du plan par-figurine), le preview n'est pas validable et `commit_move_plan` est refusé (`hazard_required`) — 09.07 BEFORE MOVING, corrigé le 2026-09-17 : le plan passait sans jet. Le mode retenu vit dans `FALL_BACK_MODES_KEY` (dict `{escouade: desperate_escape | ordered_retreat}`, écrit par `select_desperate_escape_mode` / `select_ordered_retreat_mode`, libéré par `release_fall_back_mode`), lu par les pools (prédicat gardé par la phase `desperate_escape_transit_exempt` :  (traversée des figurines ennemies ET de la bande d'EZ — pool d'ancre, `build_move_traversal_blocked`, `build_move_transit_blocked`), par les fingerprints des caches spatiaux et purgé par `end_activation`. Une escouade engagée saine au pool vide n'est plus close d'office (`no_valid_move_destinations`) : Desperate Escape peut lui ouvrir une issue. AFTER MOVING : `desperate_escape_post_move` (jet 01.07 si non battle-shocked) après chaque commit de fall-back — gym, plan par-figurine, commit rapide —, journalisé dans la phase EN COURS (`roll_battle_shock` n'écrit plus `command` en dur). `roll_hazard_for_unit`, `units_fled`. Tests `test_fall_back_modes.py` |
...
| 20.01 | Placing units in strategic reserves (plafond 50 % des points) | ✅ (2026-08-04 ; par camp 2026-09-12) | Deux entrées, un seul état : champ de roster `strategic_reserves` (validé AU CHARGEMENT par `validate_strategic_reserves_cap` — dépassement = erreur nommant les unités et le total, jamais une troncature) et DÉCISION prise à l'étape Declare Battle Formations, qui PRÉCÈDE toute mise en place — PAR CAMP, joueur 1 puis joueur 2 (`current_reserves_declarer`) : le siège humain compose (réserver / annuler) puis fige d'un clic (`deployment_validate_reserves_declaration`), le siège du modèle est interrogé escouade par escouade (type de décision `reserves_declaration` répondu par `CHOICE_i`, unité non déclarable jamais interrogée) ; état `reserves_declaration_declined` / `reserves_declaration_validated` par joueur, sauvegarde TL09 (voir la section Déploiement). `SQUAD_ACTION_WAIT` ne porte plus rien en phase de déploiement : il y faisait déclarer APRÈS avoir vu les poses adverses, ce que 20.01 interdit (mesuré : joueur 2, slot ouvert avec 4 unités adverses posées). `FORTIFICATIONS` exclues. Taille de bataille lue dans le `scale` du scénario (`points_limit`) ; sans elle le plafond vaut 0 et AUCUN dépôt n'est possible — les scénarios PvP ne la déclaraient pas, donc le conteneur était inerte quoi que fasse le joueur (verrouillé par `test_every_pvp_scenario_declares_a_battle_size`, dans `tests/unit/services/test_api_server_helpers.py`). **Siège PvP, deux gestes séparés** : (1) la DÉCLARATION : un bandeau (`ReservesDeclarationBanner`, bouton `Validate` TOUJOURS actif — déclarer zéro réserve est légal) nomme le camp qui déclare, LU de l'API (`strategic_reserves.declaring_player`, `null` une fois l'étape close). Cliquer une ligne de sa liste à déployer la SÉLECTIONNE sans rien poser et, si le moteur la publie dans `strategic_reserves.declarable`, y fait apparaître `Reserve` (`ReserveButton`, pas de `Deploy` en face : ne pas réserver, c'est déployer) ; une escouade réservée porte `Cancel` (`CancelReserveButton`) dans le conteneur tant qu'elle figure dans `strategic_reserves.cancellable`, donc jamais après `Validate`. Ces listes sont RENDUES par le moteur (`reserves_declarable_squads` / `reserves_cancellable_squads`), jamais recalculées en TS : le plafond bouge à chaque geste. Tant qu'un camp déclare, la liste à déployer de l'autre camp est inerte et aucune pose n'est acceptée (`reserves_declaration_still_open`) — la sélection libre ne rouvre donc pas la déclaration après les poses adverses ; (2) le CONTENEUR (`StrategicReservesContainer`, contour orange, ratio « 120/250 » LU de l'API) est rendu SOUS la table de statut de chaque joueur, et ne sert qu'à voir les escouades hors table et à déclencher leur arrivée 20.04. Il s'efface quand il n'a rien à dire (aucune réserve hors phase de déploiement) : pendant le déploiement il reste, même vide, car son ratio est la seule lecture du plafond restant au moment de la décision. Les deux listes partagent UNE ligne (`UnitRosterRow` : figurines, nom, [action], points, nb de figurines, id) — même escouade, même tête, qu'elle attende son déploiement ou son arrivée. Le plafond et le test FORTIFICATION restent côté moteur (`unit_can_be_placed_in_strategic_reserves`) : ils décident s'il y a une question à poser, le client ne les rejoue pas |
...
| 03.01 | Moving (traverse alliés, pas ennemis, pas hors plateau) | ✅ | config `move.can_move_through_friendly_model` / `can_move_through_enemy_model` / `can_move_through_enemy_engagement_zone` (`true` : la bande d'EZ se traverse, 03.01 ne bloque que les figurines ennemies ; la destination reste hors EZ par 09.05 « unengaged » après le move) ; bord = bounds plateau. Tests `test_move.py::TestNormalMove0905` (traversée amie, refus ennemie, traversée de la bande d'EZ sans y finir) |
...
- **Pathfinding** : même BFS que la phase de mouvement, mêmes restrictions (voir « Restrictions
  de mouvement » : murs et figurines ennemies bloquent le chemin, la bande d'EZ se traverse et
  n'est interdite qu'en destination).
- **Marquage** : l'unité entre dans `units_advanced` et son jet dans `advance_rolls` AU CLIC
  (`movement_set_advance_mode_handler`), avant tout déplacement ; ni un report (`right_click`)
  ni une activation close sans bouger (`wait`) ne l'en retirent — jusqu'à la phase de
  commandement suivante, seul un Fall Back (09.07, `commit_move`) purge ces deux marques.
- **Irréversibilité** : cliquer le logo advance est un POINT OF NO RETURN — l'engagement précède
  la connaissance de la distance ; seul un tir ASSAULT reste ensuite possible. Une escouade qui
  déclare Advance puis reste sur place a fait un Advance (09.02 : le type de move est choisi et
  résolu ; 03.02 ne « désélectionne » une unité que si sa mise en place est impossible) : pas de
  charge, tir ASSAULT seulement. Verrou : `test_move.py::TestAdvance0906::
  test_advance_puis_stationnaire_reste_un_advance`.
...
| 11.04 | Charge move (max = charge roll ; cibles à 12″ et ≤ max ; fin engaged avec TOUTES les cibles, PAS engaged avec des non-cibles ; Fights First jusqu'à fin du tour 24.13) | 🟡 | **WHILE MOVING « within 1" … must do so » / « engaged … must do so » — gym livré le 2026-09-18 (A2, lot melee-100)** : `charge_build_valid_plan` classe chaque case candidate par PALIER (0 contact `base_contact_zone`, 1 ≤ 1", 2 engagée, mémo `charge_engage_memo` = palier) et ne laisse l'intention L10 départager qu'à l'intérieur du palier le plus serré atteignable ; figurines les plus proches d'une cible placées en premier (une figurine du fond ne prend plus la seule case de contact d'une figurine de front). Même critère que le pool PvP `_charge_pool_must_socle_a_socle_if_possible`. Mesuré bot contre bot (40 parties) : contact après charge 23 % → 52 %. Analyzer #70 `charge_no_contact`. budget/cibles OK **depuis le 2026-08-08** : la borne « cible ≤ distance maximale » (BEFORE MOVING) n'existait dans AUCUN des deux chemins — seul le TRAJET était borné, donc toute cible à `jet + engagement range` était chargeable (portée doublée ; un jet de 2 réussissait, contre l'encart FAILED CHARGES). Borne unique `charge_target_within_max_distance` (`shared_utils`), lue par les QUATRE points de décision : `charge_build_valid_plan` (gym), `charge_build_valid_targets` (offre PvP), `charge_target_selection_handler` (déclaration) et `charge_preview_move_plan` (Check + commit + toggle vol — sans elle, une cible déclarée avant un Take to the skies restait committable hors de portée). Mesure bord-à-bord par `ranged_in_range`, la primitive de PORTÉE : la primitive d'ER y aurait ajouté le gate vertical 03.04 (5″), qui n'a rien à faire dans une portée de déclaration. C'est ce qui rend exact le « 2 n'est jamais suffisant » du livre. Le -2″ de 21.03 borne aussi la déclaration (« subtract 2" from the maximum distance »). Fights First → `units_charged` (lu par l'ordre Fight) ; contrainte « pas engaged avec une non-cible » → ⚠️ à vérifier |
...
| 12.03 | Pile-in move (3″ ; éligible si engaged / a chargé / overrun ; fin engaged ; modèles en base-contact non déplaçables) | 🟡 | 3″ via `3 * scale` ; base-contact = adjacence hex ; branche overrun ✅ depuis la refonte 12.04 (`fight_v11_can_overrun_pile_in`, plan `fight_pile_in_plan` partagé avec 12.02 — 2026-09-12) ; WHILE « each model that is moved must end its move closer to the closest pile-in target » : PvP (`_fight_pile_in_build_model_pool`, `start_min`) ET gym (`_assign_cells_toward_enemies`, filtre `admissible` par-figurine — 2026-09-12) ; WHILE « and engaged with it if possible » : PvP par figurine (`_fight_model_legal_destinations` — une case engagée avec le palier, cohérente et conservant les engagements de départ de la figurine, existe ⇒ seule une telle case est légale ; `per_model` des preview_plan, pool exposé au front, repli ET arêtes ILP de `pile_in_autoplace_plan` — une figurine qui a une option engagée-avec-le-palier ne reçoit que des slots engageant aussi le palier, sinon un Focus hors palier produisait un plan refusé — 2026-09-12) ; gym **livré le 2026-09-18 (A1, lot melee-100), palier contact retiré le 2026-09-19** : `_assign_cells_toward_enemies` en DEUX paliers — case ENGAGÉE avec la cible la plus proche dans tout le budget (primitive d'engagement, `target_ids` passés par `fight_pile_in_plan` / `squad_consolidate_plan_with_targets`), sinon case la plus proche dans tout le budget (avant : premier anneau qui rapproche) ; légalité de l'EMPREINTE entière des candidats, et OCCUPATION suivie par empreinte, jamais par centre (03 ENDING A MOVE « No models in that unit are on another model » : à x5 deux socles round/6 se chevauchent sans partager leur centre). Le palier contact couplait les figurines aux VOISINS DU CENTRE ennemi, six cases toutes dans l'empreinte de l'ennemi à x5 — jamais applicable — et maximisait le CONTACT quand l'encart 12 demande de maximiser les figurines ENGAGÉES. Ordre de service : la plus contrainte d'abord (nombre de cellules engageantes), puis une passe de REPRISE pour celles qu'une camarade encore à sa position de départ bloquait. Analyzer #72 `fight_pile_in_no_engage` ; AFTER « each model that started this move engaged […] must still be engaged with that enemy unit » : PvP (`_fight_pile_in_preview_plan`, `kept_engagements` ; autoplace : arêtes ILP et les DEUX branches du repli de `pile_in_autoplace_plan` filtrées par `_keeps_start_engagements`, sinon la figurine reste sur place — 2026-09-12) ET gym (`_assign_cells_toward_enemies`, filtre par-figurine des candidats bord-à-bord et de repli — 2026-09-12) |
| 12.04 | Fight : alterné, **Fights First** puis **Remaining** ; éligible si engaged ou a chargé | ✅ (re-vérifié 2026-08-27 ; passe PDF 25 livrée 2026-09-18) | **PDF 25 « Eligible to fight, but unable to fight » (A5, lot melee-100)** : quand TOUTES les unités éligibles du sélecteur — celles du pool ET celles de l'autre étape (une Remaining engagée interdit la passe pendant Fights First ; review 2026-09-18) — sont à plus de 5" (`pile_in_target_range`) de tout ennemi (`fight_v11_can_pass`), le slot sans cible est la PASSE (`squad_fight_pass`, aucune unité marquée, main à l'adversaire jusqu'à SA sélection : `fight_pass_handoff`, honoré par `fight_v11_advance_selection` / `fight_v11_fight_selection_pool` — l'adversaire sans unité Fights First sélectionne dans Remaining, jamais de retour au passeur sans sélection intermédiaire ; `fight_pass_streak` remis à 0 par toute sélection) — deux passes de suite, ou une passe face à un adversaire sans unité éligible, ferment l'étape (`fight_step_passed`, lu par `fight_v11_advance_selection` et `fight_v11_fight_selection_pool`) ; les unités passées consolident comme « eligible to fight this phase » (`units_eligible_when_passed`) ; à ≤ 5" sans cible atteignable, le combat à vide reste obligatoire. Siège humain : action `fight_pass` + `can_pass`/`fight_can_pass` dans l'état d'attente (`fight_can_pass` remis à False à `fight_v11_start` et `fight_v11_enter_consolidate` : la passe n'existe qu'en étape FIGHT), bouton « Passer » (`TurnPhaseTracker`). Ligne `PASSED FIGHT` (step.log ; analyzer `fight_pass`, sans alternance). Récompense 0 (+ marge VP). machine `fight_step` ∈ {fights_first, remaining} + `fight_selector`, initialisée par `fight_v11_enter_fight_step` avec **sélecteur = joueur actif** (conforme « the player whose turn it is ») ; alternance et handoff par `fight_v11_advance_selection` ; FF = `units_charged` OU ability (`is_fights_first`) ; retour FF pendant Remaining implémenté (inatteignable tant que FF = charge seule) |
| 12.05 | Normal fight (engaged) | ✅ | Résolution 04 côté mêlée : ligne ci-dessous |
| 04.01 / 04.02 / 24.11 (mêlée) | Select weapons : UNE arme de mêlée par figurine + TOUTES ses [EXTRA ATTACKS] ; select targets : chaque cible engagée avec LA figurine qui porte l'arme | ✅ **gym / bot PvE livré le 2026-09-18 (D+, lot melee-100)** | `_fight_resolve_with_target` → `squad_auto_declare_fight_weapons` (`shared_utils`) : pour chaque figurine engagée avec la cible désignée, arme ordinaire unique déclarée d'office + toutes les [EXTRA ATTACKS] ; ≥ 2 armes ordinaires → question d'arme (`PENDING_FIGHT_WEAPON_KEY` + `model_ids`, `fight_weapon_eligible_slots` restreint aux figurines non déclarées et aux armes ordinaires), reposée tant qu'il reste des figurines interrogées sans arme — la réponse vaut pour toutes les porteuses du profil interrogées (restriction documentée) ; figurines engagées SEULEMENT avec d'autres ennemis (`_fight_continue_declarations`) : déclarées d'office si l'ennemi commun est unique, sinon question de cible restreinte (`PENDING_FIGHT_TARGET_KEY` + `model_ids`, reprise sans repartir de zéro) ; répartition d'une arme entre deux unités non exposée. Avant : une seule ligne `squad_declare_fight_weapon_qty` sur le code choisi (53 % des figurines engagées frappaient, mesuré bot contre bot). Aucun modèle des rosters Armageddon ne porte deux armes ordinaires : la question d'arme n'y est jamais posée. Machine manuelle inchangée (`squad_declare_fight`, clic-cible). Analyzer #71 `fight_engaged_idle` ; courbe `06_fight/e_engaged_idle_models` (doit valoir 0) |
| 12.06 | Overrun fight (unengaged devenu engaged → pile-in additionnel) | ✅ (re-vérifié 2026-09-12 ; B3 2026-09-18) | **B3 (lot melee-100)** : pour une unité NON engagée, le pile-in additionnel vise la cible DÉSIGNÉE par l'agent (12.03 BEFORE MOVING « select one or more enemy units within 5" » est un choix du joueur — `overrun_pile_in_plan_for_slot`, `fight_pile_in_plan(target_ids=…)` validé par `pile_in_select_targets_12_03`) ; repli sur toutes les cibles à 5" si la désignée n'y est pas ; unité engagée : cibles imposées. `fight_v11_is_overrun_eligible` (« was unengaged at the start of the Fight step » = négation du snapshot). AUTO/gym (2026-09-12) : pile-in additionnel automatique dans `_continue_squad_fight_after_selection`, condition `fight_v11_can_overrun_pile_in` (les DEUX cas 12.06 — non engagée, OU engagée depuis le snapshot p. ex. par le pile-in overrun d'un ennemi — et garde `overrun_pile_in_done`, alimentée après le commit), plan `fight_pile_in_plan` (cibles 12.03 : engagée → engagées, sinon ≤5") ; **parité masque/commit livrée le 2026-09-18** : masque et commit appellent le MÊME oracle `overrun_pile_in_plan_for_slot` (shared_utils), qui rend le plan du slot joué — cible désignée si elle est à ≤5" et que son plan aboutit, sinon plan groupé « toutes les cibles à 5" » (12.03 « select one **or more** enemy units within 5" ») — et le masque ouvre un slot ssi le plan de CE slot finit engagé avec sa cible. Avant : le masque projetait le seul plan groupé pour tous les slots, donc n'ouvrait que la cible la plus proche pendant que le commit frappait la cible désignée ; le repli sur le plan groupé est ce qui garantit qu'une action reste jouable (sans lui, un état ferme tous les slots ET le combat à vide). Avant : `not _fight_v11_engaged_now` + `_fight_overrun_pile_in_plan` (cibles ≤5" fixes, supprimé) — l'unité engagée par l'adversaire restait où il l'avait mise. PvP humain (2026-09-12) : action `overrun_pile_in` sur l'unité active (bouton « Overrun », `fight_v11_can_overrun_pile_in` = éligible et pas déjà fait), AVANT le choix de cible → même plan par-figurine que le pile-in 12.02 (`_fight_v11_pile_in_model_plan_step`, cibles `_fight_v11_pile_in_targets` : engagées si engagée, sinon ≤5" ; AFTER : l'unité finit engagée) ; le commit (log `overrun_pile_in`) rend la main au combat de l'unité, cibles recalculées, et la VERROUILLE (`_fight_v11_activation_locked`, même verrou que l'Exhortation : « one additional pile-in move, THEN fights », le joueur ne peut plus lui préférer une autre unité avant qu'elle ait combattu) ; `skip` sous plan = renoncer au move sans passer l'unité. Même flux sur les New Foes to Face (12.08 AFTER, sous-phase consolidate) : un New Foe dont l'engageur est mort avant sa sélection est désengagé → `overrun_eligible` sur l'état New Foes, action `overrun_pile_in` via la source unique `_fight_v11_overrun_step` (bouton « Overrun » de la barre New Foes, attaque ensuite par clic-cible direct) |
| 12.07 | Consolidate (les deux joueurs) | ✅ | étape groupée `consolidate` (`fight_v11_enter_consolidate`). **« with all of their eligible units they CHOOSE to move » — B2 livré le 2026-09-18 (lot melee-100)** : la consolidation ENGAGING est une décision de l'agent (`consolidation_engaging`, 15e type de `AGENT_DECISION_TYPE_IDS`, `CHOICE_0` consolider / `CHOICE_1` rester, `arm_consolidation_engaging_decision` armée par `_fight_v11_gym_settle` APRÈS le plan et SEULEMENT s'il existe — 12.07 « they CHOOSE to move » suppose un mouvement possible, et sans plan les deux réponses produisent le même état (2026-09-18) ; le plan n'est pas mémorisé avec la décision, il est recalculé à la reprise, réponse mémorisée par phase dans `consolidation_engaging_answers`, appliquée par `apply_consolidation_engaging_decision` puis reprise du settle) ; ongoing et objective restent automatiques ; bot de référence : `CHOICE_0` (ancien comportement, baseline inchangée, `bot_action_for_pending_choice`) ; siège humain inchangé (machine manuelle). Courbes `06_fight/b_engaging_consolidations_*`, `c_new_foes_subies` |
| 12.08 | Consolidation move (3″ ; 3 modes : Ongoing / Engaging / Objective ; Engaging peut tirer de nouvelles unités au combat) | ✅ (re-vérifié 2026-09-12 : clause « if possible » livrée côté PvP) | cascade `fight_v11_consolidation_mode` : `ongoing` (engagée) → `engaging` (ennemi dans `consolidation_trigger_range`, 3″) → `objective` (objectif dans 3″) ; AFTER Engaging « New Foes to Face » : ennemis engagés non sélectionnés gelés par `fight_v11_consolidation_freeze_new_foes` (sélecteur = adversaire du propriétaire de l'unité), combattus in-place sur pool restreint (`fight_v11_fight_selection_pool`, source du masque FIGHT et du commit `squad_fight`) sur les deux chemins — manuel (`_fight_v11_consolidation_new_foes_step`) et programmatique (`_fight_v11_gym_settle` rend la main au siège adverse ; bot PvE et politique gym, cf. bloc « Qui conduit la machine — par SIÈGE ») — 2026-09-12 ; AFTER Ongoing (engagements de départ conservés par figurine) : même filtre gym que 12.03 (`_assign_cells_toward_enemies`, 2026-09-12) ; WHILE « engaged with it if possible » (Ongoing/Engaging) et « within range of the selected objective if possible, or closer to it if not » (Objective) : PvP par figurine (`_fight_model_legal_destinations`, même source que 12.03 — `per_model` de `_fight_consolidation_preview_plan`, pool exposé au front — 2026-09-12) ; gym **livré le 2026-09-18 (A3 / A4, lot melee-100)** : `squad_consolidate_plan_with_targets` — Engaging : la sélection est le sous-ensemble des ennemis à 3" que le plan engage RÉELLEMENT (point fixe : plan vers S, E = engagés, replanification vers E tant que E ⊊ S ; aucun engagé = pas de consolidation), journalisée `[targets: …]` et reflétée par `fight_v11_consolidation_freeze_new_foes` ; Objective : « or closer to it if not » — une figurine qui n'atteint pas la zone finit sur la case atteignable la plus proche de la zone (avant : sur place) ; Ongoing/Engaging : mêmes deux paliers que 12.03 (`_assign_cells_toward_enemies`, palier contact retiré le 2026-09-19). Analyzer #73 `fight_consolidation_not_all_selected` |
...
- **Jalons datés portés par ce document** : 2026-07-26 (V11 T-B, type de tir squad ; audit
  ...
  re-vérifiés contre `w40k_core` / `command_handlers` / `fight_handlers`) ; 2026-09-12 (20.01
  déclaration PAR CAMP au lieu d'une file par escouade, sauvegarde TL09).
- **Anciens noms retirés du code** (rencontrés dans de vieux commentaires ou commits) :
  ...
  `fight_subphase` / `fight_step` / `fight_selector` ; file 20.01 `build_reserves_declaration_queue`
  / `reserves_declaration_queue` / `strategic_reserves.pending_declaration` /
  `ReservesDeclarationPrompt` → déclaration par camp (`current_reserves_declarer`,
  `reserves_declaration_declined` / `reserves_declaration_validated`,
  `strategic_reserves.declaring_player` / `declarable` / `cancellable`, `ReservesDeclarationBanner`).
