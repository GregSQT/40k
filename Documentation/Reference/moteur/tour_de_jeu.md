...
l'étape avant Pre-battle Abilities, elle-même avant Begin the Battle). Concrètement, au `reset` le
moteur fige une FILE alternée de questions (`build_reserves_declaration_queue`, une entrée par
unité à poser, joueur 1 d'abord) et n'ouvre aucun slot de pose tant qu'elle n'est pas épuisée :
`deployment_commit_plan` refuse par `reserves_declaration_still_open`.

Chaque question est un point d'arrêt joueur du mécanisme générique « décision agent »
(type `reserves_declaration`, `CHOICE_0` = mettre en réserves, `CHOICE_1` = garder pour le
déploiement) : aucune dimension d'action n'est ajoutée et `obs_size` ne bouge pas. Une unité que
la règle refuse (FORTIFICATION, ou plafond de 50 % déjà atteint) n'est pas interrogée — un
candidat unique n'est pas une décision. Le siège humain répond par la même file
(`deployment_place_in_strategic_reserves`, champ `declare` obligatoire) ; les bots et le
déploiement `auto` déclinent toujours (`reserves_declaration_decline_slot`) : 20.01 est une
décision de LISTE, jamais de doctrine.
...
| 20.01 | Placing units in strategic reserves (plafond 50 % des points) | ✅ (2026-08-04) | Deux entrées, un seul état : champ de roster `strategic_reserves` (validé AU CHARGEMENT par `validate_strategic_reserves_cap` — dépassement = erreur nommant les unités et le total, jamais une troncature) et DÉCISION prise à l'étape Declare Battle Formations, qui PRÉCÈDE toute mise en place (file figée au reset, type de décision `reserves_declaration` répondu par `CHOICE_i`, unité non déclarable jamais interrogée). `SQUAD_ACTION_WAIT` ne porte plus rien en phase de déploiement : il y faisait déclarer APRÈS avoir vu les poses adverses, ce que 20.01 interdit (mesuré : joueur 2, slot ouvert avec 4 unités adverses posées). `FORTIFICATIONS` exclues. Taille de bataille lue dans le `scale` du scénario (`points_limit`) ; sans elle le plafond vaut 0 et AUCUN dépôt n'est possible — les scénarios PvP ne la déclaraient pas, donc le conteneur était inerte quoi que fasse le joueur (verrouillé par `test_every_pvp_scenario_declares_a_battle_size`, dans `tests/unit/services/test_api_server_helpers.py`). **Siège PvP, deux gestes séparés** : (1) la DÉCLARATION est une question FERMÉE (`ReservesDeclarationPrompt`, boutons `Reserve` / `Deploy`, tous deux actifs) portée par la ligne de l'escouade que le moteur INTERROGE — publiée par `strategic_reserves.pending_declaration`, jamais choisie par le client : une sélection libre rouvrirait la déclaration après les poses adverses. Tant qu'une question est en attente, la liste à déployer est inerte pour les DEUX joueurs ; (2) le CONTENEUR (`StrategicReservesContainer`, contour orange, ratio « 120/250 » LU de l'API) est rendu SOUS la table de statut de chaque joueur, et ne sert qu'à voir les escouades hors table et à déclencher leur arrivée 20.04. Il s'efface quand il n'a rien à dire (aucune réserve hors phase de déploiement) : pendant le déploiement il reste, même vide, car son ratio est la seule lecture du plafond restant au moment de la décision. Les deux listes partagent UNE ligne (`UnitRosterRow` : figurines, nom, [action], points, nb de figurines, id) — même escouade, même tête, qu'elle attende son déploiement ou son arrivée. Le plafond et le test FORTIFICATION restent côté moteur (`unit_can_be_placed_in_strategic_reserves`) : ils décident s'il y a une question à poser, le client ne les rejoue pas |
