# Verticalité — étages, niveaux, coûts de montée/descente

> **Objet** : référence de conception du moteur multi-niveaux (étages de ruines) — règles 40K applicables, modèle `level`/`floors`, occupation, mouvement vertical, engagement 3D, mêlée 3D, LoS 3D et rendu.
> **Sources absorbées** : `stage.md` (ce document en est le renommage ; l'original part dans `Documentation/Archives/docs/` avec un bandeau retour).
> Les identifiants historiques « chantier N » / « §N.N » sont conservés à l'identique — le code les cite (voir « Correspondance des sources » en fin de document).
> Les ancres fichier:ligne de l'original ont été converties en symboles vérifiés ; les corrections datées sont signalées par des blocs **[Mise à jour 2026-08-28]**.
> **L'état des chantiers fait foi dans `Documentation/Roadmap/`, jamais ici** — les marqueurs ✅/⏳/⏸️ ci-dessous datent de la rédaction d'origine.

---

## 1. Objectif

Le moteur était, à l'ouverture de ce chantier, **strictement 2D** (grille hexagonale odd-q, position = `col`/`row`) ;
ce document décrit l'ajout de l'axe vertical (le champ `level` existe depuis, cf. chantier 1).
...
## 3. État du code à l'ouverture du chantier (fondations existantes)

> **[Mise à jour 2026-08-28]** Ce §3 est le **constat d'ouverture** (baseline 2D, avant chantier 1).
> Le champ `level`, l'engagement 3D et le plancher-occulteur LoS existent depuis — voir les chantiers
> 1→6 ci-dessous et l'état courant dans la Roadmap. Les briques listées ici restent les fondations
> réellement réutilisées par la feature.

### 3.1 Modèle d'état — 2D strict, **aucune** verticalité (constat d'origine)
- Position unité = entiers `col`/`row` (odd-q), posés dans `create_unit` (`UnitFactory`)
  et `_build_enhanced_unit` ([game_state.py](../../../engine/game_state.py)).
- Source de vérité runtime = `units_cache`, construit par `build_units_cache`
  ...
  occupied_hexes:Set[(col,row)], occupied_hexes_by_model:{model_id:(col,row)}}`.
- `models_cache` par figurine, `occupation_map : {(col,row) → unit_id}` (mêmes builders).
- **Recherche exhaustive** `z_level|elevation|storey|floor|altitude|height_level` sur `engine/` : **zéro** résultat
  à l'époque. Les `z_` existants = coordonnées **cube 2D** pour distances hex (pas d'altitude). `floor` = `math.floor`.
...
- `compute_occupied_hexes(center_col, center_row, base_shape, base_size, orientation)`
  ([hex_utils.py](../../../engine/hex_utils.py)) : dispatch round/oval/square,
  rotation par `orientation × 60°`.
- Champs socle sur l'unité : `BASE_SHAPE`, `BASE_SIZE`, `orientation` ([game_state.py](../../../engine/game_state.py)).
- Point d'entrée moteur `_compute_unit_occupied_hexes` ([shared_utils.py](../../../engine/phase_handlers/shared_utils.py)) —
  empreinte multi-hex calculée **seulement** si `engagement_zone > 1` (valeur déjà scalée
  ×`inches_to_subhex` au chargement → board ×10 en pratique) **et** `base_size > 1` ; sinon 1 unité = 1 cellule.
- `occupied_hexes` unité = union des footprints des figs vivantes (`build_units_cache`) ;
...
- Point d'entrée `has_line_of_sight` ([combat_utils.py](../../../engine/combat_utils.py)) →
  source unique `compute_unit_los` ([shooting_handlers.py](../../../engine/phase_handlers/shooting_handlers.py)),
  cœur `_compute_unit_los_uncached`.
...
- Cache 2D sans notion de niveau : `hex_los_cache` ([combat_utils.py](../../../engine/combat_utils.py)),
  clé `((col,row),(col,row))` — toujours vraie à ce jour, cf. chantier 5.
  *(L'original citait aussi un cache de distance de pathfinding : `calculate_pathfinding_distance` et
  son champ BFS ont été **supprimés le 2026-07-28**, faute d'appelant — docstring de
  `calculate_hex_distance`, combat_utils.)*
- Murs = `game_state["wall_hexes"]` (hexes 2D JSON, consommés par deployment_handlers/combat_utils) ;
  ...
  ([terrain_utils.py](../../../engine/terrain_utils.py)) — **aucun champ hauteur/étage** à l'époque.

### 3.4 Mouvement — champ géodésique any-angle, terrain binaire
- Budget `get_squad_move_budget` ([shared_utils.py](../../../engine/phase_handlers/shared_utils.py)) :
  normal/fall_back = `MOVE` ; advance = `MOVE + roll×échelle` ; charge = `2D6×échelle` ;
  pile_in/conso = `3×échelle`. Malus "take to the skies" −2" déjà présent (même fonction).
- Hexes atteignables = `geodesic_field` ([hex_utils.py](../../../engine/hex_utils.py),
  Lazy Theta*, clearance = rayon socle) via `_euclidean_move_field` ([geodesic_move.py](../../../engine/phase_handlers/geodesic_move.py)).
  Charge réutilise **exactement** ce champ.
- **Aucun coût de terrain** (pas de difficult terrain) : terrain **binaire** obstacle/libre.
  Toggles de traversée `_get_move_traversal_rules` ([movement_handlers.py](../../../engine/phase_handlers/movement_handlers.py)),
  config `game_config.json`. Murs bloquent toujours ; FLY = disque euclidien ignorant murs+figs
  (branche FLY de `movement_build_valid_destinations_pool`).
...
- Rendu statique dans `drawBoard` ([BoardDisplay.tsx](../../../frontend/src/components/BoardDisplay.tsx)),
  overlays + UI dans [BoardPvp.tsx](../../../frontend/src/components/BoardPvp.tsx), figs dans [UnitRenderer.tsx](../../../frontend/src/components/UnitRenderer.tsx).
- **Zone au sol type terrain** (modèle à copier pour l'empreinte d'étage) : bloc terrain de `drawBoard`
  ([BoardDisplay.tsx](../../../frontend/src/components/BoardDisplay.tsx)),
  tracé polygone = `beginFill(zoneColor, fillAlpha)` (0.1, ou 0.18 si obscuring)
  + `lineStyle(Math.max(1.5, HEX_RADIUS*0.3), color, 0.85)`, ajouté à `baseHexContainer`. Type `ObjectiveZone` ([useGameConfig.ts](../../../frontend/src/hooks/useGameConfig.ts)).
- **Changement de couleur selon état** (blanc si occupé) : reprendre le motif `objectiveControl` + ternaire
  (dans `drawBoard`). Chaque `lineStyle(w,color,alpha)` démarre un tracé de couleur.
- **Mode ghost existant** (chaîne complète dans le renderer) : flag `modelGhost?: boolean[]`
  ([UnitRenderer.tsx](../../../frontend/src/components/UnitRenderer.tsx)),
  socle `circleAlpha=0.45`, sprite `alpha=0.42`, texte `0.65`.
  ⚠️ Seul producteur d'origine : la **fig active en cours de déplacement** (BoardPvp.tsx), avec
...
- **Bouton loupe zoom** : `<button>` [BoardPvp.tsx](../../../frontend/src/components/BoardPvp.tsx).
  Deux divs imbriquées : la parente (`position:absolute, top:8, right:8, zIndex:1600`, colonne) et la
  flex row (conteneur direct du bouton). Frère horizontal → la row ; empilé → la colonne.
...
- Ajouter un champ `level` (int, défaut 0) sur : dict unité et `models[]`
  ...
  puis le propager dans `units_cache` / `models_cache` (`build_units_cache`,
  [shared_utils.py](../../../engine/phase_handlers/shared_utils.py)),
  et jusqu'au **frontend** (payload API + types TS Unit) — à l'époque aucune notion de niveau côté front.
...
- **Définition de l'empreinte d'étage — SOURCE = fichier terrain du board**
  ...
  ruines (empreinte au sol = `vertices` en subhex + `walls[]`). Ex. :
...
- Ce format JSON est parsé côté moteur dans `terrain_areas`
  ([terrain_utils.py](../../../engine/terrain_utils.py)) — étendu pour porter `floors`
  par étage `{level, height_inches, polygon_vertices, hexes}`. **À figer en premier dans le chantier 1.**
...
- Règle officielle (§13.06) : empreinte de la fig **entièrement** incluse dans le polygone de l'étage
  ...
  Réutiliser `compute_occupied_hexes` ([hex_utils.py](../../../engine/hex_utils.py)) et tester
...
- `geodesic_field` ([hex_utils.py](../../../engine/hex_utils.py)) est **strictement
...
- Le disque FLY ([movement_handlers.py](../../../engine/phase_handlers/movement_handlers.py))
...
- Restrictions par mot-clé (§2.2) : INFANTRY/BEASTS/SWARM libres ; MOBILE horizontal ; VEHICLE gravit >2"
  sans finir en hauteur. À brancher sur `_get_move_traversal_rules` ([movement_handlers.py](../../../engine/phase_handlers/movement_handlers.py)).
- FLY "take to the skies" : ignore le coût vertical (le malus −2" existe déjà, `get_squad_move_budget`,
  `shared_utils.py`).

### 4.4 Ligne de vue 3D
- Enrichir la primitive `_los_line_segment_clear` ([shooting_handlers.py](../../../engine/phase_handlers/shooting_handlers.py))
  ...
  - Étendre la clé du `hex_los_cache` ([combat_utils.py](../../../engine/combat_utils.py))
  ...
  - Répercuter la même logique dans le **miroir WASM** `has_los_fast`
    ([lib.rs](../../../frontend/wasm-los/src/lib.rs), cf. docstring de `_los_line_segment_clear`) :
...
- **Empreinte d'étage** : nouveau bloc dans `drawBoard` (à côté de `BoardDisplay.tsx`), tracé polygone
  copié du bloc terrain, couleur = `terrainColor` (contour non-occupé) / **blanc** si occupé (motif `objectiveControl`).
  Ne dessiner que les empreintes pertinentes au niveau courant.
- **Bouton "feuilles empilées"** : `<button>` frère inséré dans la div flex [BoardPvp.tsx](../../../frontend/src/components/BoardPvp.tsx),
  style du bouton loupe, badge n° niveau en bas-droite ; état `currentLevel` (nouveau, à côté de `boardZoom`).
- **Vue mono-niveau + ghost** : ne rendre que les figs du niveau courant ; passer en ghost (flag `modelGhost`
  déjà existant [UnitRenderer.tsx](../../../frontend/src/components/UnitRenderer.tsx)) les figs d'un
...
2. **Coût vertical** : les règles cumulent la distance verticale au M (§13.06). Le moteur n'avait **aucun coût de
   terrain** (§3.4). → Introduire le 1er coût non-binaire du pathfinding (arêtes inter-niveaux).
3. **Seuil 3" (Solid/Plunging)** et **5" (coherency/engagement)** : nécessitent une hauteur réelle par étage
   (`height_inches`), inexistante à l'époque. → Ajouter au modèle de terrain.
4. **Parité WASM** : toute évolution LoS doit être répliquée dans `has_los_fast`
   ([lib.rs](../../../frontend/wasm-los/src/lib.rs)) — code **Rust**, donc modification
...
5. **Parité 2D du moteur hors move/LoS** : collisions/placement (`build_occupied_positions_set`
   [shared_utils.py](../../../engine/phase_handlers/shared_utils.py),
   `_inflate_obstacles_by_footprint` [geodesic_move.py](../../../engine/phase_handlers/geodesic_move.py)),
   ...
   tout était 2D, sans niveau une fig à l'étage bloquerait/serait bloquée par le sol sous elle.
6. **Mesures 3D** (§2.7) : portée d'arme, distance de charge, "closest enemy" (surge) se mesurent en
   **3D socle-à-socle** — toutes les distances du moteur (`ranged_edge_distance`, budgets de charge,
   seuils) étaient 2D. Ajouter la composante verticale (`height_inches` des niveaux) aux
...
1. **Modèle d'état multi-niveau** — ⏳ **EN COURS** :
   - ✅ Parsing `floors` (format B) dans `_load_terrain_areas_from_ref` + `_parse_terrain_floors`
     ([game_state.py](../../../engine/game_state.py)) — validation stricte, `floors` absent → `[]`.
   ...
   - ✅ Propagation `level` dans `units_cache` + `models_cache` (ancre + par-figurine)
     ([shared_utils.py](../../../engine/phase_handlers/shared_utils.py)).
   - ✅ Exposition payload API (`_UNITS_CACHE_FRONTEND_KEYS` + `level_by_model`,
     [api_server.py](../../../services/api_server.py)) + types TS
...
2. **Occupation & placement** — ⏳ **EN COURS** :
   - ✅ **2a Validation "poser sur un étage"** (§13.06, décision §5.1 = 100% dedans par défaut) :
     ...
     - `floor_hexes_at_level(terrain_areas, level)` + `footprint_within_floor(...)` (0 débordement)
       ...
       ([terrain_utils.py](../../../engine/terrain_utils.py)). Tous testés (conditions cumulatives 13.06).
   ...
   - ⏸️ **2b Collisions/placement par niveau** (§5.5 : `build_occupied_positions_set`, masques) : **différé**.
     ...
     (à ce stade tout était au niveau 0 → no-op) ; un param `level` non consommé serait du code spéculatif.
   ...
   - ✅ **2c Déploiement à l'étage** (câblage `level` dans le pipeline de déploiement) — FAIT & testé :
     ...
     4. commit : `update_model_position(…, level)` persiste `model["level"]` → ancre `units_cache["level"]`
        → `unit["level"]` ([shared_utils.py](../../../engine/phase_handlers/shared_utils.py)).
     5. API : parsing squad-plan tolère 3/4-uplets (plus de drop silencieux) ([api_server.py](../../../services/api_server.py)).
   ...
   - ⏳ **2d Garde au sol sous un étage** (§2.11) — **champ front ajouté, reste backend + test** :
     - ✅ Champ `static MODEL_HEIGHT` (inches) sur **tous les fichiers roster** (`frontend/src/roster/**`),
...
3. **Mouvement vertical** — ⏳ **EN COURS** :
   - ✅ **3a Champ multi-niveaux (cœur algo)** : `reachable_multilevel_field`
     ([geodesic_move.py](../../../engine/phase_handlers/geodesic_move.py)) — Dijkstra sur
   ...
   - ✅ **2b Occupation niveau-consciente** : `build_occupied_positions_set(…, level)` +
     ...
     ([shared_utils.py](../../../engine/phase_handlers/shared_utils.py)). Param additif
     `level=None` = chemin historique **byte-identique** (tous les appelants de l'époque inchangés → zéro
...
6. **Rendu/UI (frontend)** — ⏳ **EN COURS** (visuel : à valider en lançant l'app) :
   - ✅ **6a Données + bouton d'étage** : floors exposés au front (`_zone_entry`
     ([api_server.py](../../../services/api_server.py)) rasterise les `floors`
     → `boardConfig.terrain_zones[].floors`, type `TerrainFloor` dans
     [useGameConfig.ts](../../../frontend/src/hooks/useGameConfig.ts)) ; état `currentLevel`
     + `maxFloorLevel` (dérivé des floors) + **bouton feuilles empilées** (🗂 + n° niveau en bas-droite,
     cycle les niveaux) à côté de la loupe, masqué si aucun étage ([BoardPvp.tsx](../../../frontend/src/components/BoardPvp.tsx)). tsc vert.
   - ✅ **6b Empreinte d'étage** dans `drawBoard` ([BoardDisplay.tsx](../../../frontend/src/components/BoardDisplay.tsx)) :
     tracé polygone de plancher, `currentLevel` threadé via `DrawBoardOptions` + deps de l'effet de dessin
     ([BoardPvp.tsx](../../../frontend/src/components/BoardPvp.tsx)) → redessine au clic du bouton.
   ...
   - ✅ **Niveau EFFECTIF par figurine** (`resolve_model_floor_level`, [terrain_utils.py](../../../engine/terrain_utils.py)) :
   ...
   - ✅ **Empreinte colorée par niveau** (palette badge : 1 vert, 2 orange, 3 rouge ; 0 = terrain) :
     vue étage L → voile + contour couleur(L) ([BoardDisplay.tsx](../../../frontend/src/components/BoardDisplay.tsx)).
   - ✅ **Contour au sol : conteneur dédié dynamique + couleur PAR-DÉCOR** (refonte) :
     - Le contour au sol n'est **plus** dessiné dans le plateau statique (`baseHexContainer`) mais dans un
       **conteneur dédié `floorContours`** (`floorContourContainer`,
       [BoardDisplay.tsx](../../../frontend/src/components/BoardDisplay.tsx)) reconstruit à **chaque** draw.
       `lvl`/`ofl` retirés de la clé de cache statique `bcKey` → changer de niveau **ne reconstruit plus**
       le plateau (fini l'apparition progressive au retour niveau 1→0) et la couleur d'occupation est **live**.
   ...
   - ✅ **Z-order des couches** (`app.stage.sortableChildren`) :
     ...
     est (re)forcé à **chaque réattache** dans [BoardPvp.tsx](../../../frontend/src/components/BoardPvp.tsx) —
   ...
   - ✅ **Collision niveau-consciente (move)** : deux corrections —
     ...
     niveau courant de chaque fig ([shared_utils.py](../../../engine/phase_handlers/shared_utils.py)).
   ...
   - ✅ **Vue mono-niveau (ghost)** : les figs qui ne sont pas au niveau d'affichage courant sont
     **atténuées** (fade). Flag `modelLevelGhost` (distinct de `modelGhost`, même rendu atténué),
     `modelLevels[i] !== currentLevel` ([UnitRenderer.tsx](../../../frontend/src/components/UnitRenderer.tsx)).
   ...
   - ✅ **6d Move par-figurine niveau-conscient (COMPLET)** — miroir exact du déploiement, bout-en-bout :
     ...
     - **API** `move_model_destinations` + `move_squad_unplaced_destinations` : acceptent `level` (vue) et un
       `provisional_plan` en `(col,row,level)` ([api_server.py](../../../services/api_server.py)).
     - **Front** : le plan de move porte `level` **de bout en bout** — `readSquadModelPositions` lit
       ...
       ([useEngineAPI.ts](../../../frontend/src/hooks/useEngineAPI.ts), [BoardPvp.tsx](../../../frontend/src/components/BoardPvp.tsx)).
     - **Publication `level_by_model`** : `_recompute_squad_occupied_hexes` (appelé à chaque mutation de position)
       ...
       d'ancre → tout traité niveau 0) ([shared_utils.py](../../../engine/phase_handlers/shared_utils.py)).
   ...
   1. **Collision inter-niveaux au MOVE** (une fig à l'étage bloquée par une fig au sol au même hex) — **réglé**.
      ...
      + `validate_move_plan` (`other_occupied_by_level`, [shared_utils.py](../../../engine/phase_handlers/shared_utils.py)).
   2. **Badge de niveau dynamique au drag** — **réglé** : le ghost de destination reçoit désormais
      `modelLevels`/`modelLevelGhost` dérivés live ([BoardPvp.tsx](../../../frontend/src/components/BoardPvp.tsx)).
   ...
   **Logs de diagnostic** : tous les logs temporaires `[DIAG ...]` (FORMATION, POOL, SQUAD-POOL, PREVIEW,
   DEPLOY-COMMIT dans [deployment_handlers.py](../../../engine/phase_handlers/deployment_handlers.py) ;
   MOVE dans [movement_handlers.py](../../../engine/phase_handlers/movement_handlers.py)) ont été
   **retirés**. Le calcul `resolve_model_floor_level` (niveau effectif) reste utilisé par le commit.
   ...
   - **6e Coût de montée §13.06 dans le pool move par-figurine** (`movement_build_model_destinations_pool`,
     [movement_handlers.py](../../../engine/phase_handlers/movement_handlers.py)) : le pool
     ...
     - `destinations` porte le **niveau réel par case** `[col,row,level]` (API `move_model_destinations`) ;
       ...
       aveugle) — `handleMoveModelInPlan` ([useEngineAPI.ts](../../../frontend/src/hooks/useEngineAPI.ts)),
     ...
     - **Refetch au changement de niveau** : une fig **active** refetch son pool au nouveau niveau (effet sur
       `currentLevel`, `activeModelId` lu via ref, [BoardPvp.tsx](../../../frontend/src/components/BoardPvp.tsx))
   ...
   - **Hauteur de modèle / clairance sous étage (§13.06 maison)** :
     - `MODEL_HEIGHT` (pouces) acheminé du roster jusqu'au moteur : déjà extrait par `UnitRegistry`, désormais
       ...
       ([game_state.py](../../../engine/game_state.py), [api_server.py](../../../services/api_server.py))
       → `units_cache`. (Root cause d'un 500 : `create_unit` re-whitelistait et **droppait** le champ.)
     - Helper `low_clearance_ground_hexes` ([terrain_utils.py](../../../engine/terrain_utils.py)) :
       union des hexes d'étages dont `height_inches` est **strictement** sous la hauteur de la figurine
       (tangence autorisée). *(Signature actuelle : `(terrain_areas, model_entry, squad_entry)` — la hauteur
       vient de `_model_height_of`, source unique de l'héritage figurine→escouade ; la version d'origine
       prenait directement `model_height`.)* Injecté dans les **obstacles AU SOL** uniquement (jamais `wall_hexes`
     ...
     - **Déploiement — clairance alignée EXACTEMENT sur le move [2026-07-09]** (`deployment_handlers.py`) :
       ...
       - **Géométrie = celle du move, pas l'empreinte hex.** Le move n'ajoute jamais `_low_clear` au filtre
         ...
         stationnaire de la clairance capsule `_segment_hits_hex`, [hex_utils.py](../../../engine/hex_utils.py)).
         Le blocage par **empreinte hex** sur-couvrait le disque (~½ hex) et posait la fig **plus loin** que
         le move (bug « dread loin en deploy / collé en move ») → retiré pour les bases rondes. Base
         **non-ronde** : empreinte hex conservée (miroir du move non-rond, empreinte discrète orientée).
       - Helpers `build_hex_center_index` / `disc_overlaps_indexed_hexes`
         ([hex_utils.py](../../../engine/hex_utils.py)) : index spatial bucketé (bucket =
   ...
   - **Charge niveau-consciente** (`_compute_plan_context` + `charge_model_plan_state`,
     [charge_handlers.py](../../../engine/phase_handlers/charge_handlers.py)) :
   ...
   **Constat de code (vérifié, ne rien assumer)** :
   - Primitive `entries_in_engagement_zone(first, second, engagement_zone, metric)`
     ([spatial_relations.py](../../../engine/spatial_relations.py)) — deux métriques :
     - `hex` : `min_distance_between_sets` d'empreintes union ([hex_utils.py](../../../engine/hex_utils.py)).
     ...
     - `euclidean` : `euclidean_edge_distance(socle_a, socle_b) ≤ engagement_minimum_clearance_norm(ez)`
       (= ez×1,5, [hex_utils.py](../../../engine/hex_utils.py)). **Métrique GAMEPLAY**
       (config 7.6) — c'est celle de la charge.
   - `socle_from_cache_entry` ([combat_utils.py](../../../engine/combat_utils.py)) construit
   ...
   - Wrappers additifs : `unit_entries_within_engagement_zone`, `unit_within_engagement_zone_footprints`,
     `move_anchor_violates_engagement_clearance` — tous délèguent à la primitive.
   ...
   **Implémentation optimale (3 briques, additive → zéro régression)** :
   ...
   1. ✅ **FAIT — Fondation data — `floor_height_by_model` (pouces) au build cache.** Helper
      `floor_height_at(terrain_areas, col, row, level)` ([terrain_utils.py](../../../engine/terrain_utils.py))
      ...
      ([shared_utils.py](../../../engine/phase_handlers/shared_utils.py)). Backend-only, aucun
      ...
      (`build_units_cache` + `_recompute_squad_occupied_hexes`,
      [shared_utils.py](../../../engine/phase_handlers/shared_utils.py)), publier la hauteur du **plancher**
      sous chaque fig : `height_inches` du floor de son `level`
   ...
   2. ✅ **FAIT — Gate vertical DANS la primitive.** Config `engagement_zone_vertical: 5` (POUCES, non scalé —
      absent de la boucle de scaling ×`inches_to_subhex` du chargement config,
      [w40k_core.py](../../../engine/w40k_core.py)) + getter
      `get_engagement_zone_vertical` (défini dans [spatial_relations.py](../../../engine/spatial_relations.py)).
      `MODEL_HEIGHT` publié dans l'entry `units_cache` (borne haute).
      `entries_in_engagement_zone(..., vertical_zone_inches=None)` + helpers `_vertical_classes` /
      `_entries_in_engagement_zone_3d` (3D par-paire à intervalles, euclidean only à l'époque, erreur explicite si
      hex+vertical ou données verticales manquantes) ; param propagé aux 3 wrappers. **Testé** (gate
      proche→engagé/loin→non, dégénérescence sol=2D, erreur hex+vertical ; échecs de suites **pré-existants**,
   ...
   2bis. **Détail conception — ⚠️ RÉVISÉ le 2026-08-04 : le gate est PILOTÉ PAR LA DONNÉE, plus par
      ...
      **Conception actuelle** : `entries_in_engagement_zone` applique le gate dès que **les deux
      ...
      - **Perf** : les préfiltres hex-distance existants des call-sites de charge sont
   ...
   3. ~~**Migration des call-sites (incrémentale).**~~ **[SANS OBJET depuis le 2026-08-04]** — il n'y a plus
      ...
      *(Les helpers d'époque `_charge_vertical_zone`/`_fight_vertical_zone` ont été **supprimés** avec cette
      révision : le seuil est résolu par `get_engagement_zone_vertical` dans la primitive.)*
      ...
      - ✅ **Infra synth vertical FAITE** : `_charge_synthetic_charger_cache_entry(..., level=None)`
        ([charge_handlers.py](../../../engine/phase_handlers/charge_handlers.py)) — `level=None`
      ...
      - ✅ **AUDIT DES CALL-SITES (2026-07-07)** — 19 appels effectifs d'engagement dans `charge_handlers`
        ...
        **16 « 3D direct »** (synth/vraie entry via primitive) : ancres, model pool, plan ctx UI, preview,
        non-cibles AFTER MOVING, eligibility, valid targets, destinations sol, départ (vraies entries).
        *(La liste d'origine énumérait ces sites par numéros de lignes de l'époque — périmés, retirés.)*
        **2 « 3D COMPLEXE » — ✅ FAITS** : `_eng` (`_compute_plan_context`) — branche
        ...
        [spatial_relations.py](../../../engine/spatial_relations.py), précalculé `tgt_vreach`/
        ...
        **1 différé** : le call-site du chemin FLY.
      - ✅ **VALIDATION D'INTÉGRATION 3a (2026-07-07)** — script **pérennisé**
        [tests/unit/engine/test_charge3d_floors_integration.py](../../../tests/unit/engine/test_charge3d_floors_integration.py)
      ...
      - 🎯 **DÉCOUPE 3a / 3b** :
        - ✅ **3a — cible/ennemi surélevé engagé DEPUIS LE SOL — FAIT & VALIDÉ** : les 2 constructeurs de synth
          ...
          [shared_utils.py](../../../engine/phase_handlers/shared_utils.py)) ; helper
          `_charge_vertical_zone` *(supprimé depuis — cf. 2bis)* ; **18 call-sites d'engagement câblés**
          (16 directs + les 2 complexes `_eng`/`_fp_engages` avec gate vertical par-ennemi `entry_vertically_reachable`)
          ([charge_handlers.py](../../../engine/phase_handlers/charge_handlers.py)). `synth level=0`
          partout (chargeur au sol). **Seul le call-site FLY reste 2D** (différé). Validé bout-en-bout par le
        ...
        - ✅ **3b — le chargeur MONTE (ancres `level ≥ 1`) — FAIT & VALIDÉ (2026-07-08)** :
          - **Helper climb charge** `_charge_model_climb_reachable_floor_cells`
            ([charge_handlers.py](../../../engine/phase_handlers/charge_handlers.py)) : miroir
          ...
          - **`_compute_plan_context` niveau-conscient** : nouveau param `view_level`. En vue étage (≥1,
            ...
            hex / FLY → structures vides → sortie **byte-identique 2D** (non-régression pytest verte).
          ...
          - **Front** (miroir move §6d/6e) : `chargeModelLevelByKeyRef` (Map `"c,r"→level`), plan
            ...
            ([useEngineAPI.ts](../../../frontend/src/hooks/useEngineAPI.ts), tsc vert).
          ...
          - **`charge_build_model_destinations_pool` : SUPPRIMÉ le 2026-07-29** (était du code mort, zéro
            appelant prod/test — pierre tombale en commentaire dans `charge_handlers.py`) — le pool interactif
            du front passe par `charge_model_plan_state` → `_compute_plan_context`, et la légalité d'une
            position isolée par `_charge_model_pos_is_closer`.
          ...
          - **Limites assumées** : « closer » d'une ancre d'étage mesuré horizontalement (`dist_tgt`, cohérent
            ...
            « passer sous une fig d'étage » non modélisée). **FLY en hauteur toujours différé** (le call-site
            FLY reste 2D).
          ...
          - Non-régression : [tests/unit/engine/test_charge3d_floors_integration.py](../../../tests/unit/engine/test_charge3d_floors_integration.py)
            étendu (chargeur montant → ancres d'étage `level≥1` dans le pool, gate climb §13.06) + suite pytest
            complète verte + tsc vert.
   ...
   > contenait **0 occurrence de `level`** → pile-in ET consolidation étaient **100 % 2D**. Signatures
   ...
   > `consolidationMovePlan` en `models: {col,row}` sans `level`. *(Constat d'époque — le flux manuel PvP
   > a depuis été livré, cf. 2bis de « RESTE À FAIRE » plus bas.)*
   ...
   **Enseignement clé** : pile-in/consolidation sont le **jumeau structurel de la charge** — mêmes primitives
   ...
   primitive engagement 3D (`vertical_zone_inches` — depuis 2026-08-04 le gate est piloté par la donnée,
   cf. 2bis), champ climb `_charge_model_climb_reachable_floor_cells` (**budget paramétrable** → passer
   `3×inches_to_subhex`), synth au niveau réel (`level=`), collision niveau-consciente
   (`_charge_obstacle_socles(..., level)`), clip sol-only, affichage mono-niveau, `commit_move` 4-uplet.
   → **tâche PLUS PETITE que 3b** (aucune infra à créer, uniquement du câblage miroir).

   **Implémentation optimale (miroir 3a/3b, additive → zéro régression 2D)** :

   1. **Pool par-figurine — étage** (`_fight_pile_in_build_model_pool` + `_fight_consolidation_build_model_pool`,
      [fight_handlers.py](../../../engine/phase_handlers/fight_handlers.py)) : en vue étage
      ...
      **Consolidation Objective (tier "zone")** :
      « within range » = **empreinte ∩ zone d'hexes** de l'objectif (`objective["hexes"]`, `_is_unit_on_objective`
      / viser la zone terrain) — test **déjà level-agnostic** (une fig à l'étage de la ruine-objectif
   ...
   3. **Base-contact lock (§12.03 / Ongoing §12.08)** : « models in base-contact cannot move ». **Vérifié** :
      `_fight_model_in_base_contact` ([fight_handlers.py](../../../engine/phase_handlers/fight_handlers.py))
      testait le contact en **2D pur** (`euclidean_edge_clearance_round_round` sur col/row, sans niveau) → une fig
   ...
   4. **Plan state** (`_fight_pile_in_model_plan_state` + `_fight_consolidation_model_plan_state`) :
   ...
   5. **Preview + commit** (`_fight_pile_in_preview_plan` + équivalents consolidation + handlers de commit) :
   ...
   6. **Front** (miroir charge, machinerie `perModelChargeLike` DÉJÀ partagée dans
      [BoardPvp.tsx](../../../frontend/src/components/BoardPvp.tsx)) : `pileInMovePlan` /
      ...
      **préserve déjà** le niveau (lit `activeChargeLikePlan.models[mid].level`) → deviendra correct dès que les
      ...
      (BoardPvp, alors seulement `move`/`charge`) aux modes pile-in/consolidation actifs.
   7. **FLY en hauteur : différé** (cohérent avec le call-site FLY de la charge).
   8. **DEUX MOTEURS de pile-in (vérifié 2026-07-08) — cadrer la migration en conséquence.**
      ...
      dépendait du mode (auto vs PvP interactif) et du moment (normal vs overrun) :
      - **Moteur B — par-figurine (interactif PvP)** : `_fight_pile_in_build_model_pool`
        ([fight_handlers.py](../../../engine/phase_handlers/fight_handlers.py)) → `_fight_pile_in_model_plan_state`
        + commit. **C'est le seul que couvrent les points 1-7 ci-dessus.**
      - **Moteur A — unité/rigide (auto/IA/gym)** : `pile_in_move_destinations_12_03`
        ([fight_handlers.py](../../../engine/phase_handlers/fight_handlers.py)) → `_ai_select_pile_in_destination`
        → apply. Utilisé alors par : le pile-in **normal auto**, la **présentation squad-level**, et **surtout
        le pile-in d'OVERRUN** — lequel était auto-résolu **même en PvP** (pas d'UI par-fig pour la pile-in
        additionnelle §12.06).
      - **Conséquence 3D (d'époque)** : migrer B → couvre le pile-in **PvP interactif normal**. L'**overrun** ET
      ...
      - **Consolidation = structure DIFFÉRENTE du pile-in (vérifié)** : (a) son auto **skippe entièrement**
        ...
        **routeur** vers des autoplaces existants dont l'AFTER coïncide avec le mode (docstring de
        ...
        `pile_in_autoplace_plan` (AFTER « chaque fig garde ses engagements »), `engaging` →
        **`charge_autoplace_plan`** (budget 3", couverture dure « toutes les cibles »), `objective` → **non supporté**.
        → À migrer pour la conso 3D : **le pool par-fig** `_fight_consolidation_build_model_pool` (points 1-7),
      ...
      - **`pile_in_autoplace_plan`** est un **maximiseur autonome** (ILP, réutilise les primitives charge
      ...
      - **POURQUOI cette asymétrie pile-in vs conso (rationale, dicté par les règles)** : **pile-in et charge sont
        ...
        (2) l'**overrun (§12.06) est une pile-in additionnelle**, pas une conso → le moteur A du pile-in servait double
   ...
   **Limites assumées (miroir charge 3b)** : « closest tier » (`_fight_pile_in_closest_tier_ids`) et
   ...
   **Validation** : script d'intégration dédié (miroir `test_charge3d_floors_integration.py` : fig montant en pile-in/
   conso → ancres d'étage + gate climb 3") + suite pytest complète + app sur `scenario_floors_test`.
   ...
   **✅ Décisions figées (2026-07-07)** :
   - **Seuils verticaux = 5" pour les DEUX contrats** : engagement range (§03.04, conforme) ET contact
     ...
     [charge_handlers.py](../../../engine/phase_handlers/charge_handlers.py)) ; les
     ...
     (clé `engagement_zone_vertical`, pendant vertical de `engagement_zone: 2`).
   ...
   - Le mode 3D était **lié à la métrique `euclidean`** (config active `game_config.json`) à l'époque de cette
     décision — **périmé depuis** : le gate vertical traite les deux métriques (cf. 2bis).

   **Débouché charge** (une fois 1→3 en place) : brancher `vertical_zone_inches` dans les tests d'engagement
   de la charge, **retirer le garde-fou** « PAS de destinations en hauteur » ([charge_handlers.py](../../../engine/phase_handlers/charge_handlers.py))
   et brancher `reachable_multilevel_field` ([geodesic_move.py](../../../engine/phase_handlers/geodesic_move.py))
   ...
   > **[Mise à jour 2026-08-28]** La tranche TIR de ce plan est **implémentée** :
   > `_los_line_segment_clear(..., floor_occluders=, z_start=, z_end=)`
   > ([shooting_handlers.py](../../../engine/phase_handlers/shooting_handlers.py)) porte le
   > plancher-occulteur par hauteur interpolée (`h(t) = z_start + (z_end − z_start)·t`, blocage si
   > strictement sous le dessus de la dalle — tangence légale, cohérent `low_clearance`) ; propagé par
   > `_los_hex_visible` / `_compute_visibility_with_obscuring` ; l'éligibilité portée+LoS par-figurine passe
   > par `_attacker_model_can_reach_squad` ([shared_utils.py](../../../engine/phase_handlers/shared_utils.py)).
   > Plunging Fire §22.05 est résolu au **calcul de BS du tir** (flag `plunging_fire_applied`, clé config
   > `plunging_fire_height` dans `config/game_config.json`).
   > **Non couvert à cette date** : les murs (`wall_set`) bloquent toujours **inconditionnellement** dans les
   > deux chemins de la primitive (le seuil Solid (b1) n'est pas câblé — la clé `solid_ground_clearance`
   > n'existe pas) ; la clé du `hex_los_cache` reste 2D `((col,row),(col,row))` (combat_utils).
   > L'état courant fait foi dans la Roadmap. Le plan d'origine suit — il reste la référence de conception
   > (cas limites, décisions, parité WASM).

   > **Constat de code vérifié (2026-07-10, ne rien assumer — depuis partiellement périmé, cf. ci-dessus)** :
   > toute la chaîne LoS était **strictement 2D**. `compute_unit_los`
   > ([shooting_handlers.py](../../../engine/phase_handlers/shooting_handlers.py))
   > → `_compute_unit_los_uncached` → `_compute_visibility_with_obscuring` →
   > `_los_hex_visible` → **primitive de tracé unique** `_los_line_segment_clear` : trace
   > `hex_line` (cube-lerp) et bloquait **uniquement** sur `wall_set` (murs, tous niveaux) et les cases
   > obscuring hors areas exclues (§13.10). **Aucune** de ces fonctions ne lisait `level`, `floor_height`,
   > `MODEL_HEIGHT` ni les `floors` du terrain (grep `level|floor|height|elevation` sur la géométrie LoS =
   > 0 hit géométrique). Le champ `level` existait (units_cache/models_cache) mais **n'entrait jamais** dans le
   > calcul de visibilité. Miroir Rust `has_los_fast` ([lib.rs](../../../frontend/wasm-los/src/lib.rs))
   > = même primitive 2D.
   ...
   **Conséquence des deux cas types (unité tireuse 1008 au niveau 1)** :
   - **Cible SOUS un étage** (fig au sol, empreinte sous l'empreinte d'un plancher supérieur) : le
     ...
     §13.05/§13.11). À l'époque le moteur ne modélisait **que les murs verticaux** (`wall_set`) ; le
     **plancher horizontal** n'était un obstacle ni de move ni de LoS → la cible ressortait **visible à tort**.
   - **Cible AU SOL à côté de l'empreinte de l'étage** (tireur surélevé) : la LoS était tracée en 2D pur
     entre empreintes ; le niveau du tireur (1) et celui de la cible (0) **n'intervenaient pas**. Seul le
     `wall_set` (mur de la ruine, prolongé à tous les niveaux, cf. décision « murs verticaux prolongés »
     du chantier 2b) bloquait — et il bloque **à tous les niveaux identiquement**, sans la nuance Solid
     §13.11 (au-dessus de 3" la vue passe par les ouvertures).
   ...
   **Données déjà disponibles (aucune nouvelle donnée terrain/unité à créer, chantier 1 + §6e + engagement 3D)** :
   - Par figurine dans `units_cache` : `level_by_model` (niveau), `floor_height_by_model` (**hauteur du
     plancher sous la fig, en pouces**, résolue par position — `build_units_cache` +
     `_recompute_squad_occupied_hexes`, [shared_utils.py](../../../engine/phase_handlers/shared_utils.py))
     et `MODEL_HEIGHT` (borne haute). → chaque fig = **intervalle vertical**
   ...
   - `floors` du terrain rasterisés : `{level, height_inches, polygon_vertices, hexes}` par étage
     ([game_state.py](../../../engine/game_state.py) `_parse_terrain_floors`) ;
     helpers `floor_hexes_at_level` / `floor_height_at` / `floor_polys_at_level`
     ([terrain_utils.py](../../../engine/terrain_utils.py)) déjà écrits.
   ...
   **Implémentation optimale (additive → zéro régression 2D, une seule primitive, miroir WASM)** :
   ...
   fois** (docstring de la primitive), puis répliquée dans le Rust. Le plan enrichit cette primitive et le fait
   par-figurine (le tracé cible est déjà par-modèle, boucle de `_compute_unit_los_uncached`), en threadant
   les niveaux/hauteurs source+cible.

   1. **Signature niveau-consciente (additive).** Ajouter à `_los_line_segment_clear` (et à `_los_hex_visible`,
      `_compute_visibility_with_obscuring`, la boucle par-modèle) les paramètres **optionnels**
      ...
      **le `hex_los_cache` 2D `((col,row),(col,row))` doit intégrer les niveaux** sous peine de collision entre
      ...
      ([combat_utils.py](../../../engine/combat_utils.py)) **et**
      `has_line_of_sight_coords` — plus l'**invalidation sélective** qui filtre les clés en supposant
      leur forme actuelle ([shooting_handlers.py](../../../engine/phase_handlers/shooting_handlers.py)).
      ...
      **Trois cas limites à spécifier (review 2026-07-10, 2ᵉ passe)** :
      - **Intervalle à cheval sur le plan** (`lo < h_f < hi` strict) : ≥1 ligne ne franchit jamais →
        **jamais bloqué par ce plancher**. C'est la géométrie correcte (pas un fallback) : l'état n'est
        atteignable que par les chemins sans clairance (pool squad/IA, voile rouge — RESTE À
   ...
   3. **Solid §13.11 — nuance ≤3" / >3".** Le `wall_set` bloque **inconditionnellement** (toujours vrai à ce
      jour). Trois variantes (décision §5) :
   ...
   4. **Obscuring §13.10 : inchangé.** L'exclusion par area occupée (tireur/cible) reste 2D et **correcte**
      ...
      **Condition de validité (review 2026-07-10)** : ce raisonnement suppose `floor ⊆ empreinte de l'area
      ...
      (`_parse_terrain_floors`, [game_state.py](../../../engine/game_state.py) —
   ...
   5. **Plunging Fire §22.05 (bonus BS, hors LoS pure).** Après confirmation de visibilité, **+1 BS** si la
      ...
      *(Implémenté depuis — cf. la mise à jour en tête de section.)*
   ...
   6. **Parité WASM (Rust).** Répercuter (2) et, si retenu, (3b) dans `has_los_fast`
      ([lib.rs](../../../frontend/wasm-los/src/lib.rs)) : passer les hauteurs src/tgt + les
   ...
   **Non-régression** : `src_height=tgt_height=None` (tous les appels d'origine) → primitive **byte-identique**
   2D ; plateau **sans `floors`** → aucun plancher-occulteur, `wall_set` seul → identique à l'existant ;
   RL/obs (métrique mono-niveau, `hex`) non concernés. **Validation** : script d'intégration dédié (miroir
   [tests/unit/engine/test_charge3d_floors_integration.py](../../../tests/unit/engine/test_charge3d_floors_integration.py) sur
   ...
   + réutilisation de
   l'invariant `assert_los_pair_cache_consistent` (défini dans
   [shared_utils.py](../../../engine/phase_handlers/shared_utils.py) ; le pair-cache doit rester cohérent
   avec la nouvelle géométrie). **Le RL étant HS**, c'est le seul filet de non-régression du gameplay 3D
   (comme pour charge/mêlée).
   ...
   **Décisions à trancher (checkpoints §5)** :
   ...
   - **Seuils configurables** : `solid_ground_clearance` (3", Solid) et `plunging_fire_height` (3", Plunging)
     ...
     *(`plunging_fire_height` existe dans `config/game_config.json` ; `solid_ground_clearance` reste à créer
     avec (b1).)*
   ...
   1. ✅ **Engagement 3D (chantier 4) — FAIT & VALIDÉ** : étapes 1 (fondation `floor_height_by_model` +
      ...
      Validé sur le vrai scénario ([tests/unit/engine/test_charge3d_floors_integration.py](../../../tests/unit/engine/test_charge3d_floors_integration.py)).
      Non-régression pytest **entièrement rétablie** (suite complète verte à la livraison ;
      dette de stubs `level`/`MODEL_HEIGHT`/`engagement_zone_vertical`/`_unit_move_version` + tests obsolètes
   ...
   2. ✅ **3b — Champ climb-aware dans la charge (chargeur qui MONTE) — FAIT & VALIDÉ EN APP (2026-07-08)** :
      ...
      `tests/unit/engine/test_charge3d_floors_integration.py` étendu + suite pytest complète verte + tsc vert.
   2bis. ✅ **Mêlée 3D — pile-in (§12.03) & consolidation (§12.08) niveau-conscients : FLUX MANUEL PvP FAIT
      (2026-07-08, compile Python + import + `tsc` verts, NON validé runtime à cette date)**. Miroir move
      complet (option A) : une fig déjà en hauteur peut **rester** sur son étage ET **changer** de niveau
      dans le budget 3".

      **Fait — backend** ([fight_handlers.py](../../../engine/phase_handlers/fight_handlers.py)) :
      ...
      - Réutilisation directe du reachable d'étage de move : ajout d'un `start_level=0` optionnel rétro-compatible à
        [`_model_climb_reachable_floor_cells`](../../../engine/phase_handlers/movement_handlers.py)
      ...
      - Plans en **4-tuples `(mid,col,row,level)`** : preview (`_fight_pile_in_preview_plan` /
        ...
        dispatch (`_prov_from_action` capture le 4ᵉ élément + `action.level` ; commit construit des 4-tuples ;
        `moveDetails` porte `toLevel`), commit via `commit_move` (gère déjà le 4ᵉ élément ; `level=None` = garder niveau).
      - Activation d'unité : `view_level` initial = niveau de l'unité (1er rendu correct).

      **Fait — frontend** ([useEngineAPI.ts](../../../frontend/src/hooks/useEngineAPI.ts) +
      [BoardPvp.tsx](../../../frontend/src/components/BoardPvp.tsx)) :
   ...
   3. **Clairance hauteur — reste du mouvement** (voile rouge + pool squad/IA) — *plan détaillé, à faire d'un
      ...
      - **3.1 Voile rouge** `movement_preview_move_plan`
        ([movement_handlers.py](../../../engine/phase_handlers/movement_handlers.py)) : calculer une
        fois `_low_clear = low_clearance_ground_hexes(...)` (hauteur de la figurine — signature actuelle
        `(terrain_areas, model_entry, squad_entry)`) puis, dans la boucle
        par-modèle, ajouter la clairance à la détection d'obstacle **uniquement pour une figurine au sol**
        (`lv == 0`) — étendre le test `fp_wall` (`fp & wall_hexes_set`) avec `fp & _low_clear`, sans
      ...
      - **3.2 Pool squad/IA** `movement_build_valid_destinations_pool`
        ([movement_handlers.py](../../../engine/phase_handlers/movement_handlers.py)) : même injection
...
- **Corrigé** :
  - Backend : l'activation pile-in/consolidation utilise `_view_level` (niveau de vue transmis par le
    front), cohérent avec refresh/commit — [fight_handlers.py](../../../engine/phase_handlers/fight_handlers.py)
  ...
  - Front : `activate_unit` en phase fight envoie `level: currentLevel` —
    [useEngineAPI.ts](../../../frontend/src/hooks/useEngineAPI.ts) (`handleSelectUnit`).
  - Charge : `unit["level"]` resynchronisé sur l'ancre au commit charge (miroir du commit move) —
    [charge_handlers.py](../../../engine/phase_handlers/charge_handlers.py) (`charge_commit_move_plan_handler`).
...
([terrain_utils.py](../../../engine/terrain_utils.py)) : pour une base **ronde**, le
...
([fight_handlers.py](../../../engine/phase_handlers/fight_handlers.py)). Corrige la
...
- Ajout de la **clairance verticale** `_low_clear = low_clearance_ground_hexes(...)`
...
- **Écart 2D/3D** : l'engagement et la sélection de cibles pile-in étaient mesurés en **2D pur**
  (`unit_entries_within_engagement_zone` sans `vertical_zone_inches`) à cette date ; le « closer » est
  mesuré en 2D horizontal vers la **projection** de la cible. Correct pour une cible en hauteur (distance
  3D monotone à hauteur constante) mais pas 3D strict (cf. §2.7). *(L'engagement fight est passé en 3D
  depuis — cf. « Chantier 4 — étape FIGHT en 3D ».)*
...
- **Primitives** ([hex_utils.py](../../../engine/hex_utils.py)) : `disc_within_polygon`
...
- **Source partagée** ([terrain_utils.py](../../../engine/terrain_utils.py)) :
...
- **Câblage boucles chaudes** : polygones précalculés **une fois par niveau** (pas par case) — pool d'étage
  du move ([movement_handlers.py](../../../engine/phase_handlers/movement_handlers.py))
...
- **Front** : champ `chargeRollOverride` persisté en `localStorage`
  ([BoardWithAPI.tsx](../../../frontend/src/components/BoardWithAPI.tsx)) ; le hook envoie
...
- **Backend** : la valeur est posée dans `game_state["charge_roll_override"]`
  ([api_server.py](../../../services/api_server.py)) ; aux 2 sites de jet 2D6
...
([game_state.py](../../../engine/game_state.py)) → toujours présent. Plateau sans étage
...
- **Move** — `_socle_overlaps_floor` **retiré** du pool
  ([movement_handlers.py](../../../engine/phase_handlers/movement_handlers.py)) : les cases sol
...
sur le plancher) reste inchangée. Base non-ronde : hex partout. Suite complète verte à la livraison.
...
Jusqu'à cette tranche, seule la CHARGE consommait le seuil vertical. Le FIGHT mesurait
...
La tranche a livré un helper `_fight_vertical_zone` (miroir strict de `_charge_vertical_zone`) câblé sur
**tous** les chemins du fight. *(**[Mise à jour 2026-08-28]** Ces deux helpers ont été **supprimés** avec
la révision du 2026-08-04 : le gate vertical n'a plus de paramètre, la primitive
`entries_in_engagement_zone` l'applique dès que les deux entrées portent leurs cartes verticales et
résout le seuil via `get_engagement_zone_vertical` — cf. 2bis du chantier 4 ENGAGEMENT 3D.)* Les chemins
couverts :

- **Prédicats** — `_fight_v11_engaged_now`, `fight_compute_engaged_snapshot` (12.04 / overrun 12.06),
  `_fight_units_engaged_with`, l'éligibilité consolidation *(le prédicat dédié d'époque a disparu — elle
  passe par `fight_v11_is_consolidation_eligible` / `fight_v11_consolidation_mode`)*,
...
**Correction associée — `_fight_synth_cache_entries_at_footprint`** *(livrée sous le nom
`_fight_synth_cache_entry_at_footprint`, renommée au pluriel depuis)*. Elle décrit une empreinte CANDIDATE
...
`parse_model_plan_with_orientation` ([shared_utils.py](../../../engine/phase_handlers/shared_utils.py)),
...
`[MODELS: <mid>@(col,row,z<hauteur>)]` (construit dans
[action_log_utils.py](../../../engine/action_log_utils.py)), où `z<hauteur>` est la hauteur du plancher
sous le socle, **en pouces**. C'est la hauteur et non le `level` qui est journalisée : la hauteur d'un
niveau dépend de la POSITION (deux ruines peuvent avoir un étage 1 à des hauteurs différentes, cf.
...
`is_within_engine_engagement_zone` ([analyzer.py](../../../ai/analyzer.py)) garde sa géométrie
HORIZONTALE mot pour mot (ancre, socle rond 1) et gagne le gate vertical : chaque hauteur distincte
présente dans l'unité devient une classe
...
- **Tir** (`shooting_handlers`, verrou de tir de `build_squad_action_mask`), cohésion, contrôle
  ...
  multi-niveaux. *(La LoS de tir a depuis reçu le plancher-occulteur — cf. la mise à jour du
  chantier 5 « TIR 3D » ; le +1 BS Plunging est câblé au calcul de BS.)*

---

## Historique et sources

- Document né sous le nom `stage.md` : reformulation du projet « étages / niveaux verticaux » fondée
  **uniquement** sur le code (`engine/`, `frontend/`) et les règles officielles
  (`Documentation/40k_rules/`) — toute affirmation « règle » citée avec son PDF source.
- Chantiers menés de 2026-07-05 à 2026-07-10 environ (modèle `level`, occupation, mouvement vertical,
  engagement 3D, charge/mêlée niveau-conscientes, rendu) ; révision majeure le **2026-08-04** (gate
  vertical piloté par la donnée dans `entries_in_engagement_zone`, suppression des helpers d'opt-in).
- Renommé `verticalite.md` le **2026-08-28** (consolidation de la référence moteur) : bandeau réécrit,
  ancres fichier:ligne converties en symboles vérifiés, liens recalculés, constats périmés annotés
  **[Mise à jour 2026-08-28]** (suppression du moteur pile-in rigide, LoS 3D tranche TIR livrée,
  signature `low_clearance_ground_hexes`, renommage `_fight_synth_cache_entries_at_footprint`,
  emplacement de `get_engagement_zone_vertical`). Aucune restructuration : ordre et numérotation des
  §/chantiers d'origine conservés.
- Les marqueurs d'état (✅/⏳/⏸️) et les mentions de suites de tests sont **historiques** — l'état des
  chantiers fait foi dans `Documentation/Roadmap/`.

## Correspondance des sources

| Source | Ancien § / identifiant | Section actuelle | Cité par |
|---|---|---|---|
| stage.md | §4.1 | §4.1 « Modèle de données (fondation) » | `engine/terrain_utils.py` (×2, `height_inches` par position) |
| stage.md | §4 / chantier 4 | « 🎯 ENGAGEMENT 3D — CONCEPTION OPTIMALE (CENTRALISÉE) — chantier 4 » (intervalle vertical `[plancher, plancher+MODEL_HEIGHT]`) | `engine/phase_handlers/shared_utils.py` |
| stage.md | chantier 4 | idem + « Chantier 4 — étape FIGHT en 3D + plans par-figurine porteurs de leur étage » | `engine/spatial_relations.py` |
| stage.md | § murs verticaux prolongés | chantier 2, bloc « 🧱 Décision — murs verticaux prolongés » | `engine/phase_handlers/deployment_handlers.py`, `engine/phase_handlers/movement_handlers.py`, `engine/phase_handlers/shared_utils.py` |
| stage.md | « idée initiale » (empreinte blanchie) | §1 « Objectif » | `frontend/src/components/BoardDisplay.tsx`, `frontend/src/components/BoardPvp.tsx` |
| stage.md | simplification escalade ext/int, sections <2" | chantier 3, 3b « Restrictions mot-clé » | `engine/phase_handlers/geodesic_move.py` |
| stage.md | champ `level` (0 = rez-de-chaussée) | §4.1 + chantier 1 | `frontend/src/types/game.ts` |
