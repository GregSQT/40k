...
- Étapes : retrait de `models_cache` puis `squad_models` → recalcul `occupied_hexes` → invalidation des paires LoS du squad → recalcul des règles d'unité en vigueur (19.04 : la mort du dernier bodyguard ou d'un leader éteint des sources de règles ; une figurine tuée par une attaque garde son effet jusqu'à la fin des attaques de l'attaquant via `_finalize_manual_allocation`) → événement `"dead"` explicite dans `action_logs` (chaque mort tracée par modèle + raison) → §24.08 DEADLY DEMISE mise en file (`MORTAL_WOUND_QUEUE_KEY`) si la figurine porte la règle ET que la mort est une attaque (`combat`) ou une blessure mortelle hors attaque (`hazard`) — jamais pour `coherency_removal` (03.03 : « do not trigger rules that apply when a model is destroyed ») ni pour une unité hors du champ de bataille (`strategic_reserves_timeout`, `deployment_no_space`) → dernière figurine : `remove_from_units_cache` ; sinon recalcul de l'ancre, `HP_CUR` agrégé et `squad_cache`.
...
### Sélection rectangle — bloc partiel (move et charge)

Outil de la barre latérale (icône carré pointillé, [SharedLayout.tsx](../../../frontend/src/components/SharedLayout.tsx)) : clic-glisser GAUCHE sur le plateau, en phase move ou charge seulement (ailleurs et au clic droit l'outil est passe-plat : plateau jouable, reset per-fig vivant), capture les figurines de l'escouade activée dont le socle touche le rectangle (voile vert en direct, une seule escouade sinon rien ; en charge, seules les figurines éligibles de la phase 11.04 courante). Au relâchement, le bloc suit le curseur en **translation rigide** et **snappe sur un pool d'ancres calculé par le moteur** — comme le squad move rigide, jamais de pose hors pool ; clic = pose dans le plan hôte (`squadMovePlan` / `chargeMovePlan`), clic droit ou Échap = abandon (figurines remises à l'origine / dé-posées).

- Moteur, move : `movement_build_block_destinations_pool` (movement_handlers.py) = intersection, en offsets **cube** relatifs à la première figurine, des pools par-figurine `movement_build_model_destinations_pool` avec les sœurs sélectionnées **soulevées** (`lifted_model_ids` : elles bougent ensemble, leur origine et leur position provisoire ne comptent pas — 03.01 : traversée des figurines amies) ; les sœurs non sélectionnées gardent leur position provisoire. Invariant « masque ⊆ exécutable » par construction (aucun prédicat dupliqué) ; la **cohésion** reste jugée sur le plan complet à la pose (`preview_move_plan`, 03.03).
- Moteur, charge : `charge_block_destinations` (charge_handlers.py) = même intersection sur les pools de charge par-figurine (`_charge_model_view_pool` : atteignable dans le jet, plus près d'une cible, ≤1"/engagée selon la phase — conditions **par figurine**, non invariantes par translation, donc jamais évaluées côté front) ; figurine non éligible → erreur explicite. Un pool vide est une réponse métier légale (paire de figurines sans ancre commune, fréquent en phase « ≤1" ») → refus affiché « aucune position commune ».
- Intersection partagée : `rigid_block_anchor_placements` (shared_utils.py). API : `move_block_destinations` (api_server.py, lecture pure) et `charge_block_destinations` (dispatch charge). Réponse `destinations = [[anchorCol, anchorRow, [[mid, col, row, level], …]], …]` + `footprint_mask_loops` (zone d'atterrissage du bloc : union des empreintes de chaque figurine sur ses destinations — `movement_block_footprint_mask_loops` / `charge_block_footprint_mask_loops`), dessinée par le board comme la zone de la figurine sélectionnée.
- Suivi PIXI pur, miroir du move preview d'escouade : un fantôme par figurine (socle + icône, alpha 0,65) snappé sur `blockPoolRef` à chaque mousemove, `app.render()` explicite, **aucun state React par hex** (un state par hex redessinait tout le plateau → lag) ; une seule écriture de plan à la pose (`onFreezeBlock(anchorKey)`), abandon sans rien restaurer. Rafraîchissements de validité (`preview_move_plan` / `charge_plan_state`) gardés par une séquence : une réponse plus ancienne que la dernière demande est ignorée.
- Front : [blockSelection.ts](../../../frontend/src/utils/blockSelection.ts) (parse, snap, hit-test socle/rectangle, règle une-seule-escouade), état `blockFollow` + handlers dans useEngineAPI.ts, rectangle/voile/suivi dans BoardPvp.tsx (capture `window` : ni PIXI, ni sélection d'unité, ni pan ne voient le pointeur). Le vecteur de prise (origine de l'ancre − hex de relâchement) garde le bloc à sa position relative au curseur.
- Tests : `tests/unit/engine/test_block_destinations_pool.py` (intersection, soulèvement, invariant d'exécution move + charge), `tests/integration/pvp/test_move.py::TestBlockDestinations`, `tests/integration/pvp/test_charge.py` (bloc de charge), vitest `blockSelection.test.ts` et `useEngineAPI.test.ts` (flux bloc).
...
- **Une déclaration par arme PHYSIQUE et par figurine** (► Multiple Weapon Profiles, renvoi de 04.01) : les profils exclusifs d'un combi (Cyclone Frag/Krak, plasma Standard/Supercharge) partagent leur `COMBI_WEAPON`, en déclarer deux dans la même activation est illégal. `_weapon_group_key` porte l'identité de l'arme physique ; `_pick_one_profile_per_weapon_group` retient, **parmi les profils déclarables**, celui de meilleure espérance de dégâts contre sa cible (`_ranged_profile_expected_damage` — même modèle que la résolution, [BLAST] et dégâts perdus en excès compris ; à égalité, l'index le plus bas). Garder l'index le plus bas laissait l'ordre du roster décider et condamnait un profil sur deux. Le masque split-fire (`shoot_weapon_sel_open_slots`) applique la même règle à un grain **plus gros** — un profil par arme physique et par **escouade** — parce que ses slots sont des profils d'escouade : restriction d'encodage, jamais une autorisation.
...
- **Contrôle d'objectif** (2026-07-30) : `oc_weight` abandonné, jamais implémenté — le contrôle est payé par `objective_rewards.vp_margin_factor` × Δ(VP_moi − VP_lui), versé au step exact où la mission attribue des VP à l'un ou l'autre joueur (ledger B6 du 2026-09-14, `_calculate_vp_margin_reward` ; avant : `objective_reward_factor` × VP propres, une fois par tour à la phase command du joueur contrôlé). Un bonus par fin de phase paierait des états de contrôle sans VP : l'agent optimiserait la mesure de substitution. La conservation d'un objectif d'un tour sur l'autre s'apprend par la value function, pas par un signal dense. Le contrôle lui-même est réévalué à chaque checkpoint (14.02, `run_objective_control_checkpoint`, game_state.py).
