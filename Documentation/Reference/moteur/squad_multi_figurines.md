...
- Intersection partagée : `rigid_block_anchor_placements` (shared_utils.py). API : `move_block_destinations` (api_server.py, lecture pure) et `charge_block_destinations` (dispatch charge). Réponse `destinations = [[anchorCol, anchorRow, [[mid, col, row, level], …]], …]`.
- Rafraîchissements de validité (`preview_move_plan` / `charge_plan_state`) gardés par une séquence : une réponse plus ancienne que la dernière demande est ignorée (le suivi en émet une par hex).
