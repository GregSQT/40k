...
- **Fonction de portée unifiée** : `def ranged_in_range` / `def ranged_edge_distance` (combat_utils). `hex` → `min_distance_between_sets(fp) <= rng` ; `euclidean` → `euclidean_edge_distance <= rng × 1.5`. La conversion ×1.5 vit **ici et dans les primitives seulement**, jamais aux call-sites. Toute distance de RÈGLE entre deux socles la prend, pas seulement le tir : depuis le 2026-09-18, la clause « each unit within 6" of that model » de Deadly Demise 24.08 (`def _roll_deadly_demise`, shared_utils) y passe aussi, avec la métrique `ranged` du run et le socle de la figurine détruite relevé à la mort (`destroy_model`, entrée de `MORTAL_WOUND_QUEUE_KEY`). Elle était calculée sur les écarts de GRILLE bruts (`sqrt(dc**2 + dr**2)`), donc à la fois anisotrope (voir la puce suivante) et centre-à-centre là où 01.04 impose le bord du socle. Verrou : `tests/unit/engine/test_deadly_demise.py` (frontière anisotrope et frontière de socle à x5).
- **Distance de règle vers un POINT** (et non vers un autre socle) : `def ranged_edge_distance_to_cell` (combat_utils), résultat en subhexes. Même contrat que ci-dessus, la cible étant une cellule : `hex` → distance hex ancre→cellule ; `euclidean` → bord du socle → centre de la cellule, ÷ 1,5. Deux règles la prennent : l'overlay de portée de tir, et la clause « within 6" of the centre of the battlefield » d'Unbreakable Resolve (`def _model_is_near_objective_or_center`, shared_utils), cette dernière via `def ranged_edge_distance_to_battlefield_center` (combat_utils) — **source unique de la position du centre**, prise aussi par le juge (`def model_near_objective_or_center`, analyzer_save). Le centre y est le **point géométrique** du plateau (`def battlefield_center_norm`, hex_utils), pas la case `(cols // 2, rows // 2)` : les deux plateaux du projet ayant des dimensions paires (44×60 à x1, 220×300 à x5), aucune case n'est le centre et celle-là en est décalée de 0,764 subhex vers l'est et le sud — 0,153" à x5, où 90 des 2967 cases-ancre de la zone basculaient de verdict (socle rond de 6 subhex, mesuré le 2026-09-18). Le bord bas se lit sur une colonne PAIRE : la dernière case des colonnes impaires est une demi-case fantôme hors plateau (`def is_phantom_bottom_hex`), et la prendre pour bord laisserait le centre 0,289 subhex trop au sud. À x1 la géométrie est hexagonale et entière : aucun point fractionnaire n'y est mesurable, la case d'indice moitié y reste la cible et l'écart y est assumé. **La grille n'est pas isotrope** : un pas de colonne vaut 1 subhex, un pas de rangée √3/1,5 ≈ 1,155 — mesurer une distance de règle en cases hex à une résolution euclidienne accorde donc la portée jusqu'à 15 % trop loin vers le nord/sud. Mesuré le 2026-09-18 sur un plateau 100×100 à x5 : 30 rangées au sud du centre valent 34,6 subhex, comptées à portée par la mesure en cases. Verrous : `tests/unit/engine/test_feel_no_pain_conditional.py` (frontière anisotrope à x5, isotrope à x1) et `tests/unit/ai/test_analyzer_fnp.py` (le juge mire la primitive).
...
**Montée 13.06 en métrique HEX (gym) — le sol est le BFS hex, l'étage reste any-angle (2026-09-16).** Le champ de montée par-figurine (`def ascent_field_for_model`, [shared_utils.py](../../../engine/phase_handlers/shared_utils.py), source unique de l'érosion du masque et de `def model_reach_predicate`) construit `def reachable_multilevel_field` en lui INJECTANT la passe de niveau 0 : en euclidien le champ any-angle de la figurine relu du cache `eucl` (jamais calculé pour l'occasion), en hex le BFS hex de plain-pied (`def geodesic_move_reach` sur le bitmap de niveau 0, coût × 1,5). Conséquence en hex : le sol d'une montée est mesuré **comme le move à plat du gym** — pas hex par cellule, sans clairance de socle — et non plus any-angle avec clairance : jusqu'à 15 % de portée en plus (√3/1,5 vers le sud, 1,0 vers l'est, cf. §4.3) et couloirs d'une case admis ; seule la passe d'**étage** reste any-angle avec clairance (13.06 « no part of its base overhangs »), reliée au sol par les portails (`hypot` centre à centre + distance verticale). C'est un changement de métrique assumé (perf : la passe any-angle au sol pesait 26 % du step x1), pas une dérive : les deux côtés de « masque ⊆ exécutable » lisent ce champ, et le pool PvP ne construit d'étage qu'en euclidien. Le **pré-check de portée** `def multilevel_target_within_straight_bound` ([geodesic_move.py](../../../engine/phase_handlers/geodesic_move.py)) suit la métrique du sol (`ground_is_hex`) : la ligne droite euclidienne ne minore pas un sol hex (1,5 × pas ≤ euclid), la borne devient 1,5 × `hex_distance` + |Δhauteur|, tolérance `_SEG_TOL` conservée. Verrous `tests/unit/engine/test_ascent_field_hex_ground.py` et `test_ascent_field_reuses_euclidean_field.py`.

**Plan rigide et niveau PAR FIGURINE sans déclaration de montée (2026-09-18).** Le squad move rigide (`def build_rigid_plan`, gym et bots) n'envoie plus toute l'escouade au sol : chaque figurine reçoit le niveau que `def model_move_destination_level` ([movement_handlers.py](../../../engine/phase_handlers/movement_handlers.py)) résout à SA case d'arrivée, lu dans `def model_rigid_level_map` (dont `model_move_destination_level` est la requête ponctuelle, sans reconstruire la carte par candidate) — carte complète du socle sous déclaration de montée (elle peut monter) ; sans déclaration, une figurine déjà en hauteur **garde son étage là où la case le porte** (13.06 MOVING VERTICALLY : longer un plancher est un mouvement horizontal) et descend au sol ailleurs, jamais plus haut — lu dans la carte `by_level` de `FloorLevelMaps` (`def floor_level_maps`, [terrain_utils.py](../../../engine/terrain_utils.py)), la carte PAR NIVEAU où chaque étage est tenable pour lui-même, et non dans sa jumelle `highest` (`def floor_level_by_cell`), qui ne retient que le niveau le plus haut et enverrait au sol une figurine à l'étage 1 sous un plancher 2 (suite 139, 2026-09-18) ; une figurine au sol reste au sol (régime bit-à-bit identique à l'ancien). L'érosion du masque (`def erode_move_pool_by_squad_block`) lit la même carte par figurine et par candidate, et refuse toute candidate où deux figurines atterriraient sur la même (niveau, case) — le prédicat `new_cells` de la validation. Pourquoi : deux figurines d'une même escouade peuvent occuper la même case sur deux étages (socle rendu REVIVED sous une survivante à l'étage, pile-in inter-étage) ; aplaties au sol elles se heurtaient sur TOUTE destination, et l'érosion, qui supposait la collision intra-plan invariante par translation, offrait quand même ces cellules — `ValueError « collision intra-plan … (dont 1#r0) »`, gate de P1 du 2026-09-17. Le coût de descente (`def squad_descent_penalty_subhex`) reste facturé au pire cas à toute l'escouade (max des hauteurs) : conservateur pour une escouade qui reste sur son étage, jamais sous-facturé. Verrous : `test_squad_move_descent_frontier.py` (`_gs_stacked`, hex), `test_rigid_plan_stacked_levels.py` (pile-in, euclidien), `test_returned_models_placement.py` (REVIVED sous une survivante à l'étage).

**Descente 13.06 en métrique HEX sur le chemin par-figurine PvP (2026-09-18).** `def movement_build_model_destinations_pool` ([movement_handlers.py](../../../engine/phase_handlers/movement_handlers.py)) n'amputait pas le sol d'une figurine partie d'un étage en métrique hex : le BFS planaire offrait le sol à M plein (mesuré sur la checklist x1 : 6 cases pour M = 6" et un plancher de 3"), là où la branche euclidienne (x5) facture la descente par le champ multi-niveaux. Branche `elif (not has_fly) and _floor_start` : le SOL est le BFS hex de plain-pied (`_hex_planar_reach`) amputé de la hauteur du plancher de départ (`floor_height_at` × `inches_to_subhex`, arrondi au supérieur — la grandeur de `def squad_descent_penalty_subhex`) ; l'ÉTAGE VU vient de `def ascent_field_for_model` seedé au niveau de la figurine (rester sur son plancher est horizontal ; le plancher d'une autre ruine ou un étage plus haut paie descente et montée), et ses cases sont celles que le commit résout à l'étage (`validate_floor_placement`) — une case de bord que seule l'appartenance hex tague « étage » est du sol, donc bornée. La validation PvP (`movement_preview_move_plan`, budget M au niveau du plan) reste plus permissive que le pool : masque ⊆ exécutable. FLY déclaré (21.03) ignore toute distance verticale (frontière M − 2, `test_a_flying_squad_on_a_floor_pays_the_skies_not_the_descent`). Verrou : `tests/integration/pvp/checklist/test_move.py::TestTerrainMovement1306::test_infanterie_redescend_en_payant_la_descente` (x1 et x5, vue sol et vue étage).
