...
| `tests/unit/ai/test_step_log_weapon_rule_tokens.py` | 77 | **Chaîne moteur → step.log → analyzer**, TIR ET MÊLÉE, pour toutes les règles d'armes que le journal nomme : [DEVASTATING WOUNDS], [HEAVY], [RAPID FIRE], [COVER], [SUSTAINED HITS], [MELTA], [PRECISION], [BLAST], [CLEAVE], les abilités de relance, et depuis le 2026-08-12 les six dernières ([TORRENT], [LETHAL HITS], [IGNORES COVER], [EXTRA ATTACKS], [ANTI-X:Y+], [PSYCHIC]). C'est le maillon de jonction que rien ne testait (V11 §0hist.38), et le seul endroit où l'on constate qu'un token franchit réellement les quatre maillons. Deux motifs d'échec y sont verrouillés nommément : une attaque **sans jet** (`Hit None(None+)` de 24.37, `Wound None(T+)` de 24.23) ne se distingue d'une ligne malformée que par son marqueur ; et le `X` d'un token porte le paramètre **déclaré par l'arme**, jamais le calcul du moteur — sinon le contrôle vérifie le moteur avec la réponse du moteur |
...
| `test_reward_calculator.py` | 11 | `_determine_winner` ; `_calculate_on_objective_reward` — le bonus « sur objectif » n'est PAS versé à une escouade battle-shocked (01.07 : OC de toutes ses figurines à '-', elle ne peut rien prendre), et il se lit **zone par zone** : le montage à deux objectifs (index 0 déjà contrôlé, index 1 neutre) verrouille que le filtre de contrôle porte sur la zone TOUCHÉE et non sur la première de la liste, et qu'il regarde TOUTES les figurines et non la seule ancre. Vérifié par mutation (retirer le filtre de contrôle, arrêter la lecture à la 1ʳᵉ figurine → rouge). Compte relevé par collecte réelle : le 23 précédent, et la mention de `_calculate_wound_target` / `_calculate_expected_damage` (absents de ce fichier), étaient périmés |
...
| `replayParser.test.ts` | 26 | Parsing de replays, et surtout le **jeu FERMÉ de tokens** : le parseur traite tout token accolé à un jet comme le nom d'une capacité d'unité, sauf ceux qu'il connaît comme règles d'armes. Un token de règle ajouté côté moteur sans son entrée ici ressort en capacité inexistante — déjà payé avec `[R:+0.0]`, et re-vérifié pour les six règles entrées au journal le 2026-08-12 |
...
### Anomalies connues (convention ANOM-XXX)

Une anomalie documentée mais non corrigée reçoit un **test sentinelle** marqué
`@pytest.mark.anomaly` (marqueur déclaré dans la config pytest) : le test met en scène le
comportement défectueux et passe en `xfail` tant que l'anomalie vit ; le jour où elle est
corrigée, le `xfail` devient inattendu et force la mise à jour. Utilisé par
`tests/unit/services/test_endless_duty_is_broken.py` (`pytestmark = pytest.mark.anomaly`) et
`tests/unit/engine/test_weapon_parser.py`. Origine et registre historique ANOM-001/002 :
`Documentation/Archives/docs/KNOWN_ANOMALIES.md` (archivé — les entrées y sont closes, la
convention, elle, reste en vigueur et est documentée ici).
