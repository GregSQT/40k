...
>
> Les lettres **o_intent** et **p** ne sont plus attribuées : `o_intent_zone_steps` et `p_intent_control_dependency` ont été retirées le 2026-09-09 avec la famille d'actions qu'elles mesuraient (intentions de zone), de même que les sept `combat/intent_*`. `o_robust_current_score` garde sa lettre — la collision de préfixe qui insérait cette courbe entre les deux métriques d'intent disparaît d'elle-même, et redécaler les survivantes casserait la comparabilité avec les runs archivés.
