...
- **d–e** : Performance d'épisode lissée sur `perf_window` (win rate, reward)
...
> La lettre **d** a été rendue au win rate le 2026-09-10, après deux jours de retrait : elle est écrite depuis `log_episode_end`, sous la même garde `winner is not None` que sa jumelle `game_critical/win_rate`, et non depuis `log_critical_dashboard` où elle vivait avant — de là, un épisode sans vainqueur lui donnerait un point que sa jumelle n'a pas, et les deux séries divergeraient. Verrou : `tests/unit/ai/test_metrics_single_writer.py::test_le_win_rate_dentrainement_a_sa_jumelle_00_critical`. La lettre **e** est revenue le même jour, écrite au même endroit, juste après l'append de `all_episode_rewards` : elle n'a AUCUNE jumelle — `game_critical/episode_reward` porte la valeur brute d'un épisode, jamais son lissage —, donc son retrait avait supprimé la mesure et pas seulement un nom. Verrou : `tests/unit/ai/test_metrics_single_writer.py::test_la_reward_lissee_du_dashboard_critique_couvre_tous_les_episodes`, dont l'épisode sans vainqueur interdit de la brancher sous la garde de sa voisine.
>
> Les lettres **o_intent** et **p** ne sont plus attribuées : `o_intent_zone_steps` et `p_intent_control_dependency` ont été retirées le 2026-09-09 avec la famille d'actions qu'elles mesuraient (intentions de zone), de même que les sept courbes `intent_*` du namespace combat. `o_robust_current_score` garde sa lettre — la collision de préfixe qui insérait cette courbe entre les deux métriques d'intent disparaît d'elle-même, et redécaler les survivantes casserait la comparabilité avec les runs archivés.
...
| **00_critical/0_gap_p1-p2** | ≈ 0 attendu APRÈS correction du siège structurel, **pas** à brut. ⚠️ DEPUIS LE 2026-09-19 LE SIGNE EST INVERSÉ : la mission compense désormais le second joueur à tous les rounds, et le jeu lui-même donne 13,0 points d'avance au SECOND joueur sur `terrain-mc1` (tout le holdout) et 18,4 sur `terrain-mc2`, bot contre le MÊME bot, sans politique apprise ; avant cette date il donnait 19,0 et 8,9 points au PREMIER joueur, et aucun chiffre de siège antérieur ne se compare à un postérieur ([plafonnement_p1.md#ecart-siege-2026-09-19](../../Chantiers/backlog/plafonnement_p1.md#ecart-siege-2026-09-19)) | **< −0.10** (meilleur en jouant SECOND) : cas inattendu, vérifier d'abord la récompense d'objectif de fin de tour — un agent avantagé au second siège suggère que le dernier tour rapporte trop | **> +0.10** (meilleur en jouant PREMIER) : `agent_seat_mode` doit rester `random` (le forcer à `p1` supprime la mesure, pas l'écart). ⚠️ NE PAS lire cet écart comme un défaut du modèle avant d'avoir retranché le siège STRUCTUREL du terrain joué (référence ci-contre) : la cause mesurée était l'instant de marquage de la mission `objectives_control`, qui faisait marquer le second joueur à sa phase de commandement — donc après un tour adverse de plus que le sien — sauf au round 5 ; corrigé le 2026-09-19, la compensation vaut maintenant à tous les rounds. Sur-échantillonner le siège faible via `agent_seat_p2_ratio` (0.75) compense l'AGENT, jamais la cause, et sous-entraîne le siège fort ; vérifier que le siège minoritaire garde assez d'épisodes pour rester mesurable (`seat_aware/episodes_agent_p1` vs `_p2`) | `combined` au siège p1 − `combined` au siège p2 (même pondération que `a_bot_eval_combined`) | Symétrie de jeu. Seule métrique d'ÉVALUATION qui distingue un agent équilibré (0.75/0.75) d'un agent qui ne sait jouer que premier (1.00/0.50) : `a_bot_eval_combined` affiche 0.75 dans les deux cas. Absente si un seul siège est couvert |
...
| **00_critical/d_win_rate** | >0.50 | **< 0.50** : sortie, pas entrée — lire d'abord `k_entropy_loss` (proche de 0 = politique figée → `ent_coef` ↑) et `i_clip_fraction`, ensuite seulement les récompenses | **> 0.85** : l'adversaire d'entraînement est trop faible, l'écart avec `a_bot_eval_combined` va se creuser — `bot_training.ratios` vers `control` / `adaptive`, `random` ↓ | Win rate lissé sur `perf_window`, **jumelle exacte** de `game_critical/win_rate` (même source, même fenêtre, mêmes abscisses) | Performance contre l'adversaire d'entraînement — le win rate au milieu des courbes de santé PPO, sans changer de namespace en cours de diagnostic |
| **00_critical/e_episode_reward_smooth** | Tendance croissante | **Plate ou décroissante** : récompenses intermédiaires trop faibles — revoir la config de reward. Vérifier d'abord `q_reward_deploy_{active,auto}` : un agrégat plat sous deux séries croissantes, c'est la rampe de déploiement, pas un plafond | **Monte alors que `d_win_rate` stagne** : reward hacking — identifier la composante exploitée dans `01_VP/` et `02_combat/` avant de toucher aux coefficients | Reward d'épisode lissée sur `perf_window` — `game_critical/episode_reward` porte la valeur brute, cette courbe n'a aucune autre jumelle | Signal d'apprentissage |
...
| **00_critical/v_n_minibatches_done** | `n_epochs × ceil(n_steps × n_envs / batch_size)` — 32 sur `x1_lineage` (4 × 8) | **Sous le plan, récurrent** : PPO coupe ses epochs à ce pas — c'est le complément de `l_approx_kl_max`, qui dit SI l'update a été coupée quand ce compte dit OÙ. Lire la fraction `done / plan` avant tout réglage de `target_kl` ou `n_epochs` : à 8/32, trois epochs sur quatre sont collectées puis jetées | *(sans objet : borné par le plan)* | Pas de gradient (`optimizer.step()`) réellement exécutés dans l'update, valeur brute par update (axe `episode_count`) ; le mini-lot qui déclenche l'early-stop n'est pas compté | Décider `target_kl` / `n_epochs` sur un comptage, pas sur `train/time_update` (1,46–5,61 s sur `run_20260912-065925`, 761/761 updates coupées) |

> **Les métriques d'intention de zone n'existent plus** (2026-09-09). La famille d'actions elle-même a été retirée : son unique effet, la prime `zone_intent_shaping`, était débranchée depuis le 2026-08-11 sur mesure, et les actions coûtaient 1,6 à 5,2 steps sans effet par épisode. Neuf courbes partent avec elle — `o_intent_zone_steps`, `p_intent_control_dependency` et, dans le namespace combat, les trois marginales `intent_invade_ratio`, `intent_defend_ratio` et `intent_attack_ratio`, plus `intent_mutual_info_bits`, `intent_control_entropy_bits`, `intent_shaping_aligned_ratio` et son `intent_shaping_aligned_baseline`. Les runs archivés qui les portent restent lisibles ; aucun run postérieur n'en émettra. Comme les autres courbes retirées de ce document, elles sont citées sans leur namespace : le garde doc → code (`tests/unit/ai/test_metrics_doc.py`) lit tout tag namespacé cité ici comme une courbe encore émise, et un `namespace/tag` réintroduit dans une note d'historique le rendrait rouge.

> **Lettres libres** : `p`, depuis le 2026-09-09. `d` et `e` l'ont été deux jours (2026-09-08 → 2026-09-10) ; `l` porte `l_approx_kl_max` depuis le 2026-09-08 ; `v` porte `v_n_minibatches_done` depuis le 2026-09-12. `k` l'était jusqu'au 2026-09-06 : l'insertion de `g_grad_share_policy_mb0` a décalé `g`→`h`, `h`→`i`, `i`→`j` et `j`→`k`, décalage absorbé par le trou `k`/`l` — les tags `m` et suivants n'ont pas bougé. `k_gradient_norm`, retiré antérieurement (redondant avec `i_clip_fraction` + `j_approx_kl`), ne doit pas être confondu avec l'actuel `k_entropy_loss`.
...
`PoolEarlyStoppingCallback` (`ai/training_callbacks.py`) évalue le modèle en cours contre le pool tous les `early_stop.probe_every_episodes` épisodes d'étape (5 000 depuis le 2026-09-17 ; jusque-là la cadence suivait `bot_eval_freq`, celle de l'évaluation contre les bots), sur `gate.eval_episodes` parties du split holdout **par membre sondé**. Deux tags par membre : `pool_eval/vs_<membre>` (valeur brute) et `pool_eval/vs_<membre>_<probe_window>ep` (moyenne glissante sur `early_stop.probe_window` sondes).
...
**Instantanés à seuils et plateau (2026-09-17).** Au premier franchissement de chaque seuil de `early_stop.snapshot_thresholds` (0,50 / 0,60 / 0,70) par la sonde **brute** contre le champion, le modèle courant est copié sous `model_<agent>_<étape>_vs<champion>_<0XX>.zip` (+ pkl, contrat, run_state) — un niveau de difficulté daté, utilisable comme membre `archive` d'une étape suivante (ligne `📸 Instantané` du log). Le callback tient aussi l'historique des moyennes glissantes contre le champion, sur lequel `evaluate_pool_decision` lit un troisième verdict, le **plateau** (`early_stop.plateau` : patience de `patience_probes` sondes sans dépasser le meilleur score lissé de `min_delta`, jamais avant `min_episodes`) : sous les seuils de promotion, l'étape est refusée sans gate (verdict `plateau`, court-circuit identique à la destruction) ; au-dessus, promue. Une étape marquée `run_to_plateau: true` (run de CHECK : P1, P4, P6, P9) ne s'arrête plus à la promotion et continue jusqu'au plateau ou à `check_budget_cap`, pour rendre son plafond visible. `curriculum.log` porte `episodes_to_gate` : l'épisode d'étape de la première sonde où les planchers ont tenu (ligne `🎯 Planchers de promotion tenus`).
