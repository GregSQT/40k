...
**Cadence — plusieurs régimes.** Les cinq courbes de santé PPO (`f_loss_mean`, `g_explained_variance`, `h_clip_fraction`, `i_approx_kl`, `j_entropy_loss`) et les lignes `thresholds/*` qui les annotent portent **un point par update PPO**. Les courbes d'évaluation bot (`a/b/c`) portent **un point par éval bot**. Les tags ventilés par mode de déploiement (`p–s`) ne portent un point que sur les épisodes du mode concerné. La plupart des autres courbes du namespace portent **un point par épisode**. Le curseur de lissage de TensorBoard compte des points : sur ces cinq-là, une position du curseur se lit donc en updates, ailleurs en épisodes. Avant le 2026-09-06 elles étaient republiées à chaque fin d'épisode, soit ~74 copies de la même valeur (39 430 points pour 529 valeurs distinctes sur `run_20260906-123804`) — sur un run antérieur à cette date, lire l'escalier comme une suite de paliers, ou basculer sur les jumelles brutes `train/*`, qui n'ont jamais eu ce défaut mais ne sont ni lissées ni superposables aux seuils (axe des timesteps).
...
| **00_critical/m_immediate_reward_ratio_mean** | < 0.9 | **Très bas alors que le reward monte** : le signal vient surtout de la fin de partie — rien à corriger tant que `bot_eval` suit | **> 0.9** : myopie, l'agent optimise les dégâts et pas la victoire → augmenter win/lose (20 → 40 ou 50), réduire les récompenses intermédiaires, vérifier `gamma` | Part du reward immédiat dans le reward total, moyennée sur une fenêtre glissante (`immediate_reward_ratio_history`) | Diagnostic de myopie — voir « Myopie » dans les problèmes courants |
