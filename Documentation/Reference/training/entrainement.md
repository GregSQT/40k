...
```
> `Actions` = lignes d'action écrites · `Steps` = lignes incrémentantes (un par jet de tir ou de mêlée : `W40KEngine._flush_squad_action_logs_to_step_logger` appelle `step_logger.log_action` avec `step_increment=True` une fois par entrée de `shootDetails`) · `Total` = `game_state['episode_steps']` : actions/activations par figurine de l'épisode (deux sites qui incrémentent ce compteur — `W40KEngine.step_with_mask` pour le step gym, `generic_handlers.end_activation` pour la fin d'activation)
...
| `advantage_source` | `gae` (défaut) | `q_head` | Avantage de l'acteur : `gae` = avantage GAE du rollout (historique, bit à bit) ; `q_head` = avantage ATTENDU `A_c(s, a)` de la tête Q de `PointerMaskablePolicy` (S14, dossier plafonnement_p1.md §5.13), CENTRÉ sous π (`center_under_policy` : `A_c = A − Σ_a π(a|s)·A(s, a)`, donc Σ_a π·A_c = 0 par état — aucune constante par état ne peut se loger dans la tête, le biais de V reste à V ; mesuré le 2026-09-15 sur le run non centré : 98 % de Var(A(s, a_jouée)) était un offset par état) — différence de deux espérances apprises au lieu de l'écart porté par le jet de dés de ce pas. Exige `q_coef` ; sur un zip chargé sans tête Q, exige `value_warmup_updates ≥ 1` (la tête apprend d'abord, politique figée). Tête Q = mêmes têtes que la politique sur le tronc critic, `Q = V.detach() + A_c`, régressée sur le retour λ ; V garde sa cible. |
...
| `logits_temperature` | `1.0` (défaut) | `1.5`–`3` | Température des logits de la politique (S9, dossier plafonnement_p1.md §9.9 étape 2b) : `π_T = softmax(logits / T)` sur les légales, appliquée dans `PointerMaskablePolicy._distribution_from` AVANT le masquage, donc à la collecte (workers) ET dans le ratio PPO / l'entropie de `train` — même π_T des deux côtés, PPO optimise π_T. Régime de RUN : attribut hors zip, posé par le constructeur (`--new`) ou le profil (`--append`) ; un zip rechargé repart à 1,0 (sondes, holdout, adversaires figés, PvE jouent T = 1 ; l'argmax est invariant à T). Refusé sur une politique qui ne tempère pas (`MlpPolicy`). Limite : absorbable par l'échelle des logits, à lire sur `train/entropy_loss`. |
| `logits_temperature_regulation` | `null` (T fixe) | `{"entropy_target": 1.0, "gain": 0.1, "t_min": 1.0, "t_max": 4.0}` | Température AUTORÉGULÉE (option 2, dossier plafonnement_p1.md, codée le 2026-09-18) : à chaque update, `T ← clip(T · exp(gain · (entropy_target − H)), t_min, t_max)` où `H = −train/entropy_loss` (entropie de π_T, nats) — T monte quand l'entropie est sous la cible, descend au-dessus (`ai/pointer_policy.py::regulate_logits_temperature`, appliquée en fin de `PatchedMaskablePPO.train`, écrite sur la politique donc transportée aux workers au rollout suivant). Tag `train/logits_temperature` = T qui a produit l'update. Quatre clés obligatoires, aucune valeur par défaut ; `null` = T fixe (régime S9), la T de départ reste `logits_temperature`. Régime de RUN hors zip (`--new` : constructeur ; `--append` : profil) — la T ATTEINTE non plus n'est pas dans le zip : une reprise (`--resume-from` après crash, `--append`) repart de la T de départ du profil et la boucle reconverge (transitoire de quelques dizaines d'updates ; persister T contredirait « un zip rechargé joue T = 1 ») ; `scripts/grad_signal_probe.py` refuse un profil régulé, la T du checkpoint se lit sur le tag. Limite à écrire dans la règle de lecture d'un run : le réseau absorbe T (S9 : 1,07 → 0,72 nat en ~40 000 parties), donc T monte jusqu'à `t_max` et la boucle devient une T fixe à 4 — « T saturée » est un résultat possible. Tests `tests/unit/ai/test_logits_temperature_regulation.py`. |
...
| `n_steps` | **8160** | 8160 |
| `vf_coef` | **0.17** | 0.5 |
| `agent_seat_p2_ratio` | **0.75** depuis le 2026-09-16 (0.7 du 2026-09-11 au 2026-09-16, 0.6 avant) | 0.75 |
...
| `train/` | `q_loss` / `adv_q_abs_mean` / `adv_q_offset_abs_mean` / `advantage_source_q` | Tête Q (S14) : perte de régression de `Q = V + A_c` sur le retour λ, amplitude moyenne de `A_c(s_t, a_t)` (centré sous π), amplitude moyenne de l'offset par état retiré par le centrage `Σ_a π·A`, et 1 si l'acteur lit la tête Q (0 = GAE). NaN / 0 sous `advantage_source: gae`. `diag/grad_norm_q_mb0` : norme du gradient du terme Q, à côté des trois autres. |
| `train/` | `q_loss_mb0` / `value_loss_mb0` | Pertes Q et V (V BRUTE, non clippée) du PREMIER mini-lot de l'update, AVANT tout pas d'optimisation : la seule comparaison HORS ÉCHANTILLON par construction (`train/q_loss` et `train/value_loss` moyennent sur les epochs, donc mélangent in-sample et hors-sample selon la coupure KL). Règle §5.13 : `q_loss_mb0 − value_loss_mb0 < 0` après l'échauffement = la tête centrée prédit une part du résidu de V sur des retours jamais vus. Erreur-type ~0,001 par update : se lit sur une fenêtre ≥ 10 updates. `q_loss_mb0` NaN sous `gae` ; `value_loss_mb0` publié sous toute source. |
