...
Effet immédiat du passage en liste noire : `frontend/src/roster/_HOW_TO_ADD_UNIT.py` a fait échouer le scan — c'était du **TypeScript sous extension `.py`**. `pyrightconfig.json` avait dû l'exclure pour la même raison. Renommé en `.md`, et l'exclusion pyright supprimée : la cause est traitée, pas contournée deux fois.
