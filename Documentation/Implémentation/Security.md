...
| 6. Analyse statique | F5 | ✅ Fait — `scripts/security_check.sh` exécuté, sortie 0 ; traités : 3 bandit HIGH, Flask 3.1.3, 10 npm high/critical, `scipy` manquant en production, `users.db`/`.claude` cuits dans l'image ; seules les 19 CVE torch restent acceptées, avec justification écrite | 2026-08-10 |
