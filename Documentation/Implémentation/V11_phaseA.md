...
- ~~**Code mort** : `_attack_sequence_rng`~~ — **SUPPRIMÉ le 2026-07-28** (§0hist.38), avec ses
  ...
  concurrente sur ce fichier (§0.40). Les branches `execute_action` « squad path expected »
  sont **conservées** : le dispatcher est vif ([w40k_core.py:6157](../../engine/w40k_core.py#L6157)),
...
| Suppression du code mort (fin P1) | ✅ **FAIT** (2026-07-28, §0hist.38) | `_attack_sequence_rng` (180 lignes) + 7 états `_rapid_fire_*` + helper `_get_rapid_fire_parameter` supprimés de `shooting_handlers` ; les 6 fichiers de tests qui l'appelaient re-pointés sur le chemin vif. Reliquat `w40k_core` (7 clés de purge, log de debug, champ `rapid_fire_bonus_shot`) laissé à l'agent §0.40 qui édite ce fichier en parallèle. | Les 6 fichiers migrés + les 6 fichiers vifs complétés = **109 tests verts** ; contre-épreuve : **7 mutations** du vif (05.02, 24.16, 24.38, 24.15, 05.04, CTP, 24.10) → 7 rouges, restauration → vert. |
