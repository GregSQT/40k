...
| 39quater | **`DEVASTATING_WOUNDS` reste NON COMPTÉE en mêlée** (5 armes concernées), et c'est un défaut du LOG, pas du compteur : le formateur de mêlée de `step_logger.py` n'écrit pas `Save [DEVASTATING WOUNDS]` là où celui du tir l'écrit. Il imprime un jet de sauvegarde même quand le moteur l'a sautée (24.10 : « no saving throw can be made »), le socle de résolution étant partagé. À traiter côté log avant de pouvoir compter quoi que ce soit ici | `step_logger.py` (branche `FOUGHT`) |
