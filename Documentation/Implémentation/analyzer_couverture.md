...
| 41ter | **attaque non allouée alors que la cible est VIVANTE** — `Save [NOT ALLOCATED]` recoupé avec l'état reconstruit de la cible. Le cas légitime (cible détruite, « excess attacks lost », 05) n'est PAS compté : le compter noierait la faute sous 1 429 lignes normales. Contrôle créé le 2026-08-11, **et il s'est allumé immédiatement : 334 occurrences** (146 au tir, 188 en mêlée) sur le run du 2026-08-11, avec §2.8 à 1 sur 600 épisodes — l'état reconstruit ne les explique pas. Vérifié à la main sur E79 T2 : l'allocation REPREND après une attaque non allouée (une ligne ultérieure inflige 2 PV), donc la cible vivait bien. **Sujet MOTEUR ouvert, pas un défaut de mesure.** Verrou : `tests/unit/ai/test_analyzer_not_allocated.py` (faute construite ET cas légitime silencieux) | `analyzer_allocation.py`, appelé depuis `shoot_handler.py` et `fight_handler.py` |
