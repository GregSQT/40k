...
| 41ter | **attaque non allouée alors que la cible a SURVÉCU À L'ACTIVATION** — `Save [NOT ALLOCATED]` recoupé avec l'état de la cible à la FIN de l'activation de l'attaquant. Le cas légitime (cible détruite, « excess attacks lost », 05) n'est pas compté : le compter noierait la faute sous 1 429 lignes normales. **0/0 sur le run du 2026-08-11** (rejoué au format corrigé) : les 1 429 attaques perdues visaient toutes une cible morte. ⚠️⚠️ **Le verdict NE PEUT PAS être rendu sur la ligne** : l'ordre des LIGNES n'est pas l'ordre d'ALLOCATION (pool trié par jet de sauvegarde croissant 05.04, lots enchaînés par profil d'arme 04.03). La première version jugeait à la lecture et a rendu **334 fausses erreurs**, à deux doigts d'être rapportées comme un défaut moteur — E79 T2 : le lot Blitzcannon tue la cible, le lot Rokkit Launcha entier est perdu, y compris une sauvegarde de 1 qui aurait été allouée en premier. Verrou : `tests/unit/ai/test_analyzer_not_allocated.py`, dont un test construit exactement ce piège (la ligne fatale doit être ≥ 2 lignes après, sinon les deux versions rendent le même verdict et le test ne prouve rien) | `analyzer_allocation.py`, appelé depuis `shoot_handler.py`, `fight_handler.py`, `episode_handler.py` et `analyzer_core.py` (vidages) |
...
| 03.03 Coherency (2" / 9") | **#68** — `analyzer_core._check_line_coherency`, mesure `analyzer_perfig.squad_coherency_offenders` | `[MODELS:]` + trois seuils dans `Run rules:` (`cohesion.*`, ajoutés au StepLogger le 2026-08-10) | **COUVERT (2026-08-10)** — jugé à la MISE EN PLACE et à la fin de CHAQUE déplacement (les 6 moves + `DEPLOYED`), une faute par formation. La mesure est per-socle (empreinte à empreinte) ; le VERDICT est délégué à `_coherency_verdict` du moteur, parce que la 1re puce est une CONNEXITÉ, pas un « au moins un voisin » — deux paquets disjoints doivent tomber. Non couvert : le volet VERTICAL (5"). La purge End of Turn est journalisée depuis le 2026-08-12 : sans sa ligne, la figurine retirée restait « vivante » pour tout lecteur du journal — elle engageait ses ennemis et bloquait leurs chemins jusqu'à la prochaine action de son escouade (mesuré sur E485 du run du 2026-08-12 : un « advance from adjacent » ET un « advance au-delà du budget », tous deux inventés) |
