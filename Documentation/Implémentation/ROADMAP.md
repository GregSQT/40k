...
- ✅ **Livraison 2026-08-12 — trois verrous de test, aucun changement de comportement moteur.**
  ...
  ✅ **Cause racine fermée le 2026-08-12** (décision utilisateur, option B) :
  `consider_namespace_packages = true` dans `pytest.ini`. Chaque fichier de test est désormais
  nommé par son chemin complet, donc un fichier = un module, sans semer 13 `__init__.py`.
  Contrepartie payée dans la même livraison : le dossier d'un test n'est plus sur `sys.path`, donc
...
9. **Refonte du panel de bots** (ouvert le 2026-08-11) — six styles, échelle de difficulté.
   ...
   **Nouvelle ligne de base : `combined = 0,7433`, pire bot `racer = 0,630`, pire scénario 0,6867.**
   ...
   ✅ **Nettoyage configs livré le 2026-08-17** : profils ramenés à 6 actifs (`x1`/`x1_long`/`x1_debug` + `x5_new`/`x5_long`/`x5_debug`) — les 5 profils `x1_selfplay`, `x1_panel`, `x1_long_panel`, `x1_new_bots`, `x5_append` supprimés ; `tactical:0.0` ajouté dans `bot_eval_weights` des 6 profils (critère §10.6) ; 29 fichiers de test mis à jour.
   🔄 **Étape 8 en cours le 2026-08-17** : run `x1_long --new --resolution 1` lancé (modèle précédent incompatible obs P3-4). `--test-only --step` à faire à la fin pour rejouer la ligne de base du panel.
...
- ✅ **La clairance sous les étages se mesure à la hauteur de LA FIGURINE** — ouvert et **LIVRÉ le
  ...
  **Effet aujourd'hui : 0** — aucune des 684 figurines du dépôt ne porte une hauteur différente de
  ...
  Complété le même jour par les arbitrages de la `/simplify` : `low_clearance_ground_hexes` prend
  **la figurine ET son escouade** au lieu d'un `float` nu, et `_model_height_of` — seule source de
  l'héritage figurine→escouade — REFUSE une entrée qui n'est pas une figurine (marque `squad_id`).
  La faute n'est plus surveillée par un test qui relisait le texte des handlers : elle n'est plus
  écrivable, et le contrôle placé sur l'héritage couvre AUSSI les six autres appels qui ne
  l'avaient pas (déploiement ×2, charge, entrée synthétique d'engagement, LoS 3D ×2). Un builder
  d'état synthétique partagé (`tests/unit/engine/_state_builders`) remplace au passage les trois
  copies du même harnais de test.
