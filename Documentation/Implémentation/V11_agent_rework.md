...
| **§0.29** | Scénario SM vs Orks fixed/active + scheduler | 🟢 **USAGE CONFIGURÉ** le 2026-07-28 (`active_ratio_end` 0.0 → **0.8**, commit `acd63b66`) — la mesure devient possible | 5 | Le run 3 joue donc une part **croissante** d'épisodes en `active` (phase de déploiement), de 0 % au début à 80 % à la fin (`p_active = start + (end−start)·progress`, [w40k_core.py:934](../../engine/w40k_core.py#L934)). Mesure attendue : win-rate en régime `fixed` vs `active` au fil de la rampe. 🔴 **RÉSERVE D'INTERPRÉTATION** : l'observation du déploiement est défaillante (§0.40, défauts 1 et 2 vérifiés, non corrigés) — l'agent apprend à déployer quasi à l'aveugle, donc un mauvais résultat en `active` ne permettra pas de distinguer la politique de l'observation. Corriger §0.40 avant d'en tirer une conclusion. |
