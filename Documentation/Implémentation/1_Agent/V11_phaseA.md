...
| ~~~10 règles observées non appliquées~~ | ✅ **SOLDÉ** — il n'en reste **aucune** | Les 9 règles listées au 2026-07-24 sont traitées : TORRENT, TWIN_LINKED, SUSTAINED_HITS, LETHAL_HITS, MELTA, ANTI_* ✅ ; EXTRA_ATTACKS ✅ ; PSYCHIC ✅. **INDIRECT_FIRE (24.19) était la seule restante ; son chantier a été ouvert et est aux 5/6 le 2026-08-16** (`A_faire/indirect_fire_10_07.md`). Ce n'est pas une règle de résolution mais un TYPE DE TIR entier (10.07), donc un chantier à part et non un portage — ce qui s'est confirmé : il a fallu faire du type de tir une DÉCISION (10.02), donc changer l'espace d'action. 2 armes concernées, aucune dans les rosters de training. ⚠️ Dépendance déjà notée : quand elle sera faite, son couvert devra passer par un point qu'[IGNORES COVER] 24.18 annule aussi. | — |
...
**Statut : ✅ FERMÉ le 2026-07-26 par la tranche **T-B** du chantier dédié [`V11_entity_encoder_pointer.md`](../Implémenté/V11_entity_encoder_pointer.md) (tranche T-B), avec 5 autres trous trouvés le même jour.** Le résolveur `resolve_squad_shooting_type` ouvre 10.05 et 10.06, le masque teste toute arme éligible, et le volet MONSTER/VEHICLE de 10.06 est implémenté (13 tests, mutations → 5 rouges). ⚠️ **Résidu** : le chemin PvP/mono ne connaît toujours pas le volet MONSTER/VEHICLE — voir §1.9 du chantier. Périmètre d'origine — c'est une **nouvelle règle à implémenter**
...
1. ✅ **LIVRÉ le 2026-07-28 (mergé sur `main` le soir même) — détail → [§0.41](V11_agent_rework.md#s0.41).** La cible de
   ...
   **Cible de mêlée** — ⚠️ **MIS À JOUR le 2026-07-16 (le fix du bug `squad_fight` a déplacé ce
   ...
   partagé, arbitrage requis (cf. `A_faire/pile_in_overrun_par_figurine.md` §3).
