...
| 4.3 | **Mesure de clôture** : durée gate ; calibrage tranches — `bot_eval_n_workers` fixé à **6** (était 16 → OOM) ; 1re mesure n=2 : wall=4 258 s, ~28 s/ép série, ~35 min estimé à n=6 (§6). **Clôture formelle : 3 reps `duration 6` machine au repos** (médiane) | — | 🟡 |
