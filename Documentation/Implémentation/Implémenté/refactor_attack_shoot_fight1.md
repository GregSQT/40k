...
- allocation des pertes conforme PDF 05.03/05.04, validée par test PvP manuel (C, Q.1) ;
...
**Plan ordonné (risque croissant, validation par conformité règle + test PvP manuel à chaque étape ; `--step`/training indisponible car cassé) :**

| # | Étape | Risque | Validation |
|---|---|---|---|
| 1 | Extraire le module d'allocation des pertes en fonction ctx-paramétrée (ordre conforme 05.04, Q.1 inclus d'emblée) ; brancher shoot via wrapper `SHOOT_CTX` | Faible | Test PvP tir manuel : allocation conforme PDF 05.03/05.04 |
...
| 6 | Non-régression gym : vérifier que le manuel ne touche jamais le chemin auto V11 (branche `_is_fight_auto_execution_allowed==False` uniquement) | Faible | Relecture structurelle ; run training reporté tant que le training est cassé — chemin auto + reward inchangés (E, F) |

Étapes 1, 2, 6 = risque faible (l'étape 1 change l'ordre d'allocation tir, volontairement, par conformité 05.04) ; le risque se concentre en 3-4.
...
### Q.3 — Hidden (dette TIR, hors scope combat) → **B2**
...
**Perf** : ≈ B1. Surcoût = distances modèle↔modèle O(Na×Nh) (~100 ops triviales/paire concernée), négligeable devant la géométrie LoS inchangée ; early-exit si la cible n'a pas de `hidden_models`. **Piège à éviter** : ne PAS recalculer la LoS géométrique par tireur (multiplierait les rayons ×Na et n'est pas requis par 13.09).

---

## R. Dettes différées (hors scope de l'implémentation immédiate)

Points identifiés en revue, **volontairement reportés à un second temps**. Aucun ne bloque le plan § P.

### R.1 — §04.03 SPLITTING MELEE ATTACKS (attribution multi-cibles) — DIFFÉRÉ

PDF 04 §04.02 : en mêlée, si une figurine cible **plusieurs unités** avec la même arme, il faut
**déclarer combien d'attaques vont à chaque cible (≥1 par cible) avant les jets**. Idem au tir.
Le plan § P (étapes 2-3) couvre l'attribution **cible/arme par figurine** mais **pas** la répartition
du nombre d'attaques entre plusieurs cibles d'une même figurine.
➡️ **Différé** : première version = 1 figurine → 1 cible. La répartition multi-cibles sera ajoutée
dans la couche attribution (commune tir+combat) dans un second temps.

### R.2 — Cover −1 BS (dette TIR pure) — DIFFÉRÉ (cf. Q.2)

Règle 13.08 : Benefit of Cover = **−1 BS**, ranged only, niveau unité tout-ou-rien. Détection code
conforme mais **effet jamais appliqué** au hit roll. **Sans aucun lien avec le refactor combat**
(inexistant en mêlée). ➡️ **Différé**, à traiter dans un chantier TIR séparé.

### R.3 — Hidden visibilité par-modèle (dette TIR pure) — DIFFÉRÉ (cf. Q.3, décision B2)

Règle 13.09 : modèle hidden visible uniquement aux ennemis à ≤15". Code actuel = exclusion **niveau
unité** ; `hidden_models` par-figurine calculé mais inutilisé. Décision B2 (visibilité par-modèle)
actée mais **hors scope combat**. ➡️ **Différé**, même chantier TIR séparé que R.2.

> Note de périmètre : R.2/R.3 sont des dettes **tir** découvertes en revue ; elles n'impactent pas
> l'alignement tir/combat objet de ce doc. À terme, les extraire dans un doc « audit visibilité/cover tir » dédié.
