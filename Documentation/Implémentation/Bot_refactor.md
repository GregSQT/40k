...
>
> **Convention** : par le §Rôle du [`ROADMAP.md`](ROADMAP.md), un chantier ouvert vit dans
> `A_faire/`. Ce fichier est à la racine parce que son chemin a été demandé explicitement ; c'est
> la même exception que `Security.md`, signalée par le ROADMAP. Un `git mv` + 1 lien le corrige.
>
> **Ce document n'est PAS la source de l'ordre du travail** : c'est [`ROADMAP.md`](ROADMAP.md).
> Il porte le CONTENU d'un chantier, pas sa priorité.
...
- [`1_Agent/V11_agent_rework.md`](1_Agent/V11_agent_rework.md) — §0.55 gel de `tactical`,
