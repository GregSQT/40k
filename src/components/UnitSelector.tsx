...
//
export default function UnitSelector({ units, currentPlayer, selectedUnitId, onSelect, unitsMoved, unitsCharged = [], unitsAttacked = [], phase = "move" }: Props) {
  console.log("UnitSelector props", { phase, unitsCharged, unitsAttacked });
  // Filter units according to phase eligibility
  const filteredUnits = units.filter(unit => {
    const reason: string[] = [];
    if (unit.player !== currentPlayer) { reason.push("not current player"); return false; }
    if (phase === "move") {
      if (unitsMoved.includes(Number(unit.id))) { reason.push("already moved"); return false; }
      return true;
    }
    if (phase === "shoot") {
      if (unitsMoved.includes(Number(unit.id))) { reason.push("already shot"); return false; }
      ...
      if (!inRange) { reason.push("no enemy in range"); return false; }
      return true;
    }
    if (phase === "charge") {
      if (unitsCharged.includes(Number(unit.id))) { reason.push("already charged"); return false; }
      const enemies = units.filter(u2 => u2.player !== currentPlayer);
      const isAdjacent = enemies.some(eu => Math.max(Math.abs(unit.col - eu.col), Math.abs(unit.row - eu.row)) === 1);
      if (isAdjacent) { reason.push("enemy adjacent"); return false; }
      const inRange = enemies.some(eu => Math.max(Math.abs(unit.col - eu.col), Math.abs(unit.row - eu.row)) <= unit.MOVE);
      if (!inRange) { reason.push("no enemy in move range"); return false; }
      return true;
    }
    if (phase === "combat") {
      if (unitsAttacked.includes(Number(unit.id))) return false;
      const enemies = units.filter(u2 => u2.player !== currentPlayer);
      const isAdjacent = enemies.some(eu => Math.max(Math.abs(unit.col - eu.col), Math.abs(unit.row - eu.row)) === 1);
  ...
  return (
    <div style={{
      ...
      <b style={{ marginBottom: 8 }}>Select your unit</b>
      {filteredUnits.map(unit => (
        <button
          ...
          disabled={
            (phase === "move" && unitsMoved.includes(Number(unit.id))) ||
        ...
        >
          <span style={{ marginRight: 6, fontWeight: 700 }}>{unit.name}</span>
          <span style={{ fontSize: 11, opacity: 0.7 }}>{unit.type}</span>
        </button>
      ))}
