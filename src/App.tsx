...
export default function App() {
  ...
  const [unitsMoved, setUnitsMoved] = useState<number[]>([]);
  const [unitsCharged, setUnitsCharged] = useState<number[]>([]);
  const [unitsAttacked, setUnitsAttacked] = useState<number[]>([]);
  const [mode, setMode] = useState<Mode>("select");
  const [phase, setPhase] = useState<"move" | "shoot" | "charge" | "combat">("move");
  ...
  function handleSelectUnit(unitId: number | string | null) {
    if (unitId === null) {
  ...
  function handleStartMovePreview(unitId: number | string, col: number | string, row: number | string) {
    unitId = Number(unitId);
    col = Number(col);
    row = Number(row);
    const unit = units.find(u => u.id === unitId);
    if (!unit || unit.player !== currentPlayer || unitsMoved.includes(unitId)) return;
  ...
  function handleStartAttackPreview(unitId: number, col: number, row: number) {
  ...
  function handleConfirmMove() {
    let movedUnitId: number | null = null;

    if (mode === "movePreview" && movePreview) {
      setUnits(prev =>
        prev.map(u =>
          u.id === movePreview.unitId
  ...
  function handleShoot(shooterId: number, targetId: number) {
  ...
  function handleCombatAttack(attackerId: number, targetId: number | null) {
    ...
    if (unitsAttacked.includes(attackerId)) return;
    // Must be adjacent
  ...
  function handleCharge(chargerId: number, targetId: number) {
  ...
  function handleMoveCharger(chargerId: number, destCol: number, destRow: number) {
    setUnits(prev =>
      prev.map(u =>
        u.id === chargerId
  ...
  function handleValidateCharge(chargerId: number) {
    setUnitsCharged(prev => prev.includes(chargerId) ? prev : [...prev, chargerId]);
    setSelectedUnitId(null);
    setMode("select");
  }
  ...
  function handleSelectCharger(unitId: number | string | null) {
    ...
    unitId = Number(unitId);
    const unit = units.find(u => u.id === unitId);
    if (!unit || unit.player !== currentPlayer || unitsCharged.includes(unitId)) return;

    // Eligibility: no enemy adjacent, at least one enemy within MOVE
    const enemyUnits = units.filter(u2 => u2.player !== currentPlayer);
    const isAdjacent = enemyUnits.some(eu =>
      Math.max(Math.abs(unit.col - eu.col), Math.abs(unit.row - eu.row)) === 1
    );
    const inRange = enemyUnits.some(eu =>
      Math.max(Math.abs(unit.col - eu.col), Math.abs(unit.row - eu.row)) <= unit.MOVE
    );
    if (isAdjacent || !inRange) return;
...
React.useEffect(() => {
  ...
  if (phase === "shoot") {
    ...
    const shootableIds = playerUnits.filter(u =>
      !unitsMoved.includes(u.id) &&
      enemyUnits.some(eu =>
        Math.max(
          Math.abs(u.col - eu.col),
          Math.abs(u.row - eu.row)
        ) <= u.RNG_RNG
      )
    ).map(u => u.id);
  ...
  if (phase === "charge") {
    ...
    const playerUnits = units.filter(u => u.player === currentPlayer);
    const enemyUnits = units.filter(u => u.player !== currentPlayer);
    function isAdjacent(unitA: Unit, unitB: Unit) {
      return Math.max(
        Math.abs(unitA.col - unitB.col),
        Math.abs(unitA.row - unitB.row)
      ) === 1;
    }
    const chargeableIds = playerUnits.filter(u => {
      if (unitsCharged.includes(u.id)) return false;
      if (enemyUnits.some(eu => isAdjacent(u, eu))) return false;
      return enemyUnits.some(eu =>
        Math.max(
          Math.abs(u.col - eu.col),
          Math.abs(u.row - eu.row)
        ) <= u.MOVE
  ...
  if (phase === "combat") {
    ...
    const eligibleIds = playerUnits.filter(u =>
      !unitsAttacked.includes(u.id) &&
      enemyUnits.some(eu =>
        Math.max(Math.abs(u.col - eu.col), Math.abs(u.row - eu.row)) === 1
      )
    ).map(u => u.id);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "row", width: "100vw", height: "100vh", background: "#222" }}>
      <UnitSelector
        units={units}
        currentPlayer={currentPlayer}
        selectedUnitId={selectedUnitId}
        onSelect={phase === "charge" ? handleSelectCharger : handleSelectUnit}
      ...
      <div style={{
        ...
        <div style={{
          ...
          <Board
            ...
            onSelectUnit={phase === "charge" ? handleSelectCharger : handleSelectUnit}
            onStartMovePreview={handleStartMovePreview}
            onStartAttackPreview={handleStartAttackPreview}
        ...
        <div style={{ marginTop: 20 }}>
          <b style={{ color: "#fff" }}>Current player:</b> <span style={{ color: "#aee6ff" }}>{currentPlayer === 0 ? "Player 1" : "Player 2"}</span>
          <br />
          <b style={{ color: "#fff" }}>Units moved:</b> <span style={{ color: "#aee6ff" }}>
            {units
