...
export const REWARDS_RANGED = {
    // Phase : Movement
    move_close: 0.2, // for moving closer to an enemy unit
    move_away: 0.4, // for moving away from an enemy unit
    move_to_safe: 0.6, // for moving to a position where the unit is not in range of any enemy unit
    move_to_rng: 0.8, // for moving to a position where the unit can attack an enemy at its maximum RNG_RNG distance
    move_to_charge: 0.2, // for moving to a position where the unit can charge an enemy at its maximum MOVE distance
    move_to_rng_charge: 0.3, // for moving to a position where the unit can shoot and charge an enemy at its maximum RNG_RNG or MOVE distance (taking the lowest)
    // Phase : Shooting
    ...
    // Phase : Charge
    charge_success: 0.2, //for successfully charging an enemy unit
    being_charged: -0.4, // for being charged by any enemy
    // Phase : Melee
    attack: 0.4, //for each point of damage to any enemy in melee phase
    enemy_killed_m: 0.2, //for each enemy killed
    enemy_killed_lowests_hp_m: 0.3, //for killing the lowest-HP enemy on the board.
    enemy_killed_no_overkill_m: 0.4, //for Killing an enemy unit having its HP >= RNG_DMG at the activation
...
export class SpaceMarineRangedUnit {
    constructor(name, hpMax, startPos) {
        this.name = name;
        this.hp = hpMax;
        this.pos = [...startPos];
        this.alive = true;
    }
    move(dx, dy, boardHeight, boardWidth, takenPositions) {
...
SpaceMarineRangedUnit.FACTION = "Space Marines";
SpaceMarineRangedUnit.TEAM_COLOR = 0x0078ff; // Example: blue
